# Full-run task list (work only: [done, not verified] = the work was done, verification lives on the generated board COVERAGE.md (MASTER_PLAN §5, §8); [~] = in progress / partial, [ ] = not started)

## Phase 0 — Lean research bible + preflight
- [done, not verified] Preflight: tools, disk, network probe (PREFLIGHT.md)
- [done, not verified] NEEDS_FROM_ME.md in first hour
- [done, not verified] Independent geometry extractions A and B → diff → SITE_SPEC.md (+ DEM edge check)
- [done, not verified] Chronology + date decision → CHRONOLOGY.md, DECISIONS.md
- [done, not verified] Terrain/landscape + climate sources → LANDSCAPE.md, CALENDAR_AND_UNITS.md (climate table)
- [done, not verified] ANACHRONISM_BLOCKLIST.md, SOURCES.md, OPEN_QUESTIONS.md, SUMMARY.md; stubs for the rest
- [done, not verified] Phase 0 independent review → FAIL → fixes → re-review PASS (with logged exceptions)

## Phase 1 — Engine foundation
- [done, not verified] Vite + TS + Three.js WebGPURenderer (WebGL2 fallback) scaffold, dev overlay (tiers, memory HUD)
- [done, not verified] Subsystem benchmarks (renderer path, instancing, terrain) → DECISIONS.md + budgets in README
- [done, not verified] Terrain pipeline: Copernicus GLO-30 ingest → local ENU heightfield, multi-ring; Terrace foot layer
- [done, not verified] Terrain spot-check tests
- [done, not verified] Sky: sun/moon/stars for date (proleptic Julian, ΔT) via astronomy-engine; physically based atmosphere
- [done, not verified] Time + date control; seeded climate-driven weather generator; §13.6 tests (sun 0.1°, monthly ±1°C, precip days ±20%)
- [done, not verified] Player: first person, walk/run, step-up, falling, visible body (placeholder flagged)
- [done, not verified] Shell: title, click-to-start, pause, settings, controls help
- [done, not verified] Save/load (world state + settings); seeded determinism
- [done, not verified] Test harness: vitest + Playwright (SwiftShader), camera rig, benchmark mode
## Phase 2 — [done, not verified] Terrace greybox from SITE_SPEC via parametric generators; plan overlay + dimension tests (passed with logged exceptions)
...
- [done, not verified] Reliefs (placeholder silhouettes on the reference layout), inscriptions carved from published texts
- [done, not verified] Fire and smoke, weather VFX, lightning; spatial audio + soundscape
- [done, not verified] 65 living NPCs: roster, names, schedules, needs, goods, nav grid, bodies, props, animation (D-010)
- [done, not verified] Speech + crowd murmur + language lint (D-011); addressing people (E)
- [done, not verified] Walkthrough bot over the whole slice (28 legs, pop-in probe, falls, stuck)
...
- [done, not verified] Apply the access patch; stairs, doors, frames, corrected outlines (D-013, D-015); overlay + dimension tests
- [done, not verified] Nav grid over the whole Terrace; walkthrough routes validated offline
- [done, not verified] Walkthrough e2e over all six areas (session 3, after the step-up fix D-034)
- [ ] Camera-rig views of the new areas at high quality
- [done, not verified] Reliefs on the Tachara/Hadish/Tripylon/Harem/Hall-100 stairs and jambs (355 figures), 11 windows, 9 niches, 22 working doors; far relief chunks (draws ~1,000) — D-048..D-052
...
- [done, not verified] Simulation LOD (abstract/full) with non-teleporting promotion; load-time catch-up (persistence)
- [done, not verified] Soak harness + gates (`npm run soak`); baseline measured: FAILS variety/events
...
- [done, not verified] Every simulated activity has a performance (D-142, session 4): 36 IK work cycles, 32 tools, 32 work objects, 5 animal species; `lint:activity` (0 placeholders) in `lint:all`. Seen only in the lab until the population is drawn (D-143)
...
- [done, not verified] Player step-up (D-034); offline walkthrough bot `tools/dev/botcheck.ts`; test camera floor fix
- [done, not verified] Translation layer e2e (D-036)
...
- [done, not verified] Human runtime integration merged (D-090..D-093)
- [done, not verified] Sky calibrated against the skylight; fog, cloud haze, smoke and rain shafts follow the horizon radiance (D-060)
- [~] Visitor mode (D-063): zones, rules, stop, halmi, escort, errand; e2e queued
- [ ] Clouds render check; `QUALITY=high npm run bench` re-run
- [done, not verified] Rebuild the walkable grid after the merges (D-067: rebuilt; doorways under 1.3 m no longer sealed; bots 77 + 28 legs pass offline)
## Phase 6 — Settlement
- [done, not verified] Research (SETTLEMENT.md, settlement.json); chronology rows + fail-closed lint (D-033)
...
- [done, not verified] Research (PLAIN.md, plain.json); horizon: far ring ±71.7 km + curvature, skyline matches SRTM (D-035)
- [done, not verified] Rivers, canals, fields by season, villages, orchards, Naqsh-e Rustam merged (D-037..D-040); DNa/DNb carved (D-061)
...
- [done, not verified] Interiors adapt (D-141); shadow bias (D-146); dome cover (D-145); detail at 1 m (D-147); block tone (D-148)
- [done, not verified] Probe leaks through thin walls (D-152); ground bounce in the hemisphere light (D-153); herb layer (D-154); probe tints above/below (D-158); frame meter (D-159)
- [done, not verified] Camera rig: page reuse per world state, photographic lenses, reframed moments, WebGL2 forcing, no aiming dot
...
- [done, not verified] Baseline (tsc, vitest, lint:all, bot 97/97); soak on the D-150 code passes all 8 gates
- [done, not verified] Smoke renders at test and high on WebGPU; effect-material compile failure fixed (D-174); WebGL2 at test renders
- [~] Indoor aerial perspective and no-rain-under-roofs fixes (D-174): node-tested, unrendered; Hadish floor blotches undiagnosed
- [done, not verified] Shadow review round 5 (two reviewers): FAILED; Phase 8 review (two lenses): FAILED
...
- [done, not verified] Merged: crowd at scale (D-143), sim rounds 5–7 (D-175, D-186, D-191, D-193), court in full assembly (D-182), Phase 8 layer/music/occlusion/writing/carving/voices (D-167, D-168, D-176..D-179, D-184, D-185, D-192)
- [done, not verified] Phase 8 gate PASSED (review round 3); majors M1, M2, M6 fixed
- [done, not verified] Lead render fixes: probe denoise and bake leak (D-180), floor blotches (D-181), G-buffer blending (D-183)
- [done, not verified] §8.2 rubric pass 1 (FAIL) → look workstreams merged: bugs and framing (D-187), surfaces and light (D-188), people (D-189), landscape (D-190)
...
- [done, not verified] Gap policy D-207 (user direction) and the gap audit (D-208); fills merged: D-198..D-202, D-204, D-206, D-209..D-215
- [done, not verified] Render pass 2 (partial: the moments, plain, crowd, settlement views; not every job) → rubric pass 2 FAILED → workstreams D-216 (light, materials) and D-217 (geometry, reliefs) merged; lead fixes: Tol-e Ajori panels, far flames, rig framing, rain shafts rebuilt, the rain-day planner hang, README budgets
- [done, not verified] Shadow review round 9 (FAIL: #76's jar) → D-213; round-10 input pick 181 generated (regenerate on the final tree)
- [done, not verified] Soak on the merged session-8 tree (all 8 gates); shadow review round 10 PASSED (both reviewers; S1 fixed)
