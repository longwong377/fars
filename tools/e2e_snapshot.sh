...
mkdir -p "$SNAP"
rm -rf "$SNAP"; mkdir -p "$SNAP"; tar -C "$ROOT" --exclude=./node_modules --exclude=./.git --exclude=./.claude --exclude=./shots --exclude=./test-results --exclude=./bench-reports -cf - . | tar -C "$SNAP" -xf -
