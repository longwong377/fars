"""Terrain pipeline (brief §5.2). Layers:
 ...
 3. Terrace and its foot: inside the terrace polygon (OSM) the ground is set to COURT - 0.02 m (the platform is modelled
    as geometry by the architecture generators). A 25-90 m band outside the W/N/S walls is replaced by a harmonic
...
for name, (half, cell) in rings.items():
    ...
    if cell <= 16:
        ...
        hh = h.copy()
        hh[inside] = plain_ref  # temporary Dirichlet value under the platform so no DSM terrace smear or mountain leaks into the foot
        known = ~band
        hh[band] = np.mean(h[known & ~inside])
