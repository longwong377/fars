...
function camOf(a: string[]): number[] {
  let cam: number[] = [];
...
function renderView(a: string[], out: string) {
  const cam = camOf(a);
...
// VIEWS=spec1;spec2;... renders each to <out>_<i>.png in one build (the args after the spec as usual)
const argv = process.argv.slice(2);
if (process.env.VIEWS) process.env.VIEWS.split(';').forEach((v, i) => renderView([argv[0], v, ...argv.slice(2)], argv[0].replace(/\.png$/, `_${i}.png`)));
else renderView(argv, argv[0]);
