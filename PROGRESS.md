...
- **D-232 (session 8 workstream): the Terrace retaining walls' new joint layout is NOT seen in a correct render** — render 1
  ...
  0.45-1.65 m, median 1.10; blocks 1.15-7 m, median 2.2, 26 % ≥ 3.5 m; a polygonal foot of big blocks/bedrock along 38 % of the
  W wall, 4.9 m high) and #33 (Grand Stair recess, by eye); C, one photograph (Q-600…Q-602). Kuh-e Rahmat now warm grey-brown
  with a 6/16/40 m mosaic; its 0.5-2 km texture still below the photo's (0.17 vs 0.29: B57). The plain's loam dry 10YR 5.5/3
  (Y 0.24, was a moist 10YR 4/2): brighter ground everywhere, the sky's ground bounce up ~35 % (every view; only the calib and
  rahmat-west-pm views rendered); probes re-baked. REVIEWS/calib24.md, DECISIONS D-232.
...
- **Rubric s7 pass 2, geometry bugs (D-217; R1, R2, R8, R9, R11).** Still weak: the relief carving is still procedural
  ...
  a baked sky occlusion, not a raking cast shadow. The forecourt's "white slab" is the real terrace-edge parapet (C, Q-451),
  flat because the stone material is flat (fix item 2). Relief far chunks cost 473 k triangles when all are far (was 196 k).
  Fixed and tested: R1 the shoulder jar seated on the shoulder with its neck in the hand (was hung beside the head); R2 the
  relief LODs keep every outline cell-exact and the large figures fine enough (the lion-and-bull reads at 8–20 m), panels
  6.0 cm deep; R8 the masons' blocks quarry-rough limestone (were flat vertex-coloured boxes); R9 the Naqsh cliff as
  jointed blocks (the curtain is gone; the fine wavy moiré is NOT fixed: not the bump, cause open); R11 the Apadana wall torch standing in the N doorway removed.
  Rendered at high: apadana-e-stair-raking, hall100-site, apadana-hall-out (dbg), court-assembly (dbg), humanlab carry.
- **Render pass 2 light and material bugs (D-216).** Still broken or unverified: fire light passes through parapets and
  façades (B24: the N court lit through the Apadana stair's parapet at night); the Gate's W face at dusk is still evenly
  orange (now at its physical level against the twilight sky; the brazier's intensity is C, Q-440); the Hadish doorway's
  floor streak is weaker (6.0× → 4.1× the floor) but patchy; the scribes' room's 0.5 m probe grid is verified in node only,
  NOT RENDERED (the 1 m grid's render left a patch at the jamb). Fixed and rendered: water no longer black (R3), the eye
  adapts to the fire light actually cast (R4: exposure 4.8 → 0.62 at brazier-close, clipping 5 % → 0; R5), the black
  rectangle is the starlit plain (R12), hall torches no longer light the porticoes through the walls, the traffic lanes of
  the red floors scuffed, not polished (R10). No moment re-rendered with the 8-frame spec.
...
- **Session 7, translations and Treasury tablet texts (D-198; the user's "fill the gaps").** RECONSTRUCTED, NOT
  SURVIVING: the scribes' room tablets now carry three Elamite memoranda (writing.json recon_texts PTR-1..3) composed by
  the project on the Treasury tablets' published formulary from sourced words only, impressed sign by sign and labelled
  "not a surviving text (C)" in data, F3 and the translation layer; no real PT text is used (B18 open, NEEDS #15, Q-362).
  Not written because not sourced: the letter-order form, the shekel, the per-head rate, the ration replaced. UNCHECKED:
  the translation layer now shows the PROJECT'S OWN English of all 15 carved texts (33 versions) and the 2 seal texts (4),
  tier C, labelled "Translation by the project from the ARIo edition; not a published translation; verify against Schmitt
  2009 / Kent 1953"; not checked against any published translation (B17a open, NEEDS #14, Q-363). The Aramaic chert and
  leather texts stay unwritten (no sourced word of their formula). Node only: nothing rendered in a browser.
...
- **Phase 6 (settlement; merged in session 3, D-041 to D-044):**
  ...
  - Weak or unverified:
    ...
    - **The houses are built (D-234; D-228's PLACEHOLDER lifted), with limits:** within ~72 m (32 m tiles) every house is drawn at full detail from its plan and its household's life (houseplan.ts: standing, age, repairs, additions, animals, court and roof things; 1,447 distinct configurations, none identical within 20 m): walls of mud brick on a fieldstone footing with hand-laid plaster, repairs, bare brick and soot; flat roofs of poplar poles, brush and earth laid to a fall, the eave oversailing the court on the pole ends, spouts; lintels, worn thresholds, pivot stones, windows, niches; porticoes (27), ladders, benches, mangers, fuel, washing; rooms furnished by use and a lamp each at night; street doors that shut at night and open, stand ajar or stay shut by day (E works them). Beyond ~72 m a simplified far level (plain boxes with the eave line; F3 says so). Still weak: the near/far switch pops a tile at a time; the near tiles are built on the main thread (B59); drawn people walk through shut street doors (B60); no one is drawn inside the rooms; every element is tier C (SETTLEMENT.md §8). Rendered once (D-234): court, door and Area B views inside the budget (49–58 draws, 1.25–1.70 M triangles added); the lane view timed out on the loaded box; the court render's faults were fixed after it and not re-rendered; the town from the Terrace not rendered.
    - Every layout is C: the house type is the Babylonian courtyard analogue (Q-082).
    - People live in the houses: every town household has a real plot of town_plots.json (D-081); the people walk the town's own lane and door network (`src/world/settlement/walk.ts`) and everyone out of doors is drawn (D-143; B11 counts 51 visible of 3,101 drawn in a lower-town lane at 12:11).
...
- **Phase 7 (plain; merged in session 3, D-037 to D-040):**
  ...
  - Placeholders (flagged in F3): the Neo-Elamite relief's figures are schematic silhouettes; the Elamite and Babylonian versions of DNa and DNb and the captions DNc–DNe are not carved (Q-290). The tomb reliefs are carved by the relief system (D-069: programme B, carving C), and DNa and DNb carry their Old Persian text from the CC0 edition, incised (D-061, D-177, D-184). No current render shows Naqsh-e Rustam (Phase 6+7 review M6).
  - **The plain's own Phase 7 limit (D-040: ≤ 150 calls, ≤ 2 M triangles) was exceeded at village P22 at high:** +2.56 M in the browser (D-149, D-190), 0.89 M of it the shadow passes. D-228 culls the near 3-D trees per tree to the view (main pass) and to shadows that can reach the view (shadow passes): a headless count of every pass, which gives the browser's +40 calls / 2.560 M exactly before the change, now gives 40 calls / 1.28 M with the rig's 40° lens and 41 / 1.41 M with the player's 70° (tests/plain.test.ts). **Not yet measured in the browser.**
