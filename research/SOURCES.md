...
| ARIO-CATF | R. Schmitt 2009 as ORACC ARIo in Canonical ATF (oracc/catf `ario.catf`), sign by sign with the edition's lines and marks; the carved texts' lines in `data/corpus/ario_catf.json` | FT (CC0) | the carved Old Persian sign sequence of every carved text, as the stone stood in 467 (D-184, replacing D-176/D-177's scraped Kent-convention copy, which is no longer stored), and the Elamite/Babylonian lineation (research/OP_SIGNS.md) |
...
| espeakng-loader 0.2.4 (libespeak-ng 1.52.0 and its espeak-ng-data) | PyPI (scratch venv) | the package bundles eSpeak-NG, GPL-3.0 | build tool: tools/build_speech.py renders the voices through the library (D-185); no code or data ships |
| praat-parselmouth 0.4.7 (Praat 6.1.38) | PyPI (scratch venv) | GPL-3.0 | build tool: final lowering by PSOLA (build_speech.py); measurement tool: pitch and formants (tools/voice_acceptance.py, D-185); nothing ships |
| Hollien & Shipp 1972, "Speaking fundamental frequency and chronologic age in males", JSHR 15:155-159 | abstract only (search extract; pubs.asha.org and PubMed refused by the proxy) | citation | voice pitch targets by age (D-185): men's F0 falls from the 20s to the 40s and rises from the 60s; the decade values were not read |
| Stoicheff 1981, "Speaking fundamental frequency characteristics of nonsmoking female adults", JSHR 24:437-441 | abstract only (search extract; PubMed refused) | citation | voice pitch targets by age (D-185): women's F0 falls in the 50s (after the menopause) and stays lower; the decade values were not read |
| Liberman & Pierrehumbert 1984, "Intonational invariance under changes in pitch range and length", in Aronoff & Oehrle (eds.), Language Sound Structure (MIT Press) | not re-read in this session (cited from the literature; C) | citation | final lowering in declaratives (D-185) |
| Traunmüller & Eriksson 1995, "The frequency range of the voice fundamental in the speech of male and female adults" (Stockholm University, manuscript) | abstract only (search extract; the hosts are refused) | citation | F0 variation is about equal for men and women in semitones and grows with livelier discourse; the T2 range floor (D-185) |
