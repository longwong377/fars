...
**D-223 (session 8, rubric s7 pass 2 fixes 8 and 9).** Nothing below is observed either; every value is C unless stated.

| Item | Value | Source | Access | Tier |
|---|---|---|---|---|
| Cliff bands | a cliff package's top 35 % is a riser (pinching out and resuming along the strike over ~60–120 m, cut by gullies; 47 % of its length) at 2.4 × the DEM's mean slope (≤ 76°), the rest a bench, so the package keeps the mean slope; drawn as a tilt of the shading normal (no height moves) while a riser spans ~1.5 px, then the mean; risers bare pale rock with run-off streaks (3 m × 25 m), benches darker | reconstruction (Q-520) | - | C |
| Talus, aprons, fans | talus on the upper bench under each riser; aprons where the slope 20–45 m uphill (landform map) exceeds ~27–40° and the ground is gentler; gravel fans where a gully runs out onto slopes under ~8–17° | reconstruction (Q-521) | - | C |
| Shrub crowns | spheres of 0.6–1.6 m in a 3-D jittered grid of 3 m cubes (in the lattice-free frame of materials.ts): round and one cover on any slope (down/across run ratio 0.99–1.01 from 0° to 75°; the D-190 plan grid 1.00 → 3.83); cover rule of D-190 kept | SAEIDI2021 (species); reconstruction | SX | B species / C cover |
| Worn paths | a branching net: each site's lane mouth runs clear and straight to the nearest point of the approach, the roads or an earlier path (nearest site first), each join adding wear to the branch it joins; one run to the nearest neighbouring site; 23 runs, 1 ends at the stair foot (D-190: 43 runs, 12 at the stair foot); trodden band ±3.5 m (was ±6 m) | reconstruction (Q-523) | - | C |
| Roads | cart-rut pairs 1.4 m apart each side of the axis, darker and 3 cm in; a ragged 1.5 m herb verge; ±8 % patches over ~40 m; the drawn course wanders ±2.2 m over ~520 m and ±0.8 m over ~90 m between the listed points | reconstruction (Q-526) | - | C |
| Irrigated fallow | by 800 m district, 8–32 % (mean 20 %, the data's mix), the crops in the data's proportions; the far mean follows the district | plain.json crop_mix; reconstruction (Q-524) | - | C |
| Trodden ground | wear ±60 % over 10–35 m (the herbs hold on in the less-trodden patches); bare earth ±10 % over ~4 m, ±6 % over ~1.2 m, darker warm stains over ~8 m | reconstruction | - | C |
| Town roofs and lanes | roofs sRGB 0.60 / 0.53 / 0.41 (a sun-bleached clay-and-straw finish coat); lanes and courts up to 30 % darker by the roofed share within 3 m | reconstruction (Q-525) | - | C |
