...
test('coverage', async ({ page }, info) => {
  ...
  for (const v of work) {
    ...
    try {
      ...
      console.log(`${v.id} ${v.sub} ${v.state}: ph ${r.fm.shares.placeholder} miss ${f.missing} low ${f.lowDetail} flat ${f.flatness} tile ${f.tiling?.periodic}/${f.tiling?.textured} rep ${r.rep.maxIdentical} people ${r.life?.people ?? '-'} (${((Date.now() - t1) / 1000).toFixed(0)} s)`);
    } catch (e) {
      ...
      saveJson(OUT, tag(v.id), { id: v.id, place: v.place, area: v.area, sub: v.sub, state: v.state, q: Q, project: info.project.name, error: msg, at: new Date().toISOString() });
    }
  }
  ...
  for (const pl of vwork) {
    ...
    saveJson(VOUT, `${pl.place}|${Q}|${info.project.name}`, { place: pl.place, sub: pl.sub, cam: [pl.e, pl.n, pl.eye, pl.az, pl.pitch], hour: pl.hour, w: pl.w, shots, at: new Date().toISOString() });
  }
