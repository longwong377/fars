...
// Two raking-light close-ups of the carved reliefs on the Apadana N stair (D-015): a noble/guard register at arm's length and a
...
test('carved relief close-ups', async ({ page }) => {
  ...
  await page.goto(`/?test&quality=${process.env.Q ?? 'test'}&day=60&hour=${process.env.HOUR ?? 17.8}`);
  ...
  for (const [n, e, no, h, az, p] of views) {
    ...
    console.log(n, JSON.stringify(await page.evaluate(() => (window as any).__parsa.stats().reliefs)));
