...
const VIEWS: V[] = [
  { n: 'terrace-w-dusk', day: 0, hour: 19.35, w: 'clear', ab: true, spot: 'terrace' },
  { n: 'terrace-w-day', day: 25, hour: 10, w: 'clear', ab: true, spot: 'terrace' },
  { n: 'lane-q_s1', day: 25, hour: 10.5, w: 'clear', spot: 'lane:q_s1' },
  { n: 'lane-q_w1-dusk', day: 0, hour: 19.5, w: 'clear', spot: 'lane:q_w1' },
...
test('settlement views', async ({ page }, info) => {
  ...
  mkdirSync('shots', { recursive: true }); const f = 'shots/settlement-stats.json'; const all = existsSync(f) ? JSON.parse(readFileSync(f, 'utf8')) : {};
  for (const v of VIEWS) {
    if (only && !only.includes(v.n)) continue;
    for (const town of v.ab ? [true, false] : [true]) {
      ...
      const cam = await page.evaluate((spot: string) => {
        ...
        if (spot === 'terrace') return [-58, -118, 1.6, 215, -3]; // W edge of the Terrace platform, looking SW over the lower town
        if (!S) return [-58, -118, 1.6, 215, -3];
        const plan = S.plan;
      ...
      all[tag] = { cam, ...st }; writeFileSync(f, JSON.stringify(all, null, 1));
