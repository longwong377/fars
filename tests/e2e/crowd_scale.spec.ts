...
test('crowd scale: the population drawn, measured', async ({ page }, info) => {
  ...
  const measure = async (s: Scene) => {
    ...
    const h = await page.evaluate(() => { const w = window as any, P = w.__parsa.world.people, st = P.crowd.stats(); return { byLod: st.byLod, people: st.people, impostors: st.impostors, draws: st.draws, triangles: st.triangles, shadowDraws: st.shadowDraws, shadowTriangles: st.shadowTriangles, placeholderActs: st.placeholderActs, perf: st.perf, impPerf: st.impPerf, view: st.view, attached: P.crowd.persons.size, clock: w.__parsa.clockLabel(), popins: (w.__parsa.popins ?? []).slice(-6) }; });
    const r = { scene: s.n, note: s.note, quality: q, project: info.project.name, clock: h.clock, inView: h.people + h.impostors, skinnedByLod: h.byLod, impostors: h.impostors, impostorBands: h.impPerf.bands, impostorWalking: h.impPerf.walking,
      ...
      visible,
      crowd: { draws: h.draws, triangles: h.triangles, shadowDraws: h.shadowDraws, shadowTriangles: h.shadowTriangles, attached: h.attached, placeholderActs: h.placeholderActs, placeholderImpostors: h.impPerf.placeholders, poseMs: h.perf.ms, feedMs: h.impPerf.feedMs, impMs: h.impPerf.impMs, popins: h.impPerf.popins - settle.popins0, popinList: h.popins },
