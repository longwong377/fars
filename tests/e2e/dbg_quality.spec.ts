...
// debug: one view at several quality levels / toggles, to isolate exposure and shadow problems in the post pipeline
test('quality comparison', async ({ page }, info) => {
  ...
  for (const q of (process.env.QS ?? 'test,medium,high').split(',')) {
    ...
    console.log(q, JSON.stringify(st));
    await page.screenshot({ path: `shots/dbgq-${q.replace(/[^a-z0-9]/gi, '_')}.png` });
  }
});
