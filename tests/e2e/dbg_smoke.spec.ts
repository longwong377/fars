...
// debug: where the town's smoke haze (blue) and plumes (red) are drawn in the two dusk views (?smokedbg)
test('smoke debug', async ({ page }, info) => {
  ...
  for (const [tag, v] of [['terrace', [-50.5, -120, 1.6, 215, -3]], ['slope', [250, -650, 1.6, 228, -4]]] as [string, number[]][]) {
    ...
    const st = await page.evaluate(() => { const P = (window as any).__parsa, sc = P.world.root; const h = sc.getObjectByName('settlement:haze'), pl = sc.getObjectByName('settlement:smoke-plumes');
      return { haze: h ? { vis: h.visible, n: h.count } : null, plumes: pl ? { vis: pl.visible, n: pl.geometry.instanceCount } : null, settle: P.world.settlement?.stats?.() ?? null, clock: P.clockLabel?.() }; });
