...
// drawn chunk from the same samples (physics.ts, Terrain.chunks), streamed around the player; Terrain.surfaceAt is the
...
describe('walked ground = drawn ground', () => {
  it('ray casts on the streamed colliders equal Terrain.surfaceAt everywhere (0-60 km, every ring and seam band)', () => {
    let worst = 0, n = 0;
    for (const [lo, hi] of [[0, 1900], [1984, 2048], [2048, 2112], [2100, 9900], [10176, 10304], [10300, 60000]]) {
      for (let i = 0; i < 120; i++) { const r = lo + (hi - lo) * ((i * 0.618034) % 1), a = i * 2.39996, x = r * Math.cos(a), z = r * Math.sin(a);
        ...
        worst = Math.max(worst, Math.abs(hit! - T.surfaceAt(x, z))); n++; }
    }
    expect(worst, `${n} samples`).toBeLessThan(0.005);
  });
  it('Terrain.surfaceAt is the drawn mesh itself (the chunk geometry at full resolution, ray-picked, at ring and chunk seams)', () => {
    ...
    for (const [x, z] of pts) {
      ...
      worst = Math.max(worst, Math.abs(hit.point.y - T.surfaceAt(x, z))); n++;
    }
...
describe('crossing every kind of seam (tests/lib/seams.ts; tools/dev/terrain_seams.ts runs 2,100)', () => {
  ...
  it('the audit D case: 24 eastward crossings of e = 1,984 m on Kuh-e Rahmat (12 fell before): none falls', () => {
    ...
    for (let k = 0; k < 24; k++) {
      ...
      for (let i = 0; i < 30 * 60; i++) { P.updateTerrain(T, pl.position); pl.update(1 / 30, { forward: 1, right: 0, run: false, yaw: -Math.PI / 2, pitch: 0 }); P.step(1 / 30); pl.rescueIfUnderground((a, b) => T.surfaceAt(a, b));
        if (pl.feetY - T.surfaceAt(pl.position.x, pl.position.z) < -0.3 || pl.rescues) { falls++; break; } }
...
describe('safety net (counted, so it cannot hide a bug)', () => {
  it('a body pushed 3 m under the ground is put back on it, logged and counted', () => {
    ...
    const g = T.surfaceAt(x, z), pl = new Player(P, x, g - 3, z); P.step(1 / 60);
    const warn = console.warn; let logged = ''; console.warn = (s: string) => { logged = s; };
    try { expect(pl.rescueIfUnderground((a, b) => T.surfaceAt(a, b))).toBe(true); } finally { console.warn = warn; }
    P.step(1 / 60);
    expect(pl.rescues).toBe(1); expect(logged).toMatch(/rescued the player/);
    expect(Math.abs(pl.feetY - g)).toBeLessThan(0.1);
    expect(pl.rescueIfUnderground((a, b) => T.surfaceAt(a, b))).toBe(false);
    P.world.removeCollider(pl.collider, false); P.world.removeRigidBody(pl.body); P.world.removeCharacterController(pl.controller);
  });
});
