...
import { marchCPU, sunScatter, K_MS, K_D, ambientAt, layerRadiance } from '../src/sky/cloudLight';
...
describe('cloud light (D-156)', () => {
  ...
  it('multiple scattering carries light deep into the cloud: far more than single scattering at optical depth 6 and 20', () => {
    const single = (c: number, tau: number) => sunScatter(c, 0) * Math.exp(-tau);
    expect(sunScatter(-0.5, 6) / single(-0.5, 6)).toBeGreaterThan(10);
    expect(sunScatter(-0.5, 20) / sunScatter(-0.5, 0)).toBeGreaterThan(0.1); // the diffusion term (Bohren): ~2/(2+3)
    expect(K_MS).toBeGreaterThan(0); expect(K_D).toBeGreaterThan(0);
  });
  it('seen from below, the base of a thick layer is grey (Bohren two-stream transmission for cumulus, ~0.25), darker than its sunlit top; thin cloud lets the sky through', () => {
    const lambert = Math.sin(40 * D) / Math.PI, top = layerRadiance(0.02, 1000, 40, -60, 180), base = (layerRadiance(0.02, 1000, 40, 60, 180, false) + layerRadiance(0.02, 1000, 40, 85, 180, false) + layerRadiance(0.02, 1000, 40, 60, 0, false) + layerRadiance(0.02, 1000, 40, 85, 0, false)) / 4;
    expect(base / lambert).toBeGreaterThan(0.18); expect(base / lambert).toBeLessThan(0.32);
    expect(base).toBeLessThan(top * 0.8);
