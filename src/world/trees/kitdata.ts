...
import { allModels, M0, K0, VARIANTS, type TreeModel } from './model';
...
/** texels per record */
export const SEG_TEX = 3, CARD_TEX = 5, SP_TEX = 3;
...
/** card texture (width K0 x 5, one row per model): (c, size), (w, ht), (up, hb), (side, tint), (n, ao) */
export function packCards(models: TreeModel[] = allModels()) {
  ...
  models.forEach((m, r) => m.cards.forEach((c, i) => out.set([c.c[0], c.c[1], c.c[2], c.size, c.w[0], c.w[1], c.w[2], c.ht, c.up[0], c.up[1], c.up[2], c.hb,
    c.side[0], c.side[1], c.side[2], c.tint, c.n[0], c.n[1], c.n[2], c.ao], (r * w + i * CARD_TEX) * 4)));
  return { data: out, width: w, height: models.length };
}
/** species texture (width 3, one row per model): (group, leaf tile, twig tile, blossom tile or -1), (bark rgb linear, H),
 *  (impostor tile side T, y0, W, CB) */
export function packSpecies(models: TreeModel[] = allModels()) {
  const out = new Float32Array(SP_TEX * models.length * 4);
  models.forEach((m, r) => { const s = m.species, o = r * SP_TEX * 4;
    const bk = barkLinear(m); out.set([groupIndex(s.group), tileIndex(s.leaf.tile), tileIndex(s.twig_tile), s.blossom_tile ? tileIndex(s.blossom_tile) : -1, bk[0], bk[1], bk[2], m.H, m.T, m.y0, m.W, m.CB], o); });
