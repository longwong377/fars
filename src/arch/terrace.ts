...
import { Part, Pt, Box, Prism, Column, ColumnOrder, Manifest, wallRing, grid, BuildResult, Material } from './parts';
...
export function buildTerrace(): BuildResult {
  ...
  if (present('gate_nations')) {
    ...
    // and plan footprint, rendered as a colossus carved from the jamb (sculpt.ts, D-014): head facing out of the
    ...
    for (const [side, sx, dir] of [['W', x0, -1], ['E', x1, 1]] as const) for (const dy of [-1, 1] as const) {
      ...
      parts.push(box(b, 'colossus', 'limestone', 'C', srcOf(row(b, 'guardians'), row(b, 'r_colossus')), pc, [K.length, K.width], fl + pl, fl + pl + K.height,
        { solid: true, sculpt: { model: /human-headed/.test(G[side]) ? 'lamassu' : 'bull', facing: dir, passage: dy === 1 ? -1 : 1 },
          note: `${G[side]} colossus (IR-PERS, B) carved from the jamb: procedural sculpture, form C (D-014)` }));
    }
