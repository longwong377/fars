...
export function figurePieces(kind: string, rng: Rng): Piece[] {
  ...
  switch (kind) {
    ...
    case 'horse': P.push({ shape: S([[-0.35, 0.28], [0.2, 0.3], [0.33, 0.48], [0.4, 0.46], [0.3, 0.3], [0.28, 0.02], [0.23, 0.02], [0.18, 0.2], [-0.2, 0.2], [-0.25, 0.02], [-0.3, 0.02], [-0.33, 0.2], [-0.4, 0.3]]), colour: PIGMENT.stone, depth: d }); break;
    case 'bull': P.push({ shape: S([[-0.35, 0.22], [0.18, 0.3], [0.32, 0.36], [0.38, 0.3], [0.28, 0.2], [0.26, 0.02], [0.2, 0.02], [0.16, 0.14], [-0.2, 0.14], [-0.25, 0.02], [-0.3, 0.02], [-0.33, 0.14]]), colour: PIGMENT.stone, depth: d }); P.push({ shape: ellipse(0.02, 0.34, 0.08, 0.06, 10), colour: PIGMENT.stone, depth: d }); break; // humped
    case 'camel': P.push({ shape: S([[-0.35, 0.35], [-0.2, 0.5], [-0.05, 0.42], [0.08, 0.5], [0.2, 0.36], [0.3, 0.55], [0.37, 0.55], [0.3, 0.3], [0.25, 0.02], [0.2, 0.02], [0.17, 0.25], [-0.22, 0.25], [-0.26, 0.02], [-0.31, 0.02], [-0.33, 0.25]]), colour: PIGMENT.stone, depth: d }); break;
    case 'ram': P.push({ shape: S([[-0.2, 0.12], [0.1, 0.18], [0.2, 0.28], [0.26, 0.22], [0.18, 0.12], [0.16, 0.01], [0.12, 0.01], [0.1, 0.08], [-0.12, 0.08], [-0.15, 0.01], [-0.19, 0.01], [-0.22, 0.1]]), colour: PIGMENT.stone, depth: d }); break;
    case 'lion': P.push({ shape: S([[-0.4, 0.3], [0.1, 0.34], [0.22, 0.46], [0.36, 0.44], [0.4, 0.34], [0.3, 0.28], [0.28, 0.02], [0.22, 0.02], [0.18, 0.18], [-0.22, 0.18], [-0.28, 0.02], [-0.34, 0.02], [-0.36, 0.2], [-0.5, 0.36], [-0.46, 0.3]]), colour: PIGMENT.stone, depth: d }); P.push({ shape: ellipse(0.24, 0.4, 0.12, 0.1, 12), colour: PIGMENT.stone, depth: d * 1.2, z: 0.002 }); break; // mane
...
export interface Placement { kind: string; variant: number; along: number; y: number; facing: 1 | -1; scale: number }

/** Plan the figures on one façade (all dimensions from SITE_SPEC). */
export function planFacade(f: Facade): Placement[] {
  const prog = v<any>('apadana', 'relief_programme')[f.id], R = v<any>('apadana', 'r_registers'), sp = v('apadana', 'r_figure_spacing');
  const third = f.length / 3, out: Placement[] = [], rng = new Rng(1, 'relief-' + f.id), nDel = v('apadana', 'r_delegation_members');
  ...
  for (const wing of ['left', 'right'] as const) {
    const content = prog[wing + '_wing']; const sgn = wing === 'left' ? -1 : 1; // along axis: −L/2 … +L/2 (viewer's left → right)
    const a0 = sgn > 0 ? third / 2 : -f.length / 2, a1 = sgn > 0 ? f.length / 2 : -third / 2;
    const facing: 1 | -1 = sgn > 0 ? -1 : 1; // walk toward the centre
    for (let r = 0; r < R.count; r++) {
      const y = R.bottom + r * (R.height + R.gap);
      if (content === 'nobles') {
        let k = 0; for (let a = a0 + sp / 2; a < a1 - sp / 2; a += sp, k++) out.push({ kind: r === 0 ? 'guard' : (k % 2 ? 'mede' : 'persian'), variant: rng.int(0, 5), along: a, y, facing, scale: 1 });
      } else {
        // delegations: registers filled in turn, each delegation = usher + members (+ animal), separated by a cypress
        const perReg = Math.ceil(DELEG.length / R.count);
        let a = facing > 0 ? a1 - sp / 2 : a0 + sp / 2; const stepA = -facing * sp; // head of each file at the centre, extending outward
        for (let di = r * perReg; di < Math.min(DELEG.length, (r + 1) * perReg); di++) {
          ...
          if ((facing > 0 && a < a0) || (facing < 0 && a > a1)) break;
        }
      }
    }
  }
