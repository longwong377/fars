...
/** Column geometry in local space (base at y = 0, top at the order's height): the sculpted order (sculpt.ts, D-014).
...
export function buildMeshes(parts: Part[], phys?: Physics): BuiltArch {
  ...
  for (const [, c] of cols) {
    ...
    lod.userData = { tier: c.parts[0].tier, src: `${c.parts[0].src};RECON`, placeholder: false, building: b,
      note: `column order ${c.order.id} (${c.order.base} base, ${c.order.capital} capital): dimensions SITE_SPEC; carving procedural sculpture, form C (D-014; scans would replace it, NEEDS #10)${c.built < 1 ? '; under construction: unfluted drums' : ''}` };
