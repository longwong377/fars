...
class SSGINode extends TempNode {
	...
	setup( builder ) {
		...
		// PĀRSA: the depth clear value marks the sky (reversed-Z clears to 0; standard and logarithmic depth to 1)
		const isSkyDepth = ( d ) => ( builder.renderer.reversedDepthBuffer === true && builder.renderer.backend.isWebGPUBackend === true ) ? d.lessThanEqual( 0.0 ) : d.greaterThanEqual( 1.0 );
		...
		const horizonSampling = Fn( ( [ directionIsRight, stepRadius, radiusVS, viewPosition, slideDirTexelSize, initialRayStep, uvNode, viewDir, viewNormal, n ] ) => {
			...
			Loop( { start: uint( 0 ), end: STEP_COUNT, type: 'uint', condition: '<' }, ( { i } ) => {
				...
				If( isSkyDepth( rawSampleDepth ), () => {
					...
					Continue(); // PĀRSA: the sky is not an occluder nor a GI source (the hemisphere light carries skylight)
