...
// are zero, which both additive and alpha blending leave unchanged — provided those attachments blend at all: three blends
// only `output` by default, so the pipeline sets the G-buffer outputs to the material's blending (D-183). Before, a flame quad added its own normal and colour
