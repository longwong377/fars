...
/** the terrain horizon's sun visibility, published by the SkySystem each frame: at the eye (the hall air-light pass,
 *  render/airlight.ts) and at any world point (for the eye model: probes/runtime.ts probeEyeVisibility's sunlit test and
 *  the probes' sun channel U, main.ts's outdoor sunE — the lead's files, not wired here; see D-156) */
export const EYE_SKY = { sunVisibility: 1, sunVisibilityAt: (_x: number, _y: number, _z: number): number => 1 };
