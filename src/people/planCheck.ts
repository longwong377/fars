...
export type PlanIssue = 'no_sleep' | 'reason' | 'meals' | 'teleport' | 'apart' | 'alone';
...
/** on one day, for everyone: a person "with" another is where that one is; a child under ten is not alone at night */
export function checkDay(P: Population, d: number, planOf: (pid: number) => Seg[]): { kind: PlanIssue; pid: number; note: string }[] {
  ...
  for (let pid = 0; pid < P.persons.length; pid++) {
    ...
    if (p.age < 10 && p.agent < 0) for (const h of [1.5, 23.5]) { const s = segAt(segs, h); if (s.where === 'road' || s.place === '-') continue;
