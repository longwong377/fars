...
export interface PTask { kind: 'reap' | 'thresh' | 'vintage' | 'fruit' | 'plough' | 'canal' | 'turn' | 'field' | 'other'; act: ActivityId; place: string; why: string; h0: number; h1: number; all: boolean; sheaves: boolean; late: number;
  ...
  /** threshing: the afternoon's wind works, and the afternoon session is the winnowing into the evening (A S6 of r8) */
  eveningWind?: boolean;
...
/** a wedding's hours (Population.weddingPlan; E-73, D-211) */
export interface WedPlan { day: number; dress: [number, number]; proc: number; arrive: number; walk: number; walk0: number; feast: [number, number]; supper: [number, number]; drum: [number, number]; leave: number; fW: Where; tW: Where; fromHome: string; toHome: string; dark: boolean }
...
export interface HDay { eaters: number; women: number[]; bake: boolean; baker: number; breakfast: number; bLen: number; noon: number; nLen: number; supper: number; sLen: number; grindEach: number; birthday: number; task: PTask | null;
  ...
  /** a girl of 9-13 who fetches one of the jars as her chore today, or -1 (Population.jarsOf; A S7 of shadow review r9) */
  helper: number;
...
const RATION_Q_H: [number, number] = [0.2, 0.55];
...
/** children's games in the lane (D-211; gap audit item 26): knucklebones are attested as finds (astragali: B); balls, tops, the
 *  wheeled clay animals of Susa and chasing games are the most probable others (C). Words only: the crowd shows the play pose */
export const GAMES = ['playing knucklebones on the beaten earth of the lane with the other children', 'playing ball in the lane with the neighbours’ children', 'playing chase along the lane with the other children',
  'playing with a top, whipping it along the lane', 'pulling a clay animal on wheels along the lane', 'playing at houses in the lane with the other children'];
/** the detailed porters come up this long before the day's caravan to carry what was left at the depot (C) */
export const DEPOT_EARLY_H = 1;
// ------------------------------------------------------------------ out of doors, and the weather's hours (D-191: one definition for
// the planners' passes and the checks of planCheck.ts)
...
export const CAMP_OPEN = /(weaving at the ground loom|spinning wool|playing|talking with the men of the band|sitting|minding the little ones|resting) by the (new )?tents?\b|by the fire (with the (band|family)|, telling|while)/;
/** (e) a baby's age in months on day d (as tools/shadow_days ageStr) */
export function monthsOld(P: Population, c: number, d: number) { const p = P.persons[c]; const born = p.born >= 0 ? p.born : (p.bday >= 0 ? p.bday - 354 * (p.age + 1) : -Math.round(354 * (p.age + 0.5))); return Math.max(0, (d - born) / 29.5); }
/** (e) the longest gap between a baby's feeds at night, by its age (infant_care.night_gap_max_h: [up to months, hours]) */
export function nightGapMax(months: number): number { for (const [m, h] of (L.infant_care.night_gap_max_h?.v ?? []) as [number, number][]) if (months < m) return h; return 99; }
/** the length of a rest "through the heat" (planCheck (d)): from the first to the last spell so worded at the place,
 *  counting short breaks between them (a feed, a drink) as part of it */
export function heatStretch(segs: Seg[], i: number) {
  const s = segs[i]; let a = i, b = i; const heat = (x: Seg) => x.place === s.place && /through the heat/.test(x.why), gap = (x: Seg) => x.place === s.place && x.where !== 'road' && x.t1 - x.t0 <= 0.5;
  for (let j = i - 1; j >= 0 && (heat(segs[j]) || gap(segs[j])); j--) if (heat(segs[j])) a = j;
...
export function wetSpells(wx: DayCtx['wx']): [number, number][] {
  const all = [...rainSpells(wx), ...(wx.stormH ? [[wx.stormH[0], wx.stormH[1]] as [number, number]] : [])].sort((a, b) => a[0] - b[0]); const out: [number, number][] = [];
  for (const x of all) { const L0 = out[out.length - 1]; if (L0 && x[0] <= L0[1] + 0.25) L0[1] = Math.max(L0[1], x[1]); else out.push([x[0], x[1]]); } return out;
}
/** "back from the field" (a meal's words, Planner.backFrom): the person was at such a place in the three hours before */
export function backFromOk(segs: Seg[], i: number) {
  const s = segs[i], m = /back from (the threshing floor|the vineyard|the orchard|the canal|the garden beds|gathering fuel|the field)/.exec(s.why); if (!m) return true;
  const pre = ({ 'the threshing floor': 'threshing', 'the vineyard': 'vineyard', 'the orchard': 'orchard', 'the canal': 'canal', 'the garden beds': 'garden', 'gathering fuel': 'outside', 'the field': 'field' } as Record<string, string>)[m[1]];
  for (let j = i - 1; j >= 0 && s.t0 - segs[j].t1 < 3; j--) if (segs[j].place.startsWith(pre) || (pre === 'field' && segs[j].place.startsWith('crown_fields'))) return true; return false;
}
...
export function dustWear(segs: Seg[], wx: DayCtx['wx']) {
    ...
    for (const s of segs) if (s.wear && /dust/.test(s.wear)) { const w = s.wear.replace(/(, and )?the face wrapped against the dust/, '').replace(/^, and /, ''); if (w) s.wear = w; else delete s.wear; }
    const dh = wx.dustH; if (!dh) return;
    ...
    for (let i = 0; i < segs.length; i++) { const s = segs[i]; if (!outdoors(s) || s.act === 'eat' || Math.min(s.t1, dh[1]) - Math.max(s.t0, dh[0]) <= 0.05) continue;
      ...
      if (e - dh[1] > 0.25) parts.push({ ...s, t0: dh[1], t1: e });
      segs.splice(i, 1, ...parts); i += parts.length - 1; }
}
/** dressed against the cold out of doors (brief §9.2; S5 of shadow review r6): where the air is below outfits.COLD_C (8 °C;
   ...
   *  shoes; the Persian robe unchanged). Joined with the dust's wrap when both. Asleep, carried, or with a companion too (S3
   *  of reviewer B, r7: a child of 15 months asleep in the lane at 0 °C had none); a spell is cut where the air crosses the
   *  threshold (a sliver of under three minutes goes to the run before), a walk under 45 min is not */
export function coldWear(segs: Seg[], wx: DayCtx['wx'], agent = false) {
    const tq = wx.tempQ; if (!tq?.length || Math.min(...tq) >= COLD_C) return;
    const cold = (q: number) => tq[Math.max(0, Math.min(95, q))] < COLD_C;
    const add = (s: Seg) => { if (/cold/.test(s.wear ?? '')) return; s.wear = s.wear ? `${s.wear}, and dressed against the cold` : 'dressed against the cold'; };
    // (a plan copied from another's, a child's from its carer's, brings her dress with it: taken off and decided here again)
    const bare = (s: Seg) => { if (!s.wear || !/cold/.test(s.wear)) return; const w = s.wear.replace(/(, and )?dressed against the cold/, '').replace(/^, and /, ''); if (w) s.wear = w; else delete s.wear; };
    for (let i = 0; i < segs.length; i++) { const s = segs[i]; bare(s); if (!outdoors(s)) continue;
      const runs: [number, number, boolean][] = []; let a = s.t0, c = cold(Math.floor(s.t0 * 4));
...
/** a storm in the day's light (W-02 keeps the day's outdoor work in); a storm only in the night keeps no one in by day
 *  (D-191: "at home: storm" all day for a storm at 02:00) */
export function dayStorm(C: DayCtx) { const s = C.wx.stormH; return !!s && s[1] > C.sun.rise - 0.5 && s[0] < C.sun.set + 0.5; }
/** the words of a day kept at home by the weather ("at home: storm", "storm: no work", "rain: no garden work", "kept in by the
 *  rain"): which weather they name, or null */
export function weatherWord(why: string): 'storm' | 'rain' | null { const m = /^(?:at home: )?(storm|heavy rain|rain)(?:: |$)|kept in by the (heavy rain|rain|storm)/.exec(why); if (!m) return null; const k = m[1] ?? m[2]; return k === 'rain' ? 'rain' : 'storm'; } // ("heavy rain" is the storm's span without thunder: A S8 of shadow review r9)
/** the weather named is on within half an hour of [a, b] (the storm's span; the rain's quarter-hours) */
export function weatherNear(wx: DayCtx['wx'], k: 'storm' | 'rain', a: number, b: number) {
  if (k === 'storm') { const st = wx.stormH; return !!st && a < st[1] + 0.5 && b > st[0] - 0.5; }
  return rainHours(wx, a - 0.5, b + 0.5) > 0 || (!!wx.stormH && a < wx.stormH[1] + 0.5 && b > wx.stormH[0] - 0.5);
}
/** the least dry hours in a working day for the day's work to go on (A S3 and B S2 of shadow review r8: a storm at 15:30, as
 *  work ended, kept all 612 builders at home; C) */
export const DRY_WORK_H = 3.5;
/** the part of a working window [w0, w1] that the weather leaves for work (W-01, W-02; A S3 and B S2 of shadow review r8): the
 *  gangs set out when a wet spell that holds the start has passed and stop where one comes on that lasts to the end; the
 *  showers between are sheltered from (Planner.workBlock). A storm's span counts with a quarter of an hour either side (the
 *  sky darkens before it, the ground is awash after it). Under a roof (`roofed`) only the storm keeps people in (the walk up
 *  is not made in it). `dry`: the hours of [a, b] out of the wet; the day's work is given up when fewer than DRY_WORK_H
 *  (was: any storm in the day's light, or more than 5 rain hours anywhere in the day, even after the work's end; C) */
export function workSpan(wx: DayCtx['wx'], w0: number, w1: number, roofed = false): { a: number; b: number; dry: number } {
  const st = wx.stormH, sp = (roofed ? (st ? [[st[0], st[1]] as [number, number]] : []) : wetSpells(wx))
    .map(([x, y]) => (st && x <= st[1] && y >= st[0] ? [x - 0.25, y + 0.25] : [x, y]) as [number, number]).filter(([x, y]) => y > w0 && x < w1);
  let a = w0, b = w1;
  for (const [x, y] of sp) if (x <= a + 0.25 && y > a) a = y;
  for (const [x, y] of [...sp].reverse()) if (y >= b - 0.25 && x < b) b = x;
  if (b <= a) return { a, b: a, dry: 0 };
  let wet = 0; for (const [x, y] of sp) wet += Math.max(0, Math.min(y, b) - Math.max(x, a));
...
export function wetHours(wx: DayCtx['wx'], a: number, b: number) {
  ...
  for (let q = Math.max(0, Math.floor(a * 4)); q < 96 && q * 0.25 < b; q++) { const x0 = Math.max(a, q * 0.25), x1 = Math.min(b, q * 0.25 + 0.25); if (x1 <= x0) continue;
    if (wx.rainQ[q]) h += x1 - x0; else if (st) h += Math.max(0, Math.min(x1, st[1]) - Math.max(x0, st[0])); }
  return h;
}
...
export class Population {
  ...
  private rawCache = new Map<number, Map<number, Seg[]>>(); private rawCount = 0;
  ...
  private precomputeLife() {
    ...
    for (const b of brides) {
      const um = u01(this.seed, S.marry, b); if (um >= pm) continue; const B = this.persons[b], H = this.households[B.hh];
      // (D-211: about half the year's weddings round the turn of the year and the spring equinox, 1 Addaru to 21 Nisannu,
      ...
      B.spouse = g; G.spouse = b; { const w = { day, bride: b, groom: g, from: B.hh, to }; (this.weddingList[day] ??= []).push(w); }
    ...
    this.foster();
    this.bereaved();
    // again after the kinswomen have moved to keep bereaved houses: a daughter of 17 who went to keep her kin's house was
    // the last grown-up of her own when both parents died later in the year, and her brothers of 11 and 8 were left alone
    // (soak on D-175; household 3794). Children already moved are not moved again
    this.foster();
  }
  ...
  nurslings(pid: number, d: number) {
    ...
    const fos = (this.nursedBy.get(pid) ?? []).filter(c => d >= this.persons[c].marry && this.ageOn(c, d) <= 1 && this.present(c, d) && this.home(c, d) === this.home(pid, d)); return [...own, ...fos]; } // (weaned at two, as her own: D-175)
  ...
  draftedOn(pid: number, d: number, C: DayCtx) {
    const p = this.persons[pid], H = L.harvest_draft; if (!(C.agri.has('E-41') || C.agri.has('E-42')) || dayStorm(C) || this.households[this.home(pid, d)].zone !== 'town' || this.ageOn(pid, d) < 14 || p.group < 0) return false;
  ...
  hday(h: number, d: number): HDay {
    ...
    // (no field work on a festival day or at a wedding of the house: D-211)
    const winter = C.season === 'winter'; const bake = (d + h) % hb.bake_every_days === 0; const task0 = H.zone === 'plain' && !C.festival && !(this.weddingList[d] ?? []).some(w => w.from === h || w.to === h) ? this.ptask(h, d, C) : null, task = task0 ? this.dryTask(task0, C) : null;
    ...
    const supper = C.sun.set - M.supper_before_sunset_h - (winter ? M.winter_earlier_h : 0) + (task?.sheaves || (task?.eveningWind && task.pm) ? M.harvest_later_h : 0) + habit; // (later after a harvest day's sheaves or an evening's winnowing)
    ...
    const kidsIn = mem.filter(x => { const q = this.persons[x]; return this.ageOn(x, d) >= 7 && this.ageOn(x, d) <= 13 && (q.job === 'child' || (q.job === 'farmer' && !(task?.all && this.ageOn(x, d) >= 12))) && !this.sick(x, d) && !this.mourning(x, d) && x !== keeper && q.agent < 0; }).sort((a, b) => this.ageOn(b, d) - this.ageOn(a, d) || a - b); // (not a child in mourning: its day is the mourning day, not child()'s, so it holds no minding: soak on D-175)
    ...
    const bringer = fieldT && task!.h1 > noon + 0.5 && rainHours(C.wx, noon - 1, Math.min(task!.h1, noon + 1.5)) === 0 && !dayStorm(C) ? kidsIn.filter(x => x !== minder && x !== fieldHelper).slice(-1)[0] ?? -1 : -1;
    ...
    const r: HDay = { eaters, women, bake, baker: women[0] ?? -1, breakfast, bLen, noon, nLen, supper, sLen, grindEach: hb.grind_h_per_eater * eaters / Math.max(1, women.length), birthday, task, habit,
      minder, fieldHelper, bringer, kneadH, bakeH, slack: hb.slack_h[0] + (hb.slack_h[1] - hb.slack_h[0]) * u(3), bakeAM: false, waterer: -1, helper: -1, jars: 0, wetOff: !!task0 && !task };
    ...
    if (H.zone === 'town' || H.zone === 'plain') {
      ...
      // woman draws the rest (C: jarsOf)
      const girl = mem.filter(x => this.persons[x].job === 'child' && this.persons[x].sex === 'f' && this.ageOn(x, d) >= 9 && this.ageOn(x, d) <= 13 && !this.sick(x, d) && this.persons[x].agent < 0 && x !== r.waterer && x !== minder && !keep.has(x) && !this.mourning(x, d))[0];
      if (r.jars >= 2 && girl !== undefined && r.waterer >= 0 && this.persons[r.waterer].sex === 'f' && u(9) < 0.5) r.helper = girl; }
  ...
  ptask(h: number, d: number, C: DayCtx): PTask | null {
    ...
    // the winnowing waits for the wind (Q-141: the afternoon and evening breeze, the traditional winnowing hour; C): when the
    // afternoon's wind works, the household goes back to the floor from about 16:00, or when the heat breaks if earlier, and
    // winnows into the evening, half an hour before sunset; in still air the straw is turned after the heat as before (A S6 of
  ...
  dryTask(T0: PTask, C: DayCtx): PTask | null {
    ...
    // (a storm counts as the rain does, its span and a quarter of an hour after it: a storm in the afternoon no longer keeps the
    // men in through a dry morning, nor one that has passed by breakfast through the day; A S3 of shadow review r8)
    const w = C.wx; if (!w.rain && !w.stormH) return T; const len = T.h1 - T.h0; if (len <= 0) return T;
    if (wetHours(w, T.h0, T.h1) >= 0.5 * len) return null;
    const st = w.stormH, sp = wetSpells(w).map(([a, b]) => (st && a <= st[1] && b >= st[0] ? [a, b + 0.25] : [a, b]) as [number, number]).find(([a, b]) => a <= T.h0 + 0.25 && b > T.h0);
    if (!sp) return T;
    const h0 = sp[1] + 0.2; // they set out once it has cleared (C)
    if (T.h1 - h0 < Math.max(1.5, len / 3) || wetHours(w, h0, T.h1) > 0.3 * (T.h1 - h0)) return null;
  ...
  toddlerSleep(pid: number, d: number, wakeM: number) {
    ...
    // (not before sunset + 0.6 h, later with age: S4 of reviewer B, r7, a child of 15 months asleep 2 min after sunset and 16.5 h
    // asleep in 24; 11-14 h at one to two years, RECOLLECTION, NOT SEEN; C)
    const bedtime = Math.max(C.sun.set + 0.6 + 0.15 * a, Math.min(21, hd.supper + hd.sLen + 0.4 + (a >= 3 ? 0.5 : 0))), wakeT = Math.max(wakeM, hd.breakfast - 0.15 - 0.3 * u(0));
  ...
  mindDay(h: number, d: number): MindDay {
    ...
    if (this.weddingOf(mn, d) || this.festDay(mn, d)) return out; // (a festival or a wedding: the little ones are with their mothers, D-211)
    ...
    for (const k of mem) { const q = this.persons[k], a = this.ageOn(k, d); if (q.job !== 'child' || a < 1 || a > 4 || this.sick(k, d) || q.agent >= 0 || k === mn) continue;
      ...
      const ms = this.rawPlan(mo, d); const wakeM = ms.find(s => s.t0 > 2 && s.act !== 'sleep' && !NIGHT_FEED.test(s.why))?.t0 ?? 6; const sl = this.toddlerSleep(k, d, wakeM);
  ...
  /** a sick guard is nursed at home by his family today (lives.json guard_sick_home; C): an illness of three days or more (two
   *  leave no day to be nursed between the walk down and the walk back up), a wife (a woman of 14 or more) of his house in the
   *  town well on its second day, and the episode's draw */
  guardNursedHome(pid: number, d: number) {
    const p = this.persons[pid], H = this.households[p.hh]; if (p.job !== 'guard' || H.zone !== 'town' || !this.sick(pid, d)) return false;
    const { k, n } = this.sickDayOf(pid, d); if (k < 1 || n < 3) return false; const s0 = d - k; // (down on the second morning, up on the last evening)
    return this.membersOn(H.id, s0 + 1).some(x => x !== pid && this.persons[x].sex === 'f' && this.ageOn(x, s0 + 1) >= 14 && !this.sick(x, s0 + 1))
      && u01(this.seed, S.sick, pid, s0, 77) < L.guard_sick_home.v;
  }
  ...
  hideDelivery(d: number): { t0: number; t1: number; n: number; receivers: number[] } | null {
    ...
    if (n > 0) { let t0 = 99, t1 = -1;
      for (const c of this.shepherds) { if (this.persons[c].sub !== 'hides' || !this.present(c, d)) continue; for (const s of this.rawPlan(c, d)) if (/^delivering hides/.test(s.why)) { t0 = Math.min(t0, s.t0); t1 = Math.max(t1, s.t1); } }
  ...
  nightAway(pid: number, d: number): { place: string; where: Where } | null {
    ...
    const segs = this.rawPlan(pid, d - 1), last = segs[segs.length - 1];
  ...
  farmChore(pid: number, d: number, C: DayCtx): [ActivityId, string] | null {
    ...
    if (!ok.length) return null; const c = ok[Math.floor(u01(this.seed, S.assign, 7400 + pid, d) * ok.length)];
    // (after the first rain the roof is not plastered "before the rains": it is mended where the rain came through; S1 of
    // shadow review r7, 61,748 person-days so labelled after the rains had come)
    return [c.act, C.firstRain && /before the rains/.test(c.why) ? 'mending the roof with mud and straw where the rain came through' : c.why];
  }
   ...
   *  A S2 and B S3 of shadow review r8: a sick child of 1-4 went wherever its mother worked, 91 % of 31,434 sick child-days,
   *  20,797 h on the threshing floors, in the lane at 2-7 °C beside a grandfather at home; C). A baby is its mother's (or its
   *  wet nurse's: the breast); a child of one to four is kept by the grandparent of the house when there is one fit to keep
   *  it, else by its mother, else by another woman of the house, else by a man of it. Fit: here, not ill, not in mourning, not in the days after a
   *  birth, not marrying, not a guard. A child of one still at the breast is its mother's too. The keeper stays in all day
   *  (Planner.nursing): no outings, no lane, no well */
  sickKeepers(h: number, d: number): Map<number, number> {
    const key = h * 512 + d; const c = this.keepCache.get(key); if (c) return c; if (this.keepCache.size > 60000) this.keepCache.clear();
    const out = new Map<number, number>(); const mem = this.membersOn(h, d);
    const up = (x: number) => x >= 0 && mem.includes(x) && this.present(x, d) && !this.sick(x, d) && this.ageOn(x, d) >= 14 && this.persons[x].job !== 'guard' && this.persons[x].marry !== d && this.gaveBirth(x, d) !== 0;
    const fit = (x: number) => up(x) && !this.mourning(x, d) && !(this.gaveBirth(x, d) >= 0 && this.gaveBirth(x, d) <= this.postpartumDays(x));
    for (const c0 of mem) { const q = this.persons[c0]; if (q.job !== 'child' || this.ageOn(c0, d) > 4 || !this.present(c0, d) || !this.sick(c0, d)) continue;
      const nurse = q.nurse !== undefined && d >= q.marry ? q.nurse : -1;
      // (the mother in her days after a birth keeps her own baby: her day is at home, Planner.postpartum)
      const k = this.ageOn(c0, d) <= 1 && (up(q.mother) || up(nurse)) ? (up(q.mother) ? q.mother : nurse) : this.ageOn(c0, d) === 0 ? -1
        : mem.filter(x => this.persons[x].job === 'elder' && fit(x)).sort((a, b) => (this.persons[a].sex === 'f' ? 0 : 1) - (this.persons[b].sex === 'f' ? 0 : 1) || a - b)[0]
          ?? (fit(q.mother) ? q.mother : mem.find(x => this.persons[x].sex === 'f' && fit(x)) ?? (up(q.mother) ? q.mother : mem.find(x => fit(x) && x !== c0 && this.vigilMan(h, d - 1) !== x) ?? -1)); // (a house with no woman: its man; not the one back at first light from a night by the grain heap)
      if (k >= 0) out.set(c0, k); }
    this.keepCache.set(key, out); return out;
  }
  private keepCache = new Map<number, Map<number, number>>();
  /** this person keeps a sick little one of the house at home today (sickKeepers) */
  keepsSick(pid: number, d: number) { for (const k of this.sickKeepers(this.home(pid, d), d).values()) if (k === pid) return true; return false; }
  /** the woman who stays home with a sick household member on the first day (C) */
  carerToday(pid: number, d: number) {
    const p = this.persons[pid]; if (this.keepsSick(pid, d)) return true; // (a sick little one's keeper, every day of its illness: sickKeepers)
    if (p.sex !== 'f' || this.ageOn(pid, d) < 14) return false; const H = this.households[this.home(pid, d)];
  ...
  // ================================================================== festivals, weddings, hearings, healers (D-211; C)
  /** the person's day is not taken already by what build0 decides first: here, not a traveller or a herder, not ill, not in
   *  labour or the days after a birth, not a little one, not a guard, not in mourning, not keeping the sick at home */
  private dayFree(pid: number, d: number) {
    const p = this.persons[pid]; if (!this.present(pid, d) || p.job === 'traveller' || p.job === 'herder' || p.job === 'guard' || p.zone === 'transient') return false;
    if (this.sick(pid, d) || (p.job === 'child' && this.ageOn(pid, d) < 5)) return false;
    const b = this.gaveBirth(pid, d); if (b === 0 || (b > 0 && b <= this.postpartumDays(pid))) return false;
    return !this.mourning(pid, d) && !this.carerToday(pid, d);
  }
  ...
  weddingsOn(d: number) {
    const c = this.wedCache.get(d); if (c) return c;
    const out = (this.weddingList[d] ?? []).filter(w => this.dayFree(w.bride, d) && this.dayFree(w.groom, d) && ['town', 'plain'].includes(this.households[w.from].zone) && ['town', 'plain'].includes(this.households[w.to].zone));
    if (this.wedCache.size > 400) this.wedCache.clear(); this.wedCache.set(d, out); return out;
  }
  /** a person's part in a wedding today: the bride, the groom, one of the house she leaves ('from') or of the house of the
   *  feast ('to'); null when none or when the person's day is taken already (dayFree) */
  weddingOf(pid: number, d: number): { w: { day: number; bride: number; groom: number; from: number; to: number }; role: 'bride' | 'groom' | 'from' | 'to' } | null {
    const ws = this.weddingsOn(d); if (!ws.length) return null;
    for (const w of ws) { if (pid === w.bride) return { w, role: 'bride' }; if (pid === w.groom) return { w, role: 'groom' }; }
    // (the Terrace slice's people keep their day on the Terrace: the detailed tier is not emptied by a kinsman's wedding; C)
    const h = this.home(pid, d); if (this.persons[pid].agent >= 0 || !ws.some(w => w.from === h || w.to === h) || !this.dayFree(pid, d)) return null;
    for (const w of ws) { if (h === w.to) return { w, role: 'to' }; if (h === w.from) return { w, role: 'from' }; }
    return null;
  }
  /** a house holding or sending a wedding today */
  weddingHouse(h: number, d: number) { return this.weddingsOn(d).some(w => w.from === h || w.to === h); }
  private wedPlanCache = new Map<number, WedPlan>();
  /** the wedding's hours (C): the bride dressed by the women of her house; the procession with the dowry through the lanes at
   *  a procession's pace; the feast in the groom's courtyard from its arrival; the supper; the women's drum at dusk; the bride's
   *  kin walk home after it */
  weddingPlan(w: { day: number; bride: number; from: number; to: number }): WedPlan {
    const c = this.wedPlanCache.get(w.bride); if (c && c.day === w.day) return c;
    const d = w.day, F = this.households[w.from], T = this.households[w.to], fW: Where = F.zone === 'plain' ? 'plain' : 'town', tW: Where = T.zone === 'plain' ? 'plain' : 'town';
    const hf = this.hday(w.from, d), ht = this.hday(w.to, d), u = (k: number) => u01(this.seed, S.marry, w.bride, 40 + k), C = this.cal.ctx(d);
    const walk0 = this.walkH(F.home, T.home, d, fW, tW), walk = Math.max(0.12, walk0 * 1.6);
    const proc = Math.max(hf.breakfast + hf.bLen + 1.2, ht.breakfast + ht.bLen + 1.2, Math.min(9.8 + 1.2 * u(1), ht.noon + 0.3 - walk)), arrive = proc + walk;
    const feast: [number, number] = [arrive + 0.25, arrive + 0.25 + 1.1 + 0.4 * u(2)];
    const s0 = Math.max(feast[1] + 1, ht.supper), supper: [number, number] = [s0, s0 + ht.sLen + 0.35];
    const drum: [number, number] = [supper[1], supper[1] + 0.6 + 0.5 * u(3)];
    const r = { day: d, dress: [proc - 0.7, proc] as [number, number], proc, arrive, walk, walk0, feast, supper, drum, leave: drum[1], fW, tW, fromHome: F.home, toHome: T.home, dark: drum[1] > C.sun.set + 0.3 };
    if (this.wedPlanCache.size > 200) this.wedPlanCache.clear(); this.wedPlanCache.set(w.bride, r); return r;
  }
  /** the festival day off (E-38; D-211, C): the jobs whose day it is (not the watch, the flocks, the Treasury's desk, the
   *  stores, the station, the magi, the palaces' doors, the officials) */
  static readonly FEST_OFF = new Set<Job>(['builder', 'camp', 'porter', 'weaver', 'miller', 'brewer', 'craftsman', 'gardener', 'farmer', 'homemaker', 'elder', 'child', 'servant', 'steward']);
  /** a festival day off for this person (E-38): a free day (dayFree), no wedding, no move, not the keeper of a bereaved house,
  ...
  festDay(pid: number, d: number) {
    ...
    if (p.job === 'child' && this.hday(H.id, d).minder === pid && this.membersOn(H.id, d).some(k => { const q = this.persons[k]; return q.job === 'child' && this.ageOn(k, d) <= 4 && q.mother >= 0 && q.mother !== pid && this.present(q.mother, d) && this.home(q.mother, d) === H.id && !this.festDay(q.mother, d); })) return false;
    return true;
  }
  private sipCache = new Map<number, { pid: number; leave: number; len: number; walk: number; back: number } | null>();
  /** the house's trip to the šip for its share of the meat (E-33; C): the eldest man of 16-65 whose day it is, else the eldest
   *  woman; from a house within an hour's walk of the offering place; in the morning's dry, clear hours, back home before the
   *  house's midday meal; null when no one goes (the share is then not fetched: not shown) */
  sipTrip(h: number, d: number) {
    const key = h * 512 + d; if (this.sipCache.has(key)) return this.sipCache.get(key)!; if (this.sipCache.size > 60000) this.sipCache.clear();
    const C = this.cal.ctx(d), F = C.festival, H = this.households[h]; let out: { pid: number; leave: number; len: number; walk: number; back: number } | null = null;
    if (F && (H.zone === 'town' || H.zone === 'plain' || H.zone === 'terrace')) {
      const W: Where = H.zone === 'plain' ? 'plain' : 'town', walk = this.walkH(H.home, 'offering_place', d, W, 'town');
      const mem = this.membersOn(h, d).filter(x => this.ageOn(x, d) >= 16 && this.ageOn(x, d) <= 65 && this.festDay(x, d)).sort((a, b) => this.ageOn(b, d) - this.ageOn(a, d) || a - b);
      const who = mem.find(x => this.persons[x].sex === 'm') ?? mem.find(x => this.persons[x].sex === 'f');
      if (who !== undefined && walk <= 1) { const hd = this.hday(h, d), u = (k: number) => u01(this.seed, S.fam, 7100 + h, d, k), len = 0.45 + 0.5 * u(1);
        const a = Math.max(hd.breakfast + hd.bLen + 0.3, F.hours[0] - walk), b = Math.min(F.hours[1] - len - walk, hd.noon - 0.25 - len - 2 * walk);
        const ok = (t: number) => wetHours(C.wx, t, t + len + 2 * walk) === 0 && !(C.wx.dustH && C.wx.dustH[0] < t + len + walk && C.wx.dustH[1] > t + walk);
        if (b > a) { let t = a + (b - a) * u(2); for (let k = 0; k < 16 && !ok(t); k++) t = a + ((t - a + 0.25) % (b - a + 1e-9)); if (ok(t)) out = { pid: who, leave: t, len, walk, back: t + 2 * walk + len }; } } }
    this.sipCache.set(key, out); return out;
  }
  /** the official who presides over the šip (E-33; unnamed, C): the first official of the year's roll who is here and well */
  sipPresider(d: number) { const C = this.cal.ctx(d); if (!C.festival) return -1; for (let i = 0; i < this.persons.length; i++) { const p = this.persons[i]; if (p.job === 'official' && this.present(i, d) && !this.sick(i, d) && !this.mourning(i, d)) return i; } return -1; }
  /** a dispute of yesterday brought before an official at the official building (E-74's rare escalation; Babylonian judges at the
   *  gate as the analogy, B for Babylonia; C here): the two, each with a witness of their own, on the next morning; settled by a
   *  pledge or a share of the ration (D-211). A share of the disputes (lives.json hearing.p), when both can come (not a
   *  festival, a wedding or a move) */
  hearing(pid: number, d: number): { other: number; t: number; why: string } | null {
    if (d < 1) return null; const x = this.cal.ctx(d - 1).disputes.get(pid); if (!x) return null; const C = this.cal.ctx(d); if (C.festival) return null;
    const a = Math.min(pid, x.other); if (u01(this.seed, S.disp, a, d - 1, 77) >= L.hearing.p) return null;
    for (const y of [pid, x.other]) { const q = this.persons[y], H = this.households[this.home(y, d)]; if (!this.dayFree(y, d) || this.ageOn(y, d) < 14 || (H.zone !== 'town' && H.zone !== 'plain') || this.walkH(H.home, 'official_bldg', d, H.zone === 'plain' ? 'plain' : 'town', 'town') > 1
      || this.weddingOf(y, d) || d === q.marry || this.keeperOn(H.id, d) === y || this.draftedOn(y, d, C)) return null; }
    return { other: x.other, t: 8.6 + 1.2 * u01(this.seed, S.disp, a, d - 1, 78), why: x.why };
  }
  /** the healer of a quarter (D-211; the Babylonian asû as an analogy, B for Babylonia; C here): a woman of 40-65 of the
   *  quarter, a homemaker or an old woman, drawn once for the year */
  private healerOf = new Map<string, number>();
  healer(q: string): number {
    const c = this.healerOf.get(q); if (c !== undefined) return c; let best = -1, bu = 2;
    for (const x of this.quarters[q]?.women ?? []) { const p = this.persons[x]; if (p.age < 40 || p.age > 65 || (p.job !== 'homemaker' && p.job !== 'elder') || p.agent >= 0 || p.dies < REGNAL_DAYS || p.marry < 1e9) continue; const u = u01(this.seed, S.fam, 9700 + x); if (u < bu) { bu = u; best = x; } }
    this.healerOf.set(q, best); return best;
  }
  private roundCache = new Map<number, { house: number; sick: number }[]>();
  /** the healer's calls today (D-211; C): up to lives.json healer.calls_max houses of her quarter within half an hour's walk
   *  of her own, where someone lies ill on the second day or later of an illness of three days or more */
  healerRound(pid: number, d: number): { house: number; sick: number }[] {
    const key = pid * 512 + d; const c = this.roundCache.get(key); if (c) return c; if (this.roundCache.size > 20000) this.roundCache.clear(); const out: { house: number; sick: number }[] = [];
    const H0 = this.households[this.home(pid, d)], q = H0.q, W: Where = H0.zone === 'plain' ? 'plain' : 'town';
    if (this.healer(q) === pid) { const seen = new Set<number>([H0.id]);
      for (const x of this.quarters[q]?.women ?? []) { if (out.length >= L.healer.calls_max) break; if (!this.present(x, d)) continue; const h = this.home(x, d); if (seen.has(h)) continue; seen.add(h); const H = this.households[h]; if (H.zone !== H0.zone || this.walkH(H0.home, H.home, d, W, W) > 0.5) continue;
        const ill = this.membersOn(h, d).find(y => { if (!this.present(y, d) || !this.sick(y, d) || this.persons[y].job === 'guard' || this.home(y, d) !== h) return false; const { k, n } = this.sickDayOf(y, d); return k >= 1 && n >= 3; });
        if (ill !== undefined) out.push({ house: h, sick: ill }); } }
    this.roundCache.set(key, out); return out;
  }
  /** the jars of water a person of the house draws today (A S7 of shadow review r9: 1.5 times the need, and the boys drawing
   *  while the women were at home): the house's waterer draws its need, less the one jar a girl of 8-13 fetches as her chore
   *  on some days (hday.helper); nobody else draws for the house (C) */
   ...
   *  porters' day is planned round it (Planner.terracePorter; A S4 of shadow review r8: the afternoon's wait for a caravan
   *  that had come in the morning) */
  caravan(d: number): { h: number; sacks: number } | null {
    const C = this.cal.ctx(d), st = C.wx.stormH, rn = C.wx.rain; const inS = (h: number) => !!st && h >= st[0] && h <= st[1], inR = (h: number) => !!rn && h >= rn[0] && h <= rn[1];
    let h = 9; for (let k = 0; k < 4 && (inS(h) || inR(h)); k++) { if (h >= 15) return null; h = Math.max(inS(h) ? st![1] : h, inR(h) ? rn![1] : h) + 1 / 60; }
    if (inS(h) || inR(h)) return null;
    const r = new Rng(this.seed, `caravan:${d}`); const n = r.int(1, 2); let sacks = 0; for (let i = 0; i < n; i++) sacks += r.int(20, 40);
    return { h, sacks };
  }
  ...
  depotHours(pid: number, d: number): [number, number] {
    const C = this.cal.ctx(d), w0 = this.workWindow(C)[0], c = this.caravan(d), done = this.caravanDone(d), g = this.persons[pid].group, iss = g >= 0 ? C.issue.get(g) : undefined;
    if (!c || done === null) { const a = workSpan(C.wx, w0, this.workWindow(C)[1]).a; return [a, a + 1.5]; } // (from when the weather lets him go up)
    return [Math.max(w0, Math.min(c.h - DEPOT_EARLY_H, iss !== undefined ? iss - 0.3 : 99)), Math.max(done, iss !== undefined ? iss + 0.8 : 0)];
  }
  ...
  builderAvailable(pid: number, d: number, C: DayCtx) {
    const p = this.persons[pid]; if (!this.present(pid, d) || this.sick(pid, d) || this.mourning(pid, d)) return false;
    if (C.winter && (p.squad + d) % 2 === 1) return false;
    return !this.rainedOff(C);
  }
  /** the working windows a Terrace worker's day is judged on for the weather (workSpan; A S3, B S2 of shadow review r8): the
   *  gangs', the camp's (its bakers from before sunrise), under a roof or in the open (a caretaker's day may be either), or
   *  [] for other jobs */
  terraceWindows(pid: number, d: number): { w: [number, number]; roofed: boolean }[] {
    const p = this.persons[pid], C = this.cal.ctx(d);
    if (p.job === 'camp') return [{ w: [p.sub === 'baker' ? C.sun.rise - 0.6 : C.sun.rise + 0.4, C.heatRest ? 12 : 14.5], roofed: false }];
    if (p.work === 'treasury_inside' || p.work === 'treasury_store') return [{ w: this.workWindow(C, true), roofed: true }];
    if (p.job === 'caretaker') return [{ w: this.workWindow(C, true), roofed: true }, { w: this.workWindow(C), roofed: false }];
    if (p.job === 'builder' || (p.job === 'porter' && p.sub === 'terrace')) return [{ w: this.workWindow(C), roofed: false }];
    return [];
  }
  /** the gangs' day given up to the weather: fewer than DRY_WORK_H dry hours in the working window (workSpan; A S3, B S2 of
   *  shadow review r8: was any storm in the day's light, or more than 5 rain hours in the whole day) */
  rainedOff(C: DayCtx) { const w = this.workWindow(C); return workSpan(C.wx, w[0], w[1]).dry < DRY_WORK_H; }
  /** man-days of stonecutting, hauling and brickwork put in today */
  buildCredit(d: number, C: DayCtx) {
    const cr = { stone: 0, labour: 0, brick: 0 }; const w = this.workWindow(C), sp = workSpan(C.wx, w[0], w[1]); let hours = sp.dry - (C.heatRest ? 0 : 0.55); // (the dry part of the window: workSpan)
  ...
  workWindow(C: DayCtx, roofed = false): [number, number] { return [C.sun.rise + L.work_day.start_after_sunrise_h, C.heatRest && !roofed ? L.work_day.heat_end_h : L.work_day.end_h]; }
   ...
   *  meal is the person's own and the talk is the spindle (a woman), play (a child) or rest (a man); "kept for the
   *  late-comer" only after a meal the household began a quarter of an hour or more before; a child "near the mother while
   *  she talks" only while she does (C) */
  private relabel(pid: number, d: number, raw: Seg[]): Seg[] {
    if (!raw.some(s => /with the household|kept for the late-comer|while the mother talks|the others of the house gone out/.test(s.why))) return raw;
    const mem = this.membersOn(this.home(pid, d), d).filter(x => x !== pid && this.present(x, d)), age = this.ageOn(pid, d), f = this.persons[pid].sex === 'f';
    const at = (s: Seg, h: number) => mem.some(x => segAt(this.rawPlan(x, d), h).place === s.place);
    // (only little ones of four or under there: a woman "with the household" has the spindle in her hands while they play
    // round her, a man is at home with them; B S5 of shadow review r8: a homemaker's "talk with the household" was her
    // daughter of four at play)
    const older = mem.filter(x => this.ageOn(x, d) >= 5), atOlder = (s: Seg, h: number) => older.some(x => segAt(this.rawPlan(x, d), h).place === s.place);
    let out: Seg[] | null = null; const put = (i: number, v: Seg) => { out ??= raw.map(x => ({ ...x })); out[i] = v; };
    for (let i = 0; i < raw.length; i++) { const s = raw[i]; if (s.t1 - s.t0 < 0.1 || s.where === 'road') continue;
      // (a child's breakfast "alone" while another of the house eats there then is the household's breakfast; A S4 of shadow
      // review r9: wrong in 67 % of such breakfasts, the guess made from the members' jobs; D-211)
      if (/the others of the house gone out/.test(s.why)) { const m = (s.t0 + s.t1) / 2;
        if (older.some(x => this.rawPlan(x, d).some(o => o.act === 'eat' && o.place === s.place && o.t0 <= m && o.t1 >= m))) { put(i, { ...s, why: 'breakfast with the household' }); continue; }
        if (atOlder(s, m)) { put(i, { ...s, why: 'breakfast, the others of the house busy about it' }); continue; } }
      if (/with the household/.test(s.why)) { const e = Math.min(0.05, (s.t1 - s.t0) / 4);
        if (atOlder(s, (s.t0 + s.t1) / 2) || (atOlder(s, s.t0 + e) && atOlder(s, s.t1 - e))) continue;
        if (s.act === 'talk' && age >= 14 && s.why === 'with the household' && (at(s, (s.t0 + s.t1) / 2) || (at(s, s.t0 + e) && at(s, s.t1 - e)))) { const sun = this.cal.ctx(d).sun; put(i, f && s.t0 > sun.rise && s.t1 < sun.set ? { ...s, act: 'spin', why: 'spinning, the little ones beside her' } : { ...s, act: 'rest', why: 'at home with the little ones' }); continue; }
        if (at(s, (s.t0 + s.t1) / 2) || (at(s, s.t0 + e) && at(s, s.t1 - e))) continue;
        let why = s.why.replace(/ with the household and /, ' with ').replace(/(^|,? )with the household/, '').trim(), act = s.act;
        if (act === 'talk' && (why === '' || /^sitting up$/.test(why))) { if (why === 'sitting up') { act = 'rest'; why = 'sitting up at home'; } else if (age < 13) { act = 'play'; why = 'playing at home'; } else if (f) { act = 'spin'; why = 'spinning'; }
          else { const sun = this.cal.ctx(d).sun, m = (s.t0 + s.t1) / 2, job = this.persons[pid].job; // (a man alone: the day's mending, not more rest; in the dark, at rest)
            if (s.t0 < sun.rise - 0.45 || s.t0 > sun.set + 0.3 || m > sun.set + 0.3 || ['official', 'steward', 'priest', 'scribe', 'storekeeper'].includes(job)) { act = 'rest'; why = 'at home'; } else { act = 'craft'; why = job === 'messenger' ? 'mending the harness and the saddle-bags' : 'mending tools and baskets'; } } }
        else if (why === '') why = 'at home';
...
class Planner {
  ...
  private homeHours(until: number, why = 'resting at home') {
    ...
    while (this.t < until - 0.3) {
      ...
      const outD = !this.nursing && !this.C.wx.wet && !this.C.wx.dust && !this.C.wx.storm && this.t > this.sun.rise && this.t < this.sun.set - 1.2 && until - this.t > 1 && (this.cur ?? this.home) === this.home && (this.homeW === 'town' || this.homeW === 'plain');
      const outM = menHouse && !this.nursing && !this.C.wx.wet && !this.C.wx.dust && !hot && this.t > 7 && this.t < this.sun.set - 1 && until - this.t > 0.8 && (this.cur ?? this.home) === this.home;
      const out = f && this.adult() && !this.nursing && this.homeW !== 'terrace' && !this.C.wx.wet && !this.C.wx.dust && !hot && this.t > 7 && this.t < this.sun.set - 1.5 && until - this.t > 1 && (this.cur ?? this.home) === this.home;
      const restLeft = farmMan || womanHome ? restCap - this.daylightRest() : Infinity;
      // (the doorstep: the day's light work taken out to the lane at the door in dry, clear daylight, not in the E-64 heat's
      // lull: D-211, gap audit item 9; its hours count as work, not rest)
      const doorOk = (this.hh.zone === 'town' || this.hh.zone === 'plain') && !this.nursing && !hot && !dark && this.t > this.sun.rise + 0.5 && this.t < this.sun.set - 1 && until - this.t > 0.7 && (this.cur ?? this.home) === this.home && this.adult() && wetHours(this.C.wx, this.t - 0.25, Math.min(until, this.t + s1) + 0.25) === 0 && !(this.C.wx.dustH && this.C.wx.dustH[0] < this.t + s1 + 0.2 && this.C.wx.dustH[1] > this.t - 0.2) && !this.C.wx.storm;
      const door = doorOk ? (f ? L.doorstep.women : ranked ? 0 : L.doorstep.men) : 0;
      const wts = f ? { door, rest: restLeft < 0.3 ? 0 : WF.rest, grind: WF.grind, weave: WF.weave, loom: dark ? 0 : WF.loom ?? 0, craft: 0, affairs: 0, animals: 0, talk: othersOut || restLeft < 0.3 ? 0 : WF.talk, sleep: hot, lane: out ? WF.lane * (this.P.membersOn(this.hh.id, this.d).some(x => this.ageOf(x) < 5 && this.P.persons[x].mother === this.pid) ? 2 : 1) : 0, water: 0, wash: 0, fuel: 0, chore: 0 }
        : { door, rest: restLeft < 0.3 ? 0 : H.men.rest, grind: 0, weave: 0, loom: 0, craft: ranked || dark ? 0 : H.men.craft, affairs: ranked && !dark ? H.men_of_standing.affairs : 0, animals: plain ? H.men.animals_plain : 0, talk: othersOut || restLeft < 0.3 ? 0 : H.men.talk, sleep: hot, lane: outM ? 0.25 : outD && !hot ? 0.1 : 0, water: (outM || (noneHome && outD)) && this.canDraw() ? 0.15 : 0, wash: outM && until - this.t > 1.3 ? 0.08 : 0, fuel: cold && outD && until - this.t > 1.4 ? 0.3 : 0, chore: chore && !hot && !dark && !(OPEN_WHY.test(chore[1]) && wetHours(this.C.wx, this.t - 0.25, this.t + 1.5) > 0) ? H.farm_chore : 0 };
      ...
      lastK = k;
      let t1 = Math.min(until, this.t + r.range(s0, s1)); if ((k === 'rest' || k === 'talk') && (farmMan || womanHome)) t1 = Math.min(t1, this.t + Math.max(0.3, restLeft));
      ...
      if (k === 'door') { const l = `lane:${this.hh.q}`, W = this.homeW, e = Math.max(this.t + 0.4, t1 - this.P.walkH(l, this.home, this.d, W, W)); this.go(l, W, 'out to the doorstep');
        const DW: [ActivityId, string][] = f ? [['spin', 'spinning on the doorstep, a word with the neighbours'], ['craft', 'picking over the grain on a tray on the doorstep'], ['craft', 'mending clothes on the doorstep']] : farmMan || restLeft < 0.3 ? [['craft', 'mending tools and baskets on the doorstep']] : [['craft', 'mending tools and baskets on the doorstep'], ['rest', 'sitting out on the doorstep with the neighbours']];
        const [a, w] = DW[Math.floor(r.next() * DW.length)]; this.add(e, l, a, w, W); this.go(this.home, W); continue; }
      ...
      if (k === 'craft') this.atHome(t1, 'craft', this.p.job === 'messenger' ? 'mending the harness and the saddle-bags' : 'mending tools and baskets'); else if (k === 'animals') this.atHome(t1, 'tend_animals', 'seeing to the household’s animals');
      ...
      else if (k === 'chore') { // (a job out of doors, on the roof, stops where the rain begins: S1 of shadow review r7, 42388 plastering in a storm)
        if (OPEN_WHY.test(chore![1])) for (let q = Math.ceil(this.t * 4); q * 0.25 < t1; q++) if (wetHours(this.C.wx, q * 0.25, q * 0.25 + 0.25) > 0) { t1 = Math.max(this.t + 0.1, q * 0.25); break; }
        this.atHome(t1, chore![0], chore![1]); }
  ...
  /** hours of rest at a place (not on the road) between sunrise and sunset so far today, and of talk with the household at
   *  home (B S5 of shadow review r8: the talk stood in for the capped rest, a homemaker with 6.8 h of rest, talk and play); on a
   *  day of the E-64 heat the rest from noon to 16:00 is the heat's, not counted (C) */
  private daylightRest() { const { rise, set } = this.sun, heat = this.C.heatRest; let s = 0;
    for (const x of this.segs) if ((x.act === 'rest' || (x.act === 'talk' && x.place === this.home && /^with the household$/.test(x.why))) && x.where !== 'road') { const a = Math.max(x.t0, rise), b = Math.min(x.t1, set); if (b <= a) continue; s += b - a; if (heat) s -= Math.max(0, Math.min(b, 16) - Math.max(a, 12)); } return s; }
  ...
  /** keeping a sick little one of the house at home today (Population.sickKeepers): no outings, no lane, no well; the
   *  little one lies ill beside the keeper (A S2, B S3 of shadow review r8) */
  private get nursing() { return this._nursing ??= this.P.keepsSick(this.pid, this.d); }
  private _nursing: boolean | undefined;
  ...
  private go(to: string, whereTo: Where, why = 'walking', act: ActivityId = 'walk') {
    ...
    if (from === this.home && L1 && L0 && L1.where === 'road' && (sliver || L1.t1 >= this.t - 1e-6) && L0.place === to && L0.where !== 'road' && L0.act !== 'sleep') {
      ...
      const water = /water/.test(L1.why); this.add(this.t + 0.05, this.home, water ? 'carry_jar' : 'rest', water ? 'pouring the water into the house jar' : this.home === 'station' ? 'setting the rations down at the station lodging' : 'setting the load down at home', this.homeW); }
    ...
    if (La && this.segs.length > 1 && La.where === 'road' && La.t1 >= this.t - 1e-6 && wf !== 'terrace' && !(whereTo === 'terrace' && this.p.job === 'guard') && !from.startsWith('road:')) {
      ...
      // (a traveller's stop at the station between two errands: the load set down at the lodging, not a moment "at home": A S9 of
      // shadow review r9, 175 stops of under 6 min)
      if (this.home === 'station' && atHome) this.add(this.t + 0.12, from, 'rest', loadIn ? 'setting the rations down at the station lodging' : 'at the station lodging', wf);
      else this.add(this.t + 0.05, from, a, w, wf); }
  ...
  /** the end of the light for work or games out of doors: civil dusk, about 0.45 h after sunset at 30° N (planCheck (b)) */
  private lightEnd() { return this.sun.set + 0.4; }
  /** the start of the light: civil dawn */
  private lightStart() { return this.sun.rise - 0.4; }
  ...
  build(): Seg[] {
    ...
    if (!P.present(this.pid, this.d)) return segs;
    if (this.p.job === 'traveller') { this.siesta(segs); this.words(segs); this.dustVeil(segs); this.coldWear(segs); return segs; }
    // a band's people (S4): their mothers nurse on demand like any; their days need no water, fire or field passes
    if (this.p.job === 'herder') { this.siesta(segs); if (this.p.sex === 'f' && this.age >= 14) this.nurse(segs); if (this.age >= 2) this.meals(segs); if (this.p.sex === 'f' && this.age >= 14) { this.feedGaps(segs); this.littleFeeds(segs); } this.joinSlivers(segs); this.tidy(segs); this.words(segs); this.carries(segs); this.dustVeil(segs); this.coldWear(segs); return segs; }
    this.siesta(segs); // (first: the meals' safety net and the feeds may then fall in the hours past it)
  ...
  /** the sleep in the heat of the day, at most HEAT_SLEEP_CAP_H (B S6 of shadow review r8: 12:19-17:01 asleep after keeping a
   *  door, 0.9 % of adult days with 12.5 h of sleep or more; C): from fourteen, what is past it of a day's sleeps in the heat
   *  (at home, in the shade at the field or the floor) is rest in the shade, or for a woman at home the spindle. A night's
   *  work slept off by day is not the heat's (after or before a night watch, night duty, the grain heap) */
  private siesta(segs: Seg[]) {
    if (this.age < 14) return; let left = HEAT_SLEEP_CAP_H;
    for (let i = 0; i < segs.length; i++) { const s = segs[i]; if (s.act !== 'sleep' || !HEAT_SLEEP.test(s.why) || s.t1 <= this.sun.rise + 2 || s.t0 >= this.sun.set - 1) continue;
      const len = s.t1 - s.t0; if (len <= left + 1e-9) { left -= len; continue; }
      const cut = s.t0 + left; left = 0; const home = s.place === this.home && s.where !== 'road' && this.p.job !== 'guard';
      const rest: Seg = { ...s, t0: cut, act: home && this.p.sex === 'f' ? 'spin' : 'rest', why: home ? (this.p.sex === 'f' ? 'spinning in the shade of the house through the heat' : 'resting in the shade of the house through the heat') : s.why.replace(/^sleeping/, 'resting') };
      if (cut - s.t0 < 0.05) { segs[i] = { ...rest, t0: s.t0 }; continue; } s.t1 = cut; segs.splice(i + 1, 0, rest); i++; }
  }
  ...
  private water(segs: Seg[]) {
    // (the jars that are this person's to draw today: the waterer's, or the one of a girl who helps; A S7 of shadow review r9)
    const H = this.hd, J = this.P.jarsOf(this.pid, this.d); if (!H || !J || dayStorm(this.C) || this.noWater) return;
    const have = segs.filter(s => s.act === 'draw_water' && s.with === undefined).length; if (have >= J) return;
    const w = `well:${this.hh.q}`, W = this.homeW, walk = this.P.walkH(this.home, w, this.d, W, W); const trip = 2 * walk + 0.22;
    const rain = this.C.wx.rain; let need = J - have;
    // (the keeper of a sick little one, when no one else can go, goes only in the warm middle of the day, the little one with
    // her; with no hour for it then, a neighbour's jar does for the day: not planned, C)
    const wins = (this.nursing ? [[11, 15]] : [[this.sun.rise - 0.2, 11], [15, this.sun.set - 0.1], [11, 15]]) as [number, number][];
    ...
    const okHome = (s: Seg) => s.place === this.home && s.with === undefined && s.where === W && !/^minding/.test(s.why) && (/^(rest|talk|play|spin|craft)$/.test(s.act) || (s.act === 'grind' && s.t1 - s.t0 >= trip + 0.5)) && !/ in the night|nurs|sick|ill|asleep/.test(s.why);
    ...
    for (const [lo, hi] of wins) {
      for (let i = 0; i < segs.length && need > 0; i++) { const s = segs[i];
        if (s.place !== this.home || s.with !== undefined || s.where !== W || /^minding/.test(s.why) || !(/^(rest|talk|play|spin|craft)$/.test(s.act) || (s.act === 'grind' && s.t1 - s.t0 >= trip + 0.5)) || / in the night|nurs|sick|ill|asleep/.test(s.why)) continue;
  ...
  private tidy(segs: Seg[]) {
    const EV = /\s*\(([^()]*\b(?:E|CE|W|Q|D|M)-\d+[^()]*|(?:HDT|PF|PT|POTTS|PW|CH|KONNER)[- ]?[\w.]*[^()]*)\)/;
  ...
  private carries(segs: Seg[]) {
    ...
    for (let i = 0; i < segs.length; i++) { const s = segs[i], w = s.why; if (s.carry || (s.with !== undefined && this.age < 5)) continue;
      ...
      else if (s.act === 'carry_bread') s.carry = /water/.test(w) ? 'bread in a basket and a water jar' : /tray/.test(w) ? 'a tray of her wares' : 'bread in a basket';
      else if (s.act === 'carry_sack') s.carry = /šip meat/.test(w) ? 'the household’s share of the meat in a basket' : /dowry: (.*?)( \(|$)/.test(w) ? /dowry: (.*?)( \(|$)/.exec(w)![1] : /carrying his things/.test(w) ? 'his bedding and his things in a bundle' : /sheaves/.test(w) ? 'sheaves' : /fuel|dung|brushwood/.test(w) ? 'dung cakes and brushwood' : /grain/.test(w) ? 'a sack of grain' : /ration/.test(w) ? 'the ration in a sack' : /sacks? of barley/.test(w) ? 'a sack of barley' : /barley/.test(w) ? 'a measure of barley' : /flour/.test(w) ? 'a sack of flour' : /straw/.test(w) ? 'a load of straw' : 'a load';
      else if (s.act === 'carry_jar') s.carry = /dowry: (.*?)( \(|$)/.test(w) ? /dowry: (.*?)( \(|$)/.exec(w)![1] : /oil/.test(w) ? 'a small jar of oil' : /water/.test(w) ? 'a water jar' : 'a jar';
      else if (s.act === 'draw_water') s.carry = 'a water jar';
      else if (p.job === 'guard' && (s.act === 'stand_guard' || s.act === 'patrol')) s.carry = arms + (s.act === 'stand_guard' && this.C.heatRest && s.t1 > 11 && s.t0 < 16 ? ', and a water jar at the post through the heat' /* E-64; S3 of reviewer A, r7; C */ : '');
  ...
  /** at the house but out of doors (the roof, the courtyard, the doorstep: OPEN_WHY) only while it is dry, and at leisure
   *  only out of the dust (W-01, W-02, W-03): in the rain or the storm the job or the play goes indoors, cut at the weather's
   *  quarter-hours (D-191, planCheck (a): children playing and minding in the courtyard through the rain, 42388's roof) */
  private shelterHome(segs: Seg[]) {
    const wx = this.C.wx; if (!wx.rain && !wx.stormH && !wx.dustH) return;
    const wetQ = (q: number) => !!wx.rainQ[Math.min(95, q)] || (!!wx.stormH && q * 0.25 + 0.25 > wx.stormH[0] && q * 0.25 < wx.stormH[1]);
    const dustQ = (q: number) => !!wx.dustH && q * 0.25 + 0.25 > wx.dustH[0] && q * 0.25 < wx.dustH[1];
    const indoor = (s: Seg, k: string): [ActivityId, string] => { const w = k === 'dust' ? 'out of the dust' : 'out of the rain'; let m: RegExpExecArray | null;
      if (/^playing in the courtyard|on the doorstep/.test(s.why)) return ['play', `playing indoors ${w}`];
      if ((m = /^minding (.+?) in the courtyard, playing with (.+)$/.exec(s.why))) return ['play', `minding ${m[1]} indoors ${w}, playing with ${m[2]}`];
      if (/roof/.test(s.why) && s.act !== 'clean') return ['craft', `mending tools and baskets indoors ${w}`];
      if (/animals out/.test(s.why)) return ['tend_animals', 'seeing to the household’s animals in the stall'];
      if (/sweeping the courtyard/.test(s.why)) return ['clean', 'sweeping the house'];
      return ['rest', `at home ${w}`]; };
    for (let i = 0; i < segs.length; i++) { const s = segs[i];
      // (the night by the grain heap: in the rain he sits up under his cloak, keeping watch, not asleep on the floor; C)
      if (s.act === 'sleep' && /by the grain heap/.test(s.why) && wetHours(wx, s.t0, s.t1) > 0) { const parts: Seg[] = []; let a = s.t0;
        for (const [ra, rb] of [...rainSpells(wx, 0.3), ...(wx.stormH ? [wx.stormH] : [])].sort((x, y) => x[0] - y[0])) { const x0 = Math.max(a, ra), x1 = Math.min(s.t1, rb); if (x1 <= x0) continue; if (x0 > a + 0.05) parts.push({ ...s, t0: a, t1: x0 }); parts.push({ ...s, t0: x0, t1: x1, act: 'rest', why: 'keeping watch over the grain heap through the rain, the cloak drawn over the head' }); a = x1; }
        if (a < s.t1 - 1e-6) parts.push({ ...s, t0: a, t1: s.t1 }); segs.splice(i, 1, ...parts); i += parts.length - 1; continue; }
      if (s.where === 'road' || s.place !== this.home || !OPEN_WHY.test(s.why)) continue;
      const leisure = /^(play|rest|sleep|talk|spin)$/.test(s.act), bad = (q: number) => (wetQ(q) ? 'rain' : leisure && dustQ(q) ? 'dust' : '');
      const runs: [number, number, string][] = []; let a = s.t0, k = bad(Math.floor(s.t0 * 4));
      for (let q = Math.floor(s.t0 * 4) + 1; q * 0.25 < s.t1 - 1e-9; q++) { const x = bad(q); if (x !== k) { runs.push([a, q * 0.25, k]); a = q * 0.25; k = x; } }
      runs.push([a, s.t1, k]); if (runs.length === 1 && !runs[0][2]) continue;
      for (let j = runs.length - 1; j > 0; j--) if (runs[j][1] - runs[j][0] < 0.05) { runs[j - 1][1] = runs[j][1]; runs.splice(j, 1); } // (no slivers)
      if (runs.length > 1 && runs[0][1] - runs[0][0] < 0.05) { runs[1][0] = runs[0][0]; runs.splice(0, 1); }
      const parts = runs.map(([x, y, kk]) => { if (!kk) return { ...s, t0: x, t1: y }; const [act, why] = indoor(s, kk); return { ...s, t0: x, t1: y, act, why }; });
      segs.splice(i, 1, ...parts); i += parts.length - 1; }
  }
  ...
  private dustVeil(segs: Seg[]) { dustWear(segs, this.C.wx); }
  private coldWear(segs: Seg[]) { coldWear(segs, this.C.wx, this.p.agent >= 0); }
  ...
  private insertAt(segs: Seg[], t: number, len: number, act: ActivityId, why: string, keep?: (s: Seg) => boolean, longBread = false): number {
    let i = segs.findIndex(s => t < s.t1); if (i < 0) return -1;
    // never into a meal, and never in place of drawing water or of the bread (S5): what is due then comes after it (a feed
    // may break a baking of an hour or more: longBread)
    while (i < segs.length && (segs[i].where === 'road' || (/^(draw_water|knead|bake)$/.test(segs[i].act) && !(longBread && segs[i].act !== 'draw_water' && segs[i].t1 - segs[i].t0 >= 1)) || (segs[i].t1 - t < 0.12 && i + 1 < segs.length && segs[i + 1].act !== 'eat'))) { t = segs[i].t1; i++; }
  ...
  private nurse(segs: Seg[]) {
    ...
    if (birthT >= 0) { for (let t = birthT + 0.2; t < 24; t += lerp(IC.day_feed_every_h[0], IC.day_feed_every_h[1], r.next())) times.push([t, false]);
      const last = times[times.length - 1]?.[0] ?? birthT, capN = nightGapMax(0); if (24 - last > 0.45 * capN && 23.9 - last > 0.4) times.push([Math.max(last + 0.5, 24 - 0.4 * capN), true]); } // (the night's gap across midnight: planCheck (e))
    else if (infant) {
      // a baby under one: by day a feed every day_feed_every_h; by night feeds no further apart than night_gap_max_h for its
      // age (planCheck (e); S4 of shadow review r7: 4 h 51 min unfed at night at 0 months), the first of the day in the
      // first half of that gap after midnight and the last in the last half of it before midnight, so that no gap across
      // midnight is longer either (C; the count of night feeds, night_feeds, comes out of the spacing)
      const capN = nightGapMax(Math.min(...kids.filter(c => this.ageOf(c) === 0).map(c => monthsOld(P, c, d)))), capX = Math.min(capN, IC.day_feed_every_h[1] + 0.4), step = () => capN * lerp(0.72, 0.9, r.next());
      const day: number[] = []; for (let t = wake + r.range(0.1, 0.6); t < bed - 0.3; t += lerp(IC.day_feed_every_h[0], IC.day_feed_every_h[1], r.next())) day.push(t);
      const d0 = day[0] ?? wake + 0.3, dl = day[day.length - 1] ?? bed - 0.3;
      let t = capN * lerp(0.15, 0.45, r.next()); if (t < d0 - 0.3) { times.push([t, true]); while (d0 - t > capX * 0.9) { t = Math.min(t + step(), d0 - 0.4); if (t <= times[times.length - 1][0] + 0.3) break; times.push([t, true]); } }
      t = dl; while (t < 24 - capN * 0.4) { t = Math.min(23.9, t + Math.min(step(), t === dl ? capX * 0.9 : capN)); times.push([t, true]); if (t >= 23.9) break; }
      for (const x of day) times.push([x, false]); }
      ...
      const n = r.int(IC.toddler_day_feeds[0], IC.toddler_day_feeds[1]); for (let k = 0; k < n; k++) times.push([lerp(wake + 0.3, bed - 0.5, (k + r.next()) / n), false]); }
    ...
    const what0 = babies.length === 2 && P.persons[babies[0]].twin === babies[1] ? 'nursing the twins' : fos ? (kids.length > 1 ? (kids.some(c => P.persons[c].nurse === this.pid && this.ageOf(c) === 0) ? 'nursing the motherless baby she wet-nurses and her little one' : 'nursing her baby and the motherless child she wet-nurses') : 'nursing the motherless child she wet-nurses')
    ...
    for (let fi = 0; fi < times.length; fi++) { let [t, night] = times[fi]; const len = lerp(IC.feed_h[0], IC.feed_h[1], r.next());
      let what = what0; if (babies.length && olds.length && !night && !todAt.has(fi)) what = babyOnly; // (a wet-nursed little one's too: 27246, 21 months, had six day feeds beside the nurse's baby)
      ...
      if (night || (s.act === 'sleep' && (t < wake || t > bed))) { const t2 = this.insertAt(segs, t, len, 'rest', `${what} in the night`); if (t2 >= 0) { const x = segAt(segs, t2 + 0.001); if (x.why === `${what} in the night`) x.why = this.nightWords(what, (x.t0 + x.t1) / 2); } continue; }
  ...
  /** a feed while the household sleeps: "in the night" in the dark, else before she gets up or before she sleeps (planCheck
   *  (d): "nursing the baby in the night" at 05:25, after sunrise) */
  private nightWords(what: string, t: number) { return t < this.sun.rise || t > this.sun.set ? `${what} in the night` : t < 12 ? `${what} at first light, before getting up` : `${what} before sleeping`; }
  /** after the other passes (the water, the fire, the bread), which can take a feed's place */
  private feedGaps(segs: Seg[]) {
    const P = this.P, d = this.d, IC = L.infant_care, kids = P.nurslings(this.pid, d); if (!kids.some(c => this.ageOf(c) === 0) || kids.some(c => P.persons[c].born === d) || segs.some(s => s.act === 'offmap')) return;
    ...
    // no gap between a baby's feeds longer than the planner's own rule allows: by day infant_care.day_feed_every_h's longest
    // and 0.4 h of slack (S4 of shadow review r6: 4 h 46 min unfed by day at one month), by night night_gap_max_h for its
    // age (S4 of shadow review r7), and across midnight (the day's first feed in the first 0.55 of a night gap, its last in
    // the last 0.45: nurse). A feed that fell on her meal, the water or the bread and found no place after it is given in
    // the gap, where she is (C)
    const capN = nightGapMax(Math.min(...babies.map(c => monthsOld(P, c, d)))), capD = IC.day_feed_every_h[1] + 0.4;
    for (let k = 0; k < 10; k++) {
      const fs = segs.filter(x => /nurs/.test(x.why)).map(x => [x.t0, x.t1] as [number, number]); const pts: [number, number][] = [[-0.45 * capN, -0.45 * capN], ...fs, [24 + 0.55 * capN, 24 + 0.55 * capN]];
      let ga = -1, gb = -1, over = 0; for (let i = 0; i + 1 < pts.length; i++) { const a = pts[i][1], b = pts[i + 1][0], m = (a + b) / 2, cap = m >= wake && m <= bed ? capD : capN; if (b - a - cap > over) { over = b - a - cap; ga = a; gb = b; } }
      if (ga < -1e8 || over <= 0) break; let done = false;
      const lo = Math.max(0.05, ga + 0.3), hi = Math.min(23.8, gb - 0.3), mid = Math.min(hi, Math.max(lo, (ga + gb) / 2));
      ...
      for (const t of tries) { if (t < lo || t > hi) continue; const x = segAt(segs, t);
        ...
        if (x.where === 'road' && this.p.job === 'herder' && x.act === 'walk' && x.t1 - t >= 0.3 && t - x.t0 >= 0.05) { const len = lerp(IC.feed_h[0], IC.feed_h[1], r.next()), i = segs.indexOf(x);
          segs.splice(i, 1, { ...x, t1: t }, { ...x, t0: t, t1: t + len, why: `${x.why}, nursing the baby at the breast as she walks` }, { ...x, t0: t + len }); done = true; break; }
        if (x.where === 'road' || ['draw_water', 'offmap'].includes(x.act) || (['knead', 'bake'].includes(x.act) && x.t1 - x.t0 < 1) || (x.act === 'eat' && (x.t1 - x.t0 < 0.6 || t - x.t0 < 0.2 || x.t1 - t < 0.35)) || x.t1 - t < 0.2) continue; // (a daytime sleep: woken to nurse, as in nurse(); at a long meal, during it)
        const night = t < wake || t > bed, work = !['talk', 'rest', 'queue', 'gamble', 'exchange', 'play', 'sleep'].includes(x.act), what = babyOnly;
        const why = x.act === 'lie_ill' ? `lying ill, ${what}` : night && x.act === 'sleep' ? this.nightWords(what, t + 0.1) : work ? `stopping to ${what.replace('nursing', 'nurse')}` : what;
        const t2 = this.insertAt(segs, t, lerp(IC.feed_h[0], IC.feed_h[1], r.next()), x.act === 'lie_ill' ? 'lie_ill' : 'rest', why, undefined, true); if (t2 >= 0) { const y = segAt(segs, t2 + 0.001); if (/ in the night$| at first light| before sleeping$/.test(y.why)) y.why = this.nightWords(what, (y.t0 + y.t1) / 2); done = true; break; } }
      if (!done) break; }
  }
  /** a little one of one or two nursed beside a baby has its day feeds (infant_care.toddler_day_feeds, at least the fewest):
   *  when the baby's feeds, now spaced by night too, and the other passes left fewer of the day's feeds shared with it, some
   *  of the baby's day feeds at home are shared (the test of D-083: a wet-nursed child of one nursed once in a day) */
  private littleFeeds(segs: Seg[]) {
    const P = this.P, d = this.d, IC = L.infant_care, kids = P.nurslings(this.pid, d), babies = kids.filter(c => this.ageOf(c) === 0), olds = kids.filter(c => this.ageOf(c) > 0);
    if (!babies.length || !olds.length || kids.some(c => P.persons[c].born === d)) return;
    const fos = kids.some(c => P.persons[c].nurse === this.pid), both = fos ? (kids.some(c => P.persons[c].nurse === this.pid && this.ageOf(c) === 0) ? 'nursing the motherless baby she wet-nurses and her little one' : 'nursing her baby and the motherless child she wet-nurses') : 'nursing the baby and the little one';
    const wake = segs.find(s => s.t0 > 2 && s.act !== 'sleep')?.t0 ?? 6, bed = [...segs].reverse().find(s => s.act !== 'sleep' && s.t1 < 24)?.t1 ?? 21;
    const day = (s: Seg) => s.t0 >= wake && s.t1 <= bed && !NIGHT_FEED.test(s.why), shared = (s: Seg) => s.why.includes(both.replace(/^nursing /, '')), home = (s: Seg) => s.place === this.home;
    let n = segs.filter(s => day(s) && /nurs/.test(s.why) && shared(s)).length; if (n >= IC.toddler_day_feeds[0]) return;
    const cand = segs.filter(s => day(s) && home(s) && /^(nursing|stopping to nurse) (the baby|the twins)$/.test(s.why)); const step = Math.max(1, Math.floor(cand.length / (IC.toddler_day_feeds[0] - n + 1)));
    for (let k = step - 1; k < cand.length && n < IC.toddler_day_feeds[0]; k += step) { const s = cand[k]; s.why = /^stopping/.test(s.why) ? `stopping to ${both.replace('nursing', 'nurse')}` : both; n++; }
  }
  ...
  private meals(segs: Seg[]) {
    ...
    const awake = segs.filter(s => s.act !== 'sleep' && s.act !== 'offmap' && s.act !== 'lie_ill' && !NIGHT_FEED.test(s.why)); if (!awake.length) return;
    ...
    for (let k = 0; k < 4; k++) {
      ...
      const cand = (hearth: boolean) => { let best = -1, bd = 1e9;
        for (const s of segs) { if (s.where === 'road' || ['sleep', 'stand_guard', 'patrol', 'eat', 'lie_ill', 'offmap', 'draw_water', 'knead', 'bake'].includes(s.act) || (hearth && !HEARTHS.includes(s.place)) || (outdoors(s) && !WET_OK_FLOCK.test(s.place) && !/out of the rain/.test(s.why) && wetHours(this.C.wx, s.t0, s.t1) > 0)) continue;
      ...
      if (t < 0 && this.p.job === 'guard' && clearOf(at) && segs.some(s => s.act === 'stand_guard' && s.t0 <= at && s.t1 >= at + 0.3))
        t = this.insertAt(segs, at, 0.25, 'eat', 'bread and water at the post, carried up from the hearth', s => s.act !== 'stand_guard');
      if (t < 0) return;
    }
  }
  private build0(): Seg[] {
    ...
    // (a baby that died in its first weeks: its mother's days after the birth are a bereaved rest, not "with the baby": A S2 of
    // shadow review r9, 705 mother-days speaking of a dead baby; D-211)
    if (born > 0 && born <= P.postpartumDays(this.pid)) return P.babyLives(this.pid, d) ? this.postpartum() : this.bereavedRest();
    ...
    if (P.carerToday(this.pid, d)) return this.homeDay(this.nursing ? 'keeping the sick little one at home' : 'tending the sick at home', true);
    const wed = P.weddingOf(this.pid, d); if (wed) return this.weddingDay(wed.w, wed.role); // (E-73, D-211)
    ...
    if (P.festDay(this.pid, d)) return this.festivalDay(); // (E-33, E-38, D-211)
    { const hr = P.hearing(this.pid, d); if (hr) return this.hearingDay(hr); } // (E-74, D-211)
    switch (p.job) {
  ...
  private morning(until: number, stay = false) {
    ...
    const fetch = this.P.jarsOf(this.pid, this.d) > 0 && this.pid === H.waterer && (!woman || !withHouse) && this.homeW !== 'terrace' && !this.C.wx.wet && this.hh.zone !== 'terrace' && !this.nursing;
    ...
    // the well is not gone to in the dark: a woman up before first light (about 0.45 h before sunrise at 30° N) works at the
    // quern first and draws the water once it is light (S1 minor of shadow review r5: #123's water drawn 77 min before
    // sunrise; C)
    // (a waterer who must leave before first light does not go then: the water pass draws the house's jars in their hours
    // at home, the evening water; D-137's jar before work only when it is light. Measured before: 1,005 of 19,336 morning
    // draws, every third person on five days, began before first light, builders and camp women among them)
    const firstLight = this.sun.rise - 0.45, fetchNow = fetch && this.t >= firstLight - 1e-9, wellLater = fetch && !fetchNow && woman;
    if (fetchNow) this.well(this.t + 0.2, withHouse ? 'fetching the day’s water' : 'fetching the day’s water before work');
    if (woman) {
  ...
  /** this person may draw a jar of the house's water now: one of the jars that are theirs today (Population.jarsOf) is still
   *  to be drawn (A S7 of shadow review r9: the chores, the evening water and the men's trips came on top of the house's need) */
  private canDraw() { return !this.noWater && this.segs.filter(s => s.act === 'draw_water' && s.with === undefined && s.place.startsWith('well:')).length < this.P.jarsOf(this.pid, this.d); }
  /** no water drawn by this person today (a wedding day's feast house: the neighbours bring it; not shown, C) */
  private noWater = false;
  ...
  /** the ration queue on the group's issue day; the grain is carried home (E-01; CE-01). The person goes at the issue hour,
   *  not at first light (S2 of shadow review r7: 1.06 h on average waiting in the town before the issue opened): the
   *  morning until then is at home; the queue takes RATION_Q_H (C) */
  private rationRun(): number | null {
    ...
    if (this.t < h - 0.02) this.add(h, G.issuePlace, 'queue', 'waiting for the ration issue', wq);
    this.dispute(G.issuePlace);
    this.add(this.t + lerp(RATION_Q_H[0], RATION_Q_H[1], this.r.next()), G.issuePlace, 'queue', `in the queue for ${this.C.issue.has(g) ? 'the monthly ration' : 'a special ration'}`, wq);
    return h;
  }
  /** a Terrace gang member's ration on the issue day: the queue at the depot at the issue hour, cut into the day's work by
   *  workBlock (after any rain there; S2 of shadow review r7: 9,083 person-days a year idle at the stair foot for 1.98 h on
   *  average before the issue opened) */
  private rationSlot: [number, number] | null = null; private rationQueued = false;
  private terraceRation(issue: number | undefined) { if (issue === undefined) return; this.dispute('stair_foot', 'terrace'); this.rationSlot = [issue, issue + lerp(RATION_Q_H[0], RATION_Q_H[1], this.r.next())]; }
  ...
  private evening(from: number) {
    ...
    const rain = this.nursing || this.rainIn(this.t, bed) > 0.3 || (!!C.wx.dustH && C.wx.dustH[1] > this.t + 0.2); // (and not in the dust, W-03: S5 r5; and not away from a sick little one)
    ...
    // light has gone and a little after, M-22; D-211, C)
      ...
      const l = `lane:${this.hh.q}`; this.go(l, this.homeW, 'out to the lane with the drum'); this.add(Math.max(this.t + 0.3, Math.min(bed - 0.3, this.t + r.range(0.6, 1.2))), l, 'talk', 'singing and clapping with the women of the lane to the frame drum, a festival evening (M-22)', this.homeW); this.go(this.home, this.homeW); }
    else if (!rain) {
      const u = r.next();
      ...
      if (this.age < 14) { const dusk = this.sun.set + 0.15, teenPlayed = this.age >= 13 ? this.segs.reduce((a, s) => a + (s.act === 'play' && s.where !== 'road' && !/^minding/.test(s.why) ? s.t1 - s.t0 : 0), 0) : 0; if (u < E.play_children && this.age >= 5 && C.season !== 'winter' && C.wx.tmin >= 6 && dusk - this.t > 0.4 && teenPlayed < L.children.work.teen_play_cap_h - 0.4) { const l = `lane:${this.hh.q}`; this.go(l, this.homeW); this.add(Math.max(this.t + 0.2, Math.min(bed - 0.2, dusk - 0.05, this.t + r.range(0.4, 1.2), this.age >= 13 ? this.t + L.children.work.teen_play_cap_h - teenPlayed : 24)), l, 'play', 'playing in the lane until dusk', this.homeW); this.go(this.home, this.homeW, 'called in at dusk'); } }
        ...
        // "after dark" began before the light had gone in 42 % of such talks, and before sunset in 15 %)
        const t0 = this.t; this.add(Math.min(bed - 0.3, this.t + r.range(0.6, 1.5)), l, lit ? 'gamble' : 'talk', lit ? 'knucklebones with neighbours' : t0 < this.lightEnd() ? 'talking with neighbours in the lane at dusk' : 'talking with neighbours in the lane after dark', this.homeW);
        if (this.t > this.lightEnd() + 0.02 && t0 < this.lightEnd()) { const g = this.segs[this.segs.length - 1], e = g.t1; g.t1 = this.lightEnd(); this.t = g.t1; this.add(e, l, 'talk', 'talking with neighbours in the lane after dark', this.homeW); }
  ...
  /** whom to visit: ties weighted by affinity (relationships change who people see). `name` and `house` say the same of a
   *  house every time: a kin house is kin's, whoever of it the tie is (B S8 of shadow review r8: h:1320 was "a kinswoman",
   *  "a friend's house" and "a neighbour's house" within three hours) */
  private visitTarget(): { place: string; where: Where; name: string; house: string } | null {
    ...
    if (!c.length) return null; let u = this.r.next() * tot; for (const [o, w] of c) { u -= w; if (u <= 0) { const hh = P.households[P.home(o, d)], nm = nameFor(P.seed, P.persons[o]), kin = this.hh.kin.includes(hh.id);
      return { place: hh.home, where: hh.zone === 'plain' ? 'plain' : 'town', name: nm ?? (kin ? 'kin' : 'a neighbour'), house: nm ? `${nm}’s house` : kin ? 'the house of kin' : 'a neighbour’s house' }; } }
    return null;
  }
  ...
  private workBlock(place: string, where: Where, act: ActivityId, why: string, t0: number, t1: number, outdoor = true, lunchPlace = place, noon = 12, mealWhy?: string,
    ...
    const C = this.C, wetSp = wetSpells(C.wx); this.dispute(place, where); if (this.t < t0) this.add(t0, place, act, why, where);
    ...
    // (the midday meal is not eaten out in the rain: S10 of reviewer B, r7, 821 meals at the field overlapping the rain on day
    // 354 alone. It ends where the rain begins; with less than a quarter of an hour of it left before the rain it is not
    // eaten out there (the meals pass gives the food at home); on the Terrace it is eaten under the Gate's roof)
    for (const sl of slots) { if (sl[2] !== 'eat' || !(outdoor || OPEN_PLACE.test(sl[4]))) continue; const w = wetSp.find(([ra, rb]) => ra < sl[1] && rb > sl[0]); if (!w) continue;
      if (where === 'terrace') { sl[4] = 'gate_hall'; sl[3] = 'the midday meal under the Gate’s roof, out of the rain'; } else if (w[0] - sl[0] >= 0.25) sl[1] = w[0]; else sl[1] = sl[0]; }
    slots.sort((a, b) => a[0] - b[0]);
    for (const [a, b, sa, sw, sp, home] of slots) { if (b <= this.t || b - a < 1e-6) continue;
  ...
  private dayWork(place: string, where: Where, act: ActivityId, why: string, t0: number, t1: number, outdoor = false, mealWhy?: string): Seg[] {
    if (dayStorm(this.C) && outdoor) return this.homeDay(`${stormWord(this.C.wx, this.sun.rise - 0.5, this.sun.set + 0.5)}: no work`);
    ...
    this.workBlock(place, where, act, why, Math.max(t0, this.t), t1, outdoor, place, 12, mealWhy);
  ...
  private homeDay(why: string, stayIn = false): Seg[] {
    ...
    if (!this.nursing) this.rationRun(); this.go(this.home, this.homeW); // (the keeper of a sick little one does not queue: the group's head draws for the group, E-01)
    ...
    this.noonAtHome(); if (this.adult() && (!stayIn || this.nursing)) this.homeHours(15 + r.range(0, 1), stayIn ? why : undefined); else this.atHome(Math.max(this.t, 15 + r.range(0, 1)), 'rest', stayIn ? why : 'resting at home');
  ...
  private sickDay(): Seg[] {
    ...
    if (this.p.job === 'guard' && k >= 1 && this.P.guardNursedHome(this.pid, this.d)) return this.guardSickHome(k, n);
    ...
    const y = k === 0 && this.d > 0 && this.P.present(this.pid, this.d - 1) ? this.P.rawPlan(this.pid, this.d - 1) : null, ly = y ? y[y.length - 1] : null;
  ...
  /** a sick guard nursed by his family in the town (Population.guardNursedHome; lives.json guard_sick_home, C): on the second
   *  morning he is helped down from the garrison; he lies ill at home, fed at the family's meals; on the last day of the
   *  illness he sits up and goes back up to the garrison in the evening */
  private guardSickHome(k: number, n: number): Seg[] {
    const P = this.P, d = this.d, fam = this.hh, FH = P.hday(fam.id, d), hv = P.walkH('garrison_sleep', fam.home, d, 'terrace', 'town'), last = k === n - 1;
    if (k === 1) { this.add(7.3 + 0.4 * this.r.next(), 'garrison_sleep', 'lie_ill', 'lying ill in the garrison quarters', 'terrace'); this.add(this.t + 0.3, 'garrison_sleep', 'eat', 'a little food brought to the sick', 'terrace');
      this.add(this.t + hv * 1.4, 'road:town', 'walk', 'helped down to his family in the town, to be nursed at home', 'road'); }
    else this.add(Math.max(FH.breakfast, this.sun.rise), fam.home, 'lie_ill', 'lying ill at home, nursed by his family', 'town');
    if (this.t < FH.breakfast + FH.bLen && k > 1) this.add(this.t + 0.3, fam.home, 'eat', 'a little food brought to the sick', 'town');
    this.add(Math.max(this.t + 0.3, FH.noon), fam.home, last ? 'rest' : 'lie_ill', last ? 'sitting up, recovering, at home with his family' : 'lying ill at home, nursed by his family', 'town');
    this.add(this.t + 0.4, fam.home, 'eat', last ? 'the midday meal with his family: recovering' : 'a little food, brought by his wife', 'town');
    if (last) { const up = Math.max(this.t + 0.5, Math.min(FH.supper, this.sun.set - 0.5) - hv - 0.1); this.add(up, fam.home, 'talk', 'sitting up with his family, recovering', 'town'); this.add(this.t + hv, 'road:terrace', 'walk', 'back up to the garrison, recovered', 'road');
      this.add(this.t + 0.5, HEARTHS[this.p.file % 3], 'eat', 'the evening meal at the hearth, back with his file', 'terrace'); return this.finish(); }
    this.add(Math.max(this.t + 0.5, FH.supper), fam.home, 'lie_ill', 'lying ill at home, nursed by his family', 'town'); this.add(this.t + 0.4, fam.home, 'eat', 'a little food, brought by his wife', 'town');
    this.add(24, fam.home, 'lie_ill', 'lying ill at home, nursed by his family', 'town'); return this.segs;
  }
  ...
  private postpartum(): Seg[] {
    ...
    else if (this.nursing) this.atHome(this.t + r.range(1, 2), 'spin', 'spinning, the sick little one beside her'); // (no outing with a sick little one: sickKeepers)
    ...
    else if (wetHours(this.C.wx, this.t, this.t + 1) === 0 && this.canDraw()) this.well(this.t + 0.3, 'fetching water with the baby'); else this.atHome(this.t + r.range(1, 2), 'spin', 'spinning, the baby beside her');
  ...
  private small(): Seg[] {
    ...
    // (a sick little one is with the one who keeps it at home today: Population.sickKeepers, A S2 and B S3 of shadow review r8)
    const kp = P.sick(this.pid, d) ? P.sickKeepers(hid, d).get(this.pid) : undefined;
    // (not a kinswoman on the day she comes to keep the house: her day begins in her own house; soak on D-175: a teleport)
    const ok = (x: number) => x !== this.pid && mem.includes(x) && (this.ageOf(x) >= 12 || x === hk) && P.persons[x].job !== 'guard' && !(x === hk && P.persons[x].marry === d);
    const nurseNow = this.p.nurse !== undefined && d >= this.p.marry && ok(this.p.nurse) ? this.p.nurse : undefined;
    let m = kp !== undefined && ok(kp) ? kp : this.p.mother >= 0 && ok(this.p.mother) ? this.p.mother : nurseNow ?? (hk >= 0 && ok(hk) ? hk : undefined) ?? mem.find(x => ok(x) && P.persons[x].sex === 'f' && this.ageOf(x) >= 14);
    ...
    const ms = P.rawPlan(m, d); const Mo = P.persons[m]; const inf = this.age === 0; const ill = P.sick(this.pid, d);
    ...
    const wakeM = ms.find(s => s.t0 > 2 && s.act !== 'sleep' && !NIGHT_FEED.test(s.why))?.t0 ?? 6;
    ...
    if (inf) { // ---- an infant
      ...
      const feedsM = lactating ? ms.filter(s => nursing(s) && !NIGHT_FEED.test(s.why)) : [];
      ...
      for (const t1 of cuts) { if (t1 <= t0) continue; const mid = (t0 + t1) / 2; t0 = t1; const M = segAt(ms, mid); const nap = napAt(mid);
        ...
        else if (M.act === 'sleep' || NIGHT_FEED.test(M.why)) { a = 'sleep'; why = `asleep beside ${rel}`; }
    ...
    const ss = sib !== undefined ? P.rawPlan(sib, d) : null;
    ...
    const eAll = elder !== undefined ? P.rawPlan(elder, d) : null; const es = eAll && this.r.chance(U.along_with_grandparent) ? eAll : null;
    ...
    const homeBound = (x: number | undefined) => { if (x === undefined) return false; const xs = P.rawPlan(x, d), xh = P.households[P.home(x, d)].home; let tot = 0, home = 0;
    ...
    const mAll = minderPid === undefined ? null : minderPid === elder ? eAll : P.rawPlan(minderPid, d);
    ...
    const myFeeds = lactating && this.age === 1 && !P.nurslings(m, d).some(c => c !== this.pid && this.ageOf(c) === 0);
    ...
    const homeRun = (xs: Seg[], at: string, t: number): [number, number] | null => { const i = xs.findIndex(s => t < s.t1); const up = (s: Seg) => s.place === at && s.act !== 'sleep' && s.act !== 'lie_ill' && !NIGHT_FEED.test(s.why);
    ...
    const outing = (am: boolean) => taken || ill ? null : (() => {
      ...
      { let e = h0; while (e < h1) { const g = segAt(ms, e + 0.01); if (g.place !== base || g.act === 'eat' || g.act === 'sleep' || NIGHT_FEED.test(g.why) || (this.age === 1 && nursing(g)) || keepIn(e + 0.01)) break; e += 0.1; } h1 = Math.min(h1, e - 0.05); }
      ...
      if (k !== 'lane') { const cand = (k === 'kin' ? this.hh.kin : Mo.ties.map(o => P.home(o, d))).map(h => P.households[h]).filter(H => H.id !== this.hh.id && H.q === this.hh.q && H.zone === this.hh.zone);
        ...
        const w = Hh ? P.membersOn(Hh.id, d).find(x => P.persons[x].sex === 'f' && this.ageOf(x) >= 14 && !!homeRun(P.rawPlan(x, d), Hh.home, mid)) : undefined;
        ...
        const run = Hh && w !== undefined ? homeRun(P.rawPlan(w, d), Hh.home, mid)! : null;
        if (Hh && run && Math.min(h1, run[1]) - Math.max(h0, run[0]) >= 0.4) return { h0: Math.max(h0, run[0]), h1: Math.min(h1, run[1]), place: Hh.home, host: w!,
          why: k === 'kin' || this.hh.kin.includes(Hh.id) ? 'at a kinswoman’s house in the lane, playing with her children' : 'playing at a neighbour’s house with their children' }; }
    ...
    const plans = new Map<number, Seg[]>(); const planOf = (x: number) => plans.get(x) ?? plans.set(x, P.rawPlan(x, d)).get(x)!;
    ...
    let t0 = 0;
    const put = (t1: number, place: string, a0: ActivityId, why0: string, where: Where, w: number) => { if (t1 <= this.t + 1e-4) return; const prev = this.segs[this.segs.length - 1];
      // fed already: a little one who has had its meal with the house's minder (the household's hour) sits by the one it is
      // with at her own later meal, and is not fed a second time (day 341: "a meal with the elder sister", then "a meal with
      // the household" as the mother came in from the canal)
      let a = a0, why = why0; if (a === 'eat' && !/nursed|softened bread/.test(why) && this.segs.some(s => s.act === 'eat' && !/nursed|softened bread/.test(s.why) && s.why !== why + taken && s.t1 > this.t - 1 && s.t0 < this.t + 1e-6)) {
        const other = w === this.p.mother ? 'the mother' : w === m ? rel : 'the one minding it'; a = 'play'; why = this.age < 2 ? `on ${other}’s knee at ${w === this.p.mother || w === m ? 'her' : 'the'} meal, fed already` : `beside ${other} at the meal, fed already`; }
    ...
    for (const t1 of cuts) {
      ...
      if (myFeeds && NIGHT_FEED.test(M.why) && nursing(M)) { put(t1, M.place, 'eat', / in the night/.test(M.why) ? 'nursed in the night, half asleep' : / at first light/.test(M.why) ? 'nursed at first light, half asleep' : 'nursed before sleeping, half asleep', M.where, m); continue; }
      ...
      if (M.act === 'sleep' || NIGHT_FEED.test(M.why) || (M.place === base && ((mid >= bedtime && mid > 12) || mid < wakeT))) { put(t1, M.place, 'sleep', 'asleep', M.where, m); continue; }
      ...
      if (this.age === 1 && lactating && nursing(M) && (myFeeds || (m === nurseNow ? /motherless child/ : /little one/).test(M.why))) { put(t1, M.place, 'eat', `nursed by ${rel}`, M.where, m); continue; }
      ...
      if (outW) { if (outW.wk > 0 && (mid < outW.h0 + outW.wk || mid > outW.h1 - outW.wk)) { const back = mid > outW.h1 - outW.wk;
          put(t1, `road:${baseW}`, 'walk', outW.place.startsWith('lane') ? (back ? 'walking home along the lane with the other children' : 'walking along the lane with the other children') : /kinswoman/.test(outW.why) ? (back ? 'walking home along the lane from the kinswoman’s house' : 'walking along the lane to the kinswoman’s house') : back ? 'walking home along the lane from the neighbour’s house' : 'walking along the lane to a neighbour’s house', 'road', -1); continue; }
  ...
  // ---------------------------------------------------------------- festivals, weddings, hearings (D-211; C)
  /** a day of the festival: in the best clothes (build's pass), the festival supper (evening) */
  private festive = false; private supperWhy: string | null = null;
  /** a stretch at home on a free day: a woman's the spindle now and then and talk, a man's talk and rest, a child's play */
  private freeHome(until: number, why = 'at home, a festival day') {
    if (until <= this.t + 0.02) return; const f = this.p.sex === 'f';
    if (this.age < 13 && this.p.job === 'child') { this.atHome(until, 'play', 'playing at home in the best clothes'); return; }
    for (let g = 0; g < 8 && this.t < until - 0.05; g++) { const e = Math.min(until, this.t + this.r.range(0.6, 1.4)); const k = this.r.next();
      if (f && k < 0.3 && this.t > this.sun.rise && e < this.sun.set) this.atHome(e, 'spin', 'spinning a little at home'); else if (k < 0.75) this.atHome(e, 'talk', 'with the household'); else this.atHome(e, 'rest', why); }
    if (this.t < until) this.atHome(until, 'rest', why);
  }
  /** out of doors at leisure is for dry, clear hours in the light, and not in the E-64 heat's lull (11:30-15:30) */
  private fairOut(a: number, b: number) {
    const wx = this.C.wx; if (wetHours(wx, a - 0.25, b + 0.25) > 0 || (wx.dustH && wx.dustH[0] < b + 0.2 && wx.dustH[1] > a - 0.2)) return false;
    if (a < this.sun.rise + 0.3 || b > this.sun.set + 0.3) return false; if (this.C.heatRest && a < 15.5 && b > 11.5) return false; return true;
  }
  /** the leisure of a free day until `until`: the lane with the neighbours in the best clothes, a visit to kin or a friend, or
   *  the house (lives.json festival.leisure; C) */
  private festLeisure(until: number, childWhy?: () => string, tag = 'a festival day') {
    const W = this.homeW, l = `lane:${this.hh.q}`, kid = this.age < 13 && this.p.job === 'child';
    let visited = false;
    for (let g = 0; g < 8 && this.t < until - 0.4; g++) {
      const len = Math.min(until - this.t, this.r.range(0.7, 1.6)), k = this.choose(L.festival.leisure as Record<'lane' | 'visit' | 'home', number>);
      const wk = this.P.walkH(this.home, l, this.d, W, W);
      if (k === 'lane' && len > 2 * wk + 0.3 && this.fairOut(this.t, this.t + len)) { this.go(l, W, 'out to the lane');
        const [a, why]: [ActivityId, string] = kid ? ['play', childWhy ? childWhy() : `playing in the lane with the other children, ${tag}`] : this.p.sex === 'm' && this.r.chance(0.35) ? ['gamble', `knucklebones with the men of the lane, ${tag}`] : ['talk', this.p.sex === 'f' ? `sitting out in the lane with the women in their best clothes, ${tag}` : `talking with the men of the lane, ${tag}`];
        this.add(Math.max(this.t + 0.3, this.t + len - 2 * wk), l, a, why, W); this.go(this.home, W); continue; }
      if (k === 'visit' && !kid && !visited) { const v = this.visitTarget(); if (v && this.fairOut(this.t, this.t + len) && len > 2 * this.P.walkH(this.home, v.place, this.d, W, v.where) + 0.4) { visited = true; this.go(v.place, v.where, 'visiting for the festival'); this.add(Math.max(this.t + 0.3, this.t + len - this.P.walkH(v.place, this.home, this.d, v.where, W)), v.place, 'talk', `visiting ${v.name} for the festival`, v.where); this.go(this.home, W); continue; } }
      this.freeHome(Math.min(until, this.t + len), `at home, ${tag}`);
    }
    if (this.t < until) this.freeHome(until, `at home, ${tag}`);
  }
  /** a festival day off (E-33, E-38; D-211, C): the morning at home, the house's head to the šip for its share of the meat, a
   *  boy or two along with him; the feast of the household at midday; the heat's lull at home; the lanes and the kin in the
   *  afternoon in the best clothes; the festival supper, and the women's drum in the lane at dusk (evening) */
  private festivalDay(): Seg[] {
    const P = this.P, d = this.d, H = this.hd, W = this.homeW, C = this.C, F = C.festival!; this.festive = true;
    const trip = P.sipTrip(this.hh.id, d), kid = this.age < 13 && this.p.job === 'child';
    const games = () => this.r.pick(GAMES);
    if (kid) { // ---- a child of 5-12
      const wake = Math.max(this.sun.rise - 0.9, H.breakfast - lerp(0.2, 0.8, this.p.trait) - 0.3 * u01(P.seed, S.kid, this.pid, d));
      this.atHome(Math.min(wake, H.breakfast - 0.05), 'sleep', 'asleep'); if (this.t < H.breakfast) this.atHome(H.breakfast, 'play', 'playing in the courtyard before the morning bread');
      this.atHome(this.t + H.bLen, 'eat', 'breakfast with the household');
      // (a boy of 7 or more goes with the house's head to the šip on one festival in two: his segments are the man's, C)
      const along = !!trip && this.p.sex === 'm' && this.age >= 7 && trip.pid !== this.pid && u01(P.seed, S.kid, this.pid, d, 61) < 0.5 && trip.leave > this.t + 0.05;
      if (along) { const fs = P.rawPlan(trip!.pid, d), i0 = fs.findIndex(s => s.t0 >= trip!.leave - 1e-6 && s.where === 'road'); let i1 = i0; while (i1 >= 0 && i1 < fs.length && fs[i1].place !== this.home) i1++;
        if (i0 > 0 && i1 < fs.length) { this.festLeisure(fs[i0].t0); if (this.t < fs[i0].t0) this.freeHome(fs[i0].t0);
          for (const s of fs.slice(i0, i1)) { const road = s.where === 'road'; this.add(s.t1, s.place, road ? 'walk' : 'rest', road ? (s.t0 < fs[i0].t0 + 0.01 ? 'walking to the šip with his father, in his best clothes' : 'walking home from the šip with his father') : 'beside his father at the šip on the open ground, watching the sheep brought up and the meat shared out', s.where, true); this.segs[this.segs.length - 1].with = trip!.pid; }
          this.cur = this.home; this.curW = W; } }
      this.festLeisure(H.noon - 0.05, games);
    } else {
      this.morning(this.rise() + 1.1, true);
      if (trip && trip.pid === this.pid) { this.freeHome(trip.leave); if (this.t < trip.leave) this.atHome(trip.leave, 'rest', 'at home, a festival day');
        this.go('offering_place', 'town', `walking to the šip at the offering place${F.bagayadis ? ' for Bāgayādiš' : ''}, in the best clothes`);
        this.add(this.t + trip.len * 0.6, 'offering_place', 'talk', 'at the šip on the open ground: the sheep and goats brought up and offered, the people of the town gathered (E-33)', 'town');
        this.add(this.t + trip.len * 0.4, 'offering_place', 'queue', 'on the open ground, taking the household’s share of the meat from the storekeepers (E-33)', 'town');
        this.go(this.home, W, 'carrying home the household’s share of the šip meat', 'carry_sack'); }
      this.festLeisure(H.noon - 0.05);
    }
    // the feast of the household (the meat of the šip where the house fetched its share; beer or wine as the stores allowed)
    if (this.t < H.noon) this.freeHome(H.noon);
    this.atHome(this.t + H.nLen + 0.5, 'eat', trip ? 'the festival meal with the household: the meat of the šip, bread and beer' : 'the festival meal with the household: bread, a little meat and beer');
    // the heat's lull at home, then the afternoon out
    if (C.heatRest || C.wx.tmax >= 31) { const e = Math.max(this.t + 0.5, 15.4); if (kid && this.age < 10) this.atHome(e, 'sleep', 'sleeping through the heat of the day'); else if (this.age >= 14) { this.atHome(Math.min(e, this.t + 2), 'sleep', 'sleeping through the heat of the day'); if (this.t < e) this.freeHome(e); } else this.atHome(e, 'rest', 'resting in the shade through the heat'); }
    this.festLeisure(Math.max(this.t + 0.3, Math.min(H.supper - 0.2, this.sun.set - 0.8)), kid ? games : undefined);
    this.supperWhy = 'the festival supper with the household';
    this.evening(this.t); return this.finish();
  }
  /** a wedding day (E-73; D-211, C): the bride's house dresses her, carries the dowry through the lanes at the procession's
   *  pace (Population.weddingPlan) and feasts with the groom's house in its courtyard until dusk, when the women drum; the
   *  bride stays in her new house. The courtyard's hours go indoors in rain, and at leisure in the dust (planCheck (a)) */
  private weddingDay(w: { day: number; bride: number; groom: number; from: number; to: number }, role: 'bride' | 'groom' | 'from' | 'to'): Seg[] {
    const P = this.P, d = this.d, WP = P.weddingPlan(w), kid = this.age < 13 && this.p.job === 'child', f = this.p.sex === 'f'; this.festive = true; this.noWater = true;
    const T = P.households[w.to], Fh = P.households[w.from], tW = WP.tW, fW = WP.fW, HT = P.hday(w.to, d), HF = P.hday(w.from, d);
    const guest = role === 'bride' || role === 'from';
    // (the lodging groom and the bride wake in the houses they leave today: the teleport check stands down on the move's day)
    const startHome = role === 'bride' ? Fh.home : role === 'groom' && P.persons[this.pid].hh !== w.to ? P.households[P.persons[this.pid].hh].home : this.home;
    const startW: Where = role === 'bride' ? fW : role === 'groom' && P.persons[this.pid].hh !== w.to ? (P.households[P.persons[this.pid].hh].zone === 'plain' ? 'plain' : 'town') : this.homeW;
    const Hm = role === 'bride' || role === 'from' ? HF : role === 'groom' && P.persons[this.pid].hh !== w.to ? P.hday(P.persons[this.pid].hh, d) : HT;
    const at = (t1: number, place: string, act: ActivityId, why: string, where: Where) => this.add(t1, place, act, why, where);
    // the yard of the feast house: out of doors while it is dry, indoors in the rain, and at leisure indoors in the dust
    const yard = (t1: number, act: ActivityId, why: string, whyIn: string) => { const wx = this.C.wx; let a = this.t;
      const bad = (q: number) => !!wx.rainQ[Math.min(95, q)] || (!!wx.stormH && q * 0.25 + 0.25 > wx.stormH[0] && q * 0.25 < wx.stormH[1]) || (act !== 'eat' && !!wx.dustH && q * 0.25 + 0.25 > wx.dustH[0] && q * 0.25 < wx.dustH[1]);
      while (a < t1 - 1e-6) { const q = Math.floor(a * 4 + 1e-6), b0 = bad(q); let e = Math.min(t1, (q + 1) * 0.25); while (e < t1 - 1e-6 && bad(Math.floor(e * 4 + 1e-6)) === b0) e = Math.min(t1, e + 0.25); at(e, T.home, act, b0 ? whyIn : why, tW); a = e; } };
    // ---- the morning
    this.cur = startHome; this.curW = startW;
    // (the women of the houses grind before the morning bread as on any day; the others rise for it)
    const grinder = f && this.age >= 14 && role !== 'bride' && !kid, wake = grinder ? Math.min(this.rise(), Hm.breakfast - 1) : Math.max(Math.min(this.rise(), Hm.breakfast - 0.3), Hm.breakfast - 0.35 - 0.4 * this.r.next());
    at(wake, startHome, 'sleep', role === 'bride' ? 'asleep (the last night in her father’s house)' : role === 'groom' && startHome !== this.home ? 'asleep (the last night in the lodging)' : 'asleep', startW);
    if (grinder && Hm.breakfast - this.t > 0.3) at(Hm.breakfast, startHome, 'grind', 'grinding the household’s flour at the quern', startW);
    else if (this.t < Hm.breakfast) at(Hm.breakfast, startHome, kid ? 'play' : 'rest', kid ? 'playing in the courtyard before the morning bread' : 'up early for the wedding', startW);
    at(this.t + Hm.bLen, startHome, 'eat', role === 'bride' ? 'the last breakfast in her father’s house' : role === 'groom' && startHome !== this.home ? 'the last breakfast in the lodging' : 'breakfast, the house up early for the wedding', startW);
    if (role === 'groom' && startHome !== this.home) { this.go(this.home, this.homeW, 'carrying his things to the new house', 'carry_sack'); at(Math.max(this.t + 0.3, WP.arrive - 0.3), this.home, 'clean', 'readying the new house for the bride: mats laid, the jars set out', tW); }
    else if (guest) {
      const dressWhy = role === 'bride' ? 'dressed for the wedding by the women of her house, the veil put on' : f && !kid && this.age >= 14 ? 'dressing the bride with the women of the house' : kid ? 'playing at home while the bride is dressed' : 'at home, the house readying the bride and her dowry';
      // (the morning before it: the children in the lane, the others about the house; the bride with the women of her house)
      if (this.t < WP.dress[0] - 0.05) { if (kid) this.festLeisure(WP.dress[0], () => this.r.pick(GAMES), 'the day of the wedding'); else if (role === 'bride') at(WP.dress[0], startHome, 'spin', 'spinning with the women of her house on the morning of her wedding', startW); else this.homeHours(WP.dress[0], 'at home before the wedding'); }
      if (this.t < WP.dress[0]) at(WP.dress[0], startHome, kid ? 'play' : 'rest', kid ? 'playing at home, the day of the wedding' : 'at home before the wedding', startW);
      at(WP.proc, startHome, kid ? 'play' : role === 'bride' || (f && this.age >= 14) ? 'talk' : 'rest', dressWhy, startW);
      // the procession: the women carry the dowry, the men and the children walk with it, the bride among them
      const k = P.membersOn(w.from, d).filter(x => P.persons[x].sex === 'f' && P.ageOn(x, d) >= 14 && x !== w.bride).indexOf(this.pid);
      const load = ['the bedding and the rolled mats of the dowry', 'the bride’s chest of cloth', 'the jars and bowls of the dowry', 'the dowry’s quern stone in a sling'][Math.max(0, k) % 4];
      if (role === 'bride') at(this.t + WP.walk, `road:${tW}`, 'walk', 'going in procession through the lanes to the groom’s house, veiled, the dowry carried before her (E-73)', 'road');
      else if (f && this.age >= 14) { at(this.t + WP.walk, `road:${tW}`, k % 3 === 2 ? 'carry_jar' : 'carry_sack', `carrying the bride’s dowry through the lanes to the groom’s house: ${load} (E-73)`, 'road'); }
      else at(this.t + WP.walk, `road:${tW}`, 'walk', kid ? 'walking with the bride’s procession through the lanes' : 'walking with the bride’s procession through the lanes to the groom’s house', 'road');
      this.cur = T.home; this.curW = tW;
      yard(WP.feast[0], kid ? 'play' : 'talk', role === 'bride' ? 'received at the groom’s door with his kin, in the courtyard' : 'at the groom’s house, the dowry set down in the courtyard', role === 'bride' ? 'received at the groom’s door with his kin, indoors out of the weather' : 'at the groom’s house, the dowry set down indoors');
    } else { // the house of the feast: the women cook and bake, the men ready the house; they receive the procession
      if (kid) { if (WP.arrive - 0.3 > this.t + 0.4) this.festLeisure(WP.arrive - 0.3, () => this.r.pick(GAMES), 'the day of the wedding'); at(WP.arrive, this.home, 'play', 'playing at home, the house readied for the wedding', tW); }
      else if (f && this.age >= 14) { if (WP.arrive - this.t > 2.4) this.homeHours(WP.arrive - 2, 'at home before the wedding');
        if (WP.arrive - this.t > 1.5) at(Math.max(this.t + 0.3, WP.arrive - 1.2), this.home, 'grind', 'grinding for the wedding bread', tW);
        if (WP.arrive - this.t > 0.8) at(Math.max(this.t + 0.3, WP.arrive - 0.55), this.home, 'bake', 'baking the bread for the wedding feast', tW); at(WP.arrive, this.home, 'cook', 'cooking the meat for the wedding feast', tW); }
      else { if (WP.arrive - this.t > 1.6) this.homeHours(WP.arrive - 1, 'at home before the wedding'); at(WP.arrive, this.home, role === 'groom' ? 'rest' : 'clean', role === 'groom' ? 'at home in his best clothes, waiting for the bride' : 'readying the house for the wedding: mats and cushions laid out', tW); }
      yard(WP.feast[0], kid ? 'play' : 'talk', role === 'groom' ? 'receiving the bride and her kin at the door, in the courtyard' : 'receiving the bride’s kin and her dowry, in the courtyard', role === 'groom' ? 'receiving the bride and her kin at the door, indoors out of the weather' : 'receiving the bride’s kin and her dowry, indoors');
    }
    // ---- the feast, the afternoon, the supper, the drum
    yard(WP.feast[1], 'eat', 'the wedding feast in the courtyard: meat, bread and beer with the kin of both houses', 'the wedding feast indoors, out of the weather, with the kin of both houses');
    ...
    if (role === 'from') { this.go(this.home, this.homeW, 'going home from the wedding'); const bed = Math.max(this.t + 0.1, this.bed());
      if (this.t < bed) this.atHome(bed, 'rest', 'home from the wedding'); return this.finish(); }
    const bed = Math.max(this.t + 0.1, this.bed()); if (this.t < bed) this.atHome(bed, 'rest', role === 'bride' ? 'in her new house' : 'sitting up after the wedding');
    return this.finish();
  }
  /** the healer's calls at the sick houses near her own in the morning (Population.healerRound; D-211, C) */
  private healerCalls() {
    const R = this.P.healerRound(this.pid, this.d); if (!R.length || this.nursing) return;
    for (const c of R) { if (this.t > 11.2) break; const H = this.P.households[c.house], w: Where = H.zone === 'plain' ? 'plain' : 'town', nm = nameFor(this.P.seed, this.P.persons[c.sick]);
      this.go(H.home, w, 'going to a sick house with her herbs'); this.add(this.t + this.r.range(0.3, 0.55), H.home, 'talk', `a healer’s call: herbs steeped and a poultice laid on for ${nm ?? 'the one lying ill'}, a word with the one nursing (E-72)`, w); }
    this.go(this.home, this.homeW, 'home from her calls at the sick houses');
  }
  /** a dispute of yesterday heard before an official at the official building (E-74; D-211, C): the morning's walk, the
   *  hearing with a witness each, the settlement; the rest of the day at home */
  private hearingDay(hr: { other: number; t: number; why: string }): Seg[] {
    const P = this.P, W = this.homeW, o = nameFor(P.seed, P.persons[hr.other]) ?? 'the other';
    this.morning(this.rise() + 0.8, true);
    const lv = hr.t - P.walkH(this.home, 'official_bldg', this.d, W, 'town') - 0.05; if (this.t < lv) this.homeHours(lv, 'at home');
    this.go('official_bldg', 'town', 'to the official building for the hearing, with a witness');
    this.add(this.t + 0.25, 'official_bldg', 'queue', 'waiting with a witness to be heard', 'town');
    this.add(this.t + this.r.range(0.4, 0.8), 'official_bldg', 'talk', `before an official: the dispute with ${o} ${hr.why} heard, a witness each, settled by a pledge (E-74)`, 'town');
    this.go(this.home, W, 'going home after the hearing');
    if (this.t < this.hd.noon - 0.3) this.homeHours(this.hd.noon, 'at home'); this.noonAtHome();
    this.homeHours(Math.max(this.t + 0.5, 15 + this.r.next()), 'at home'); this.evening(Math.max(this.t, this.sun.set - 1)); return this.finish();
  }
  /** the days after a birth whose baby has died (A S2 of shadow review r9; D-211, C): at home, the mourning of the house's
   *  first days, then rest and light work, and the women of her kin come to sit with her; no word of the baby */
  private bereavedRest(): Seg[] {
    const r = this.r, H = this.hd, mo = this.P.mourning(this.pid, this.d) > 0;
    this.atHome(Math.min(this.rise() + 0.6, H.breakfast), 'sleep', 'asleep'); if (this.t < H.breakfast) this.atHome(H.breakfast, 'rest', 'at home after the birth, the child dead');
    this.atHome(this.t + H.bLen, 'eat', mo ? 'breakfast with the household, in mourning' : 'breakfast with the household');
    this.atHome(Math.max(this.t + 1, this.t + r.range(1, 2)), mo ? 'rest' : 'spin', mo ? 'mourning the child at home' : 'spinning at home, off work after the birth');
    if (r.chance(0.6)) this.atHome(this.t + r.range(0.6, 1.2), 'talk', 'women of her kin come to sit with her');
    this.atHome(Math.max(this.t, H.noon), 'rest', mo ? 'mourning the child at home' : 'resting at home after the birth'); this.atHome(this.t + H.nLen, 'eat', mo ? 'a meal with the household, in mourning' : 'the midday meal with the household');
    this.atHome(this.t + r.range(1, 2), 'sleep', 'a sleep in the afternoon');
    if (!mo && r.chance(0.5)) this.atHome(this.t + 1, 'spin', 'spinning (off work after the birth)');
    this.evening(Math.max(this.t, this.sun.set - 1)); return this.finish();
  }
  // ---------------------------------------------------------------- jobs
  private guard(): Seg[] {
    ...
    let awayQ = ''; let awayEnd = -9; // (a while at the hearth between two trips down: no errand and the river back to back)
    const away = (t1: number, pl: string, act: ActivityId, why: string, h: number, there: string, back = 'back up to the garrison') => {
      const wk = P.walkH('garrison_sleep', pl, d, 'terrace', 'town'); awayQ = quarterOf(pl); this.add(this.t + wk, 'road:town', 'walk', there, 'road');
    ...
    const leisure = (t1: number) => { while (this.t < t1 - 0.2) { hearthRun = tail();
      const room = t1 - this.t, light = this.t > this.sun.rise - 0.2 && this.t < this.sun.set - 0.4, wetNow = wetHours(C.wx, this.t, this.t + 3) > 0 || dayStorm(C), dust = !!C.wx.dustH && C.wx.dustH[0] < this.t + 1.5 && C.wx.dustH[1] > this.t;
      ...
      const wk = (pl: string) => P.walkH('garrison_sleep', pl, d, 'terrace', 'town'), rested = this.t - awayEnd >= 0.75, fresh = (pl: string) => this.t - awayEnd >= 2 || quarterOf(pl) !== awayQ; // (not down to the same quarter again within two hours: S7 of reviewer B, r7)
      if (rested && fresh('training:q_lt_e') && drill && light && !wetNow && !dust && !hot && this.t >= 6.5 && room > 2 * wk('training:q_lt_e') + 1.2) { drill = false; away(t1, 'training:q_lt_e', 'train', p.rank === 1 ? 'the men of his file at the bow and the spear, the leader of ten setting the marks' : 'practising with the bow and the spear with men of his file (XEN-CYR 1.2.12)', r.range(1, 1.6), 'down to the practice ground in the lower town'); continue; }
      if (rested && fresh('craft_zone') && errand && light && !wetNow && !dust && this.t >= 7 && room > 2 * wk('craft_zone') + 0.8) { errand = false; away(t1, 'craft_zone', 'exchange', r.chance(0.5) ? 'an errand for his file at the craftsmen’s quarter: a new shield strap, a spear shaft to be mended' : 'an errand for his file in the town: oil for the lamps, a pot for the hearth', r.range(0.6, 1.2), 'down to the town on an errand for his file'); continue; }
      if (rested && fresh('river') && river && light && !wetNow && !dust && !hot && this.t >= 7 && room > 2 * wk('river') + 1.2) { river = false; away(t1, 'river', 'wash', 'washing his clothes at the river', r.range(1, 1.8), 'down to the river'); continue; }
      ...
      if (hearthRun >= 2 && light && !wetNow && !dust && room > 0.6) { this.add(this.t + Math.min(room, r.range(0.4, 0.9)), 'forecourt', 'talk', 'talking with men of another file in the court', 'terrace'); hearthRun = 0; continue; }
      ...
      else if (u < 0.78 && light && !wetNow && !dust && this.t > 7 && this.t < 18) { this.add(this.t + Math.min(dt, 0.6), 'forecourt', 'talk', 'talking with men of another file in the court', 'terrace'); hearthRun = 0; continue; } else this.add(this.t + dt, hearth, 'rest', 'resting', 'terrace');
      hearthRun += dt; }
    ...
    let issueH = C.issue.get(p.group); if (issueH !== undefined) for (const [ra, rb] of wetSpells(C.wx)) if (ra < issueH + 2 && rb > issueH) issueH = rb; // (carried up when the rain has passed)
    if (issueH !== undefined && issueH + 2 > 23.5) issueH = undefined;
    if (issueH !== undefined && ph >= 3 && p.idx % 3 === 1) jobs.push([issueH, issueH + 2, G.issuePlace, 'carry_sack', 'carrying the garrison’s ration up from the Terrace depot']);
     ...
     *  is the town plan's) */
    // (and surely on a day someone of the house is ill: S3 of reviewer A, r7, a son of 4 ill in the town and his father at the
    // hearth through 4 h 49 min of free daylight)
    const wantVisit = !!fam && (P.guardVisits(this.pid, d) || P.membersOn(fam.id, d).some(x => x !== this.pid && P.sick(x, d))); let visited = false;
    const free = (t1: number, visitChance: number, visitEnd?: number) => {
      ...
      if (!mine.length && fam && !visited && (wantVisit || r.chance(visitChance * 0.3)) && Math.min(tv0, this.sun.set + 0.5) - Math.max(this.t, this.sun.rise) - 2 * hv >= 1.5 && !C.wx.storm && this.rainIn(this.t, t1) < 1) {
        ...
        if (this.t - awayEnd < 2 && (fam.q ?? fam.home) === awayQ) { if (Math.min(tv0, this.sun.set + 0.5) - (awayEnd + 2) - 2 * h >= 1.5) leisure(awayEnd + 2); else { visited = false; leisure(t1); return; } } // (not down to the same quarter again within two hours: S7 of reviewer B, r7)
        awayQ = fam.q ?? fam.home;
        ...
        // (his wife ill today: he does her work in the house when he comes, the water and the fire; the neighbours' lane can
        // wait: A S10 and B S3 of shadow review r8, a guard "talking" and "resting" at home for 7 h while his children of 10 and
        // 7 fetched the water and lit the fire)
        const wifeIll = kinW.some(x => P.persons[x].sex === 'f' && this.ageOf(x) >= 14 && P.sick(x, d)), well = `well:${fam.q}`, wk = P.walkH(fam.home, well, d, 'town', 'town');
        if (wifeIll && back - this.t > 2 * wk + 1.2 && wetHours(C.wx, this.t, this.t + 2 * wk + 0.4) === 0 && this.t > this.sun.rise - 0.2 && this.t < this.sun.set - 0.6) { this.add(this.t + 0.05, fam.home, 'rest', 'taking up the water jar at home', 'town'); this.add(this.t + wk, 'road:town', 'walk', 'to the well for his family, his wife ill', 'road');
          this.add(this.t + r.range(0.2, 0.3), well, 'draw_water', 'drawing water for the house, his wife ill', 'town'); this.add(this.t + wk, 'road:town', 'carry_jar_head', 'carrying the water home', 'road'); this.add(this.t + 0.05, fam.home, 'carry_jar', 'pouring the water into the house jar', 'town'); }
        if (wifeIll && back - this.t > 0.8) this.add(this.t + 0.35, fam.home, 'cook', 'lighting the fire and warming food for the house, his wife ill', 'town');
        ...
        if (!wifeIll && back - this.t > 1.5 && r.chance(0.5) && !(C.wx.dustH && C.wx.dustH[0] < back && C.wx.dustH[1] > this.t)) this.add(Math.min(back - 0.8, this.t + r.range(0.5, 1)), `lane:${fam.q}`, r.chance(0.5) ? 'exchange' : 'talk', 'in the lane of his family’s quarter', 'town');
      ...
      if (!mine.length && !fam && !visited && t1 - Math.max(this.t, 7) > 3 && !C.wx.storm && !C.wx.wet && !C.wx.dust && this.t < 16 && P.guardVisits(this.pid, d, 0.55)) {
        ...
        visited = true; const riv = C.season !== 'winter' && r.chance(0.4); const pl = riv ? 'river' : 'lane:q_lt_e'; const h = P.walkH('garrison_sleep', pl, d, 'terrace', 'town'); leisure(Math.max(this.t + 0.2, 7, quarterOf(pl) === awayQ ? awayEnd + 2 : 0)); if (riv) river = false; awayQ = quarterOf(pl); // (not down to the same quarter again within two hours: S7 of reviewer B, r7)
        if (t1 - this.t - 2 * h < 1.2) { leisure(t1); return; }
    ...
    if (me && me.watch === 1) { // watch B, 14–22
      ...
      meal('breakfast'); free(13.2 - 0.3 * u01(P.seed, S.assign, 5500 + this.pid, d), 0.3, 13.7 - 0.2 * u01(P.seed, S.assign, 5500 + this.pid, d)); meal('a meal before the watch'); this.add(14, hearth, 'talk', 'readying for the watch', 'terrace'); watch(me, 22, d); meal('a meal after the watch'); return this.finish();
    }
  ...
  /** the words of a day kept at home by the weather: the storm's when a storm falls in the working window, else the rain's */
  private weatherOff(what: string, w: [number, number]) { const st = this.C.wx.stormH; return `${st && st[0] < w[1] && st[1] > w[0] ? stormWord(this.C.wx, w[0], w[1]) : 'rain'}: ${what}`; }
  /** the words of the walk home when the rain or the storm ends the working day early (workSpan) */
  private homeWords(b: number, w1: number) { if (b >= w1 - 1e-6) return undefined; const st = this.C.wx.stormH; return st && st[0] - 0.3 <= b && st[1] > b ? `home as the ${stormWord(this.C.wx, b - 0.3, b + 0.5)} comes on: the work stops` : 'home as the rain sets in: the work stops'; }
  private builder(): Seg[] {
    const P = this.P, p = this.p, C = this.C; const W0 = P.workWindow(C), span = workSpan(C.wx, W0[0], W0[1]); const w0 = span.a, w1 = span.b; let place = P.builderPlace(this.pid, this.d, C);
    // (the dry part of the day: out when a wet morning has cleared, home when the rain or the storm sets in for the rest of it;
    // A S3 and B S2 of shadow review r8)
    ...
    if (act === 'lay_brick') { const pick = (): [ActivityId, string] => { const k = this.choose(L.job_tasks.brick_squad.v as Record<'lay' | 'bricks' | 'mortar', number>);
      ...
      const [a1, y1] = pick(); if (this.t < 12 && !C.heatRest && w1 > 13) this.workBlock(place, where, a1, y1, Math.max(w0, this.t), 12.6, true, lunch, 12, undefined, { snap: true }); [act, why] = pick(); }
    if (pm && this.t < 12 && !C.heatRest && w1 > 13) { this.workBlock(place, where, act, why, Math.max(w0, this.t), 12.6, true, lunch, 12, undefined, { snap: true }); [place, act, why] = pm; }
    this.workBlock(place, where, act, why, Math.max(w0, this.t), w1, true, lunch);
    this.endOfWork(lunch, where, issue !== undefined, this.homeWords(w1, W0[1]));
    return this.finish();
  }
  /** the end of a Terrace working day: on a heat day (E-64) the gangs stop at noon, eat at the site and go home; then home */
  private endOfWork(lunch: string, where: Where, ration: boolean, homeWhy?: string) {
    ...
    // (a ration the day's work had no hour for is drawn at the depot on the way down)
    if (ration && this.rationSlot && !this.rationQueued) { this.rationQueued = true; if (where !== 'terrace') this.go('stair_foot', 'terrace', 'up to the depot for the ration'); this.add(this.t + (this.rationSlot[1] - this.rationSlot[0]), 'stair_foot', 'queue', 'drawing the ration at the depot on the way down', 'terrace'); }
    this.go(this.home, this.homeW, ration ? 'carrying the ration home' : homeWhy ?? 'going home', ration ? 'carry_sack' : 'walk');
    // (home before the midday meal, the work given up to the weather or done: the household's meal at home)
    if (this.t < this.hd.noon + 0.3 && !this.segs.some(x => x.act === 'eat' && x.t0 >= this.hd.noon - 1.2)) this.noonAtHome();
    this.evening(this.t);
  }
  private campWoman(): Seg[] {
    ...
    const baker = p.sub === 'baker'; const [T0, T1] = P.terraceWindows(this.pid, d)[0].w, span = workSpan(C.wx, T0, T1); // (the weather's part of her day, as the gangs': A S3, B S2 of r8)
    if (span.dry < DRY_WORK_H || infant || (C.winter && (p.id + d) % 2 === 1)) return this.homeDay(infant ? 'at home with her infant' : span.dry < DRY_WORK_H ? this.weatherOff('no bread today', [T0, T1]) : 'bread for half the gang in winter: her day at home');
    const t0 = span.a, t1 = span.b;
    ...
    const issue = C.issue.get(p.group); this.terraceRation(issue);
    ...
    let kind = baker ? (this.r.chance(L.camp_women_needed.bakers_bake) ? 'bake' : 'grind') : this.choose(L.camp_women_needed.tasks as Record<'grind' | 'bread' | 'flour', number>);
    if (kind === 'bake' && wetHours(C.wx, t0, t0 + 3.5) > 0) kind = 'grind'; // (the open ovens are not fired in the rain: planCheck (a); C)
    if (kind === 'bread' && (t0 > 10.5 || t1 < 13)) kind = 'grind'; // (the gang's bread at midday wants the whole morning and the meal in her day)
    if (kind === 'bake') { if (this.t < t0 + 1) this.add(t0 + 1, 'oven', 'knead', 'kneading dough for the gang', 'terrace'); if (this.t < t0 + 3.5) this.add(t0 + 3.5, 'oven', 'bake', 'baking bread for the gang', 'terrace'); this.workBlock('querns', 'terrace', 'grind', 'grinding flour for tomorrow', this.t, t1, true, 'work_hearth'); }
      ...
      this.workBlock('querns', 'terrace', 'grind', 'grinding grain', Math.max(this.t, t0), 11.2, true, 'work_hearth'); const wetNoon = wetHours(C.wx, 11.2, 12.7) > 0; // (in the rain the gang shelters under the Gate's roof, and the bread goes there: planCheck (a))
      this.add(11.3, 'oven', 'carry_bread', 'fetching the baked bread', 'terrace'); this.add(11.9, wetNoon ? 'gate_hall' : site, 'carry_bread', wetNoon ? 'bringing the gang its bread under the Gate’s roof, out of the rain' : 'bringing the gang its bread at midday', 'terrace');
      this.add(12.6, wetNoon ? 'gate_hall' : 'work_hearth', 'eat', wetNoon ? 'midday meal under the Gate’s roof, out of the rain' : 'midday meal', 'terrace'); this.workBlock('querns', 'terrace', 'grind', 'grinding grain', this.t, t1, true, 'work_hearth'); }
    else if (kind === 'flour') {
      ...
      this.workBlock('querns', 'terrace', 'grind', 'grinding grain', this.t, t1, true, 'work_hearth'); }
    this.endOfWork('work_hearth', 'terrace', issue !== undefined, this.homeWords(t1, T1)); return this.finish();
  }
  ...
  private child(): Seg[] {
    ...
    const follow = (ms: Seg[], camp: boolean, fe0?: number) => { const fe = fe0 ?? firstEat(ms); for (const s of spells(ms)) { const edge = s.place.startsWith('threshing') ? 'by the threshing floor' : s.place.startsWith('field') ? 'at the field edge' : 'under the trees'; let [a, why] = s.why === '(shade)' ? (age < 8 ? ['play', `playing in the shade ${edge}`] as [ActivityId, string] : ['rest', `resting in the shade ${edge}`] as [ActivityId, string]) : beside(s); let place = s.place; let w = m;
        ...
        else if (NIGHT_FEED.test(s.why)) { a = 'sleep'; why = 'asleep'; }
        ...
        if (camp && s.where === 'terrace' && ['walk', 'carry_sack', 'carry_bread', 'carry_jar_head', 'draw_water'].includes(s.act)) { a = 'play'; w = -1; if (wetHours(this.C.wx, s.t0, s.t1) > 0) { why = 'playing under the Gate’s roof with the other camp children, out of the rain'; place = 'gate_hall'; } else { why = 'playing by the querns with the other camp children while the mother fetches and carries'; place = 'querns'; } }
    ...
    if (m >= 0 && P.persons[m].job === 'camp' && p.agent >= 0) { // the slice's camp children go up with their mothers (Phase 3)
      const ms = P.rawPlan(m, d); if (ms.some(s => s.where === 'terrace')) return follow(ms, true);
    }
    ...
    if (m >= 0 && age <= 8 && p.agent < 0 && !own) { const ms = P.rawPlan(m, d);
      ...
      if (age < L.children.minded_until_age && !this.C.wx.storm) { // the mother's working hours away from home (2 h or more)
        ...
        if (away.length && ms.some(s => s.place !== this.home && s.where !== 'road' && !['talk', 'eat', 'rest', 'queue', 'exchange', 'sleep'].includes(s.act))) {
          ...
          const minder = (sis >= 0 && sis !== this.pid && this.ageOf(sis) >= 9) || mem.some(x => { if (x === this.pid || x === m || this.ageOf(x) < 14 || P.persons[x].job === 'guard' || P.sick(x, d)) return false; const xs = P.rawPlan(x, d); return away.filter(h => segAt(xs, h).place === this.home).length >= 0.7 * away.length; });
          if (!minder) {
            const homeThrough = (y: number, yh: string) => P.rawPlan(y, d).every(g => blocks.every(([x, z]) => g.t1 <= x || g.t0 >= z || g.place === yh)); // at home for the whole of every absence
            ...
            if (kw < 0 && terrace) { const women = P.quarters[this.hh.q]?.women ?? []; const k0 = Math.floor(u01(P.seed, S.assign, 8800 + this.pid, d) * women.length);
              for (let i = 0; i < Math.min(40, women.length) && kw < 0; i++) { const y = women[(k0 + i) % women.length]; const H = P.households[P.home(y, d)];
                if (H.id === this.hh.id || H.zone !== this.hh.zone || !P.present(y, d) || P.sick(y, d) || P.persons[y].job === 'guard') continue; if (homeThrough(y, H.home)) { kw = y; KH = H; who = this.hh.kin.includes(H.id) ? 'a kinswoman' : 'a neighbour'; } } }
    ...
    const CW = L.children.work; const band = age >= 13 ? 4 : age >= 12 ? 3 : age >= 10 ? 2 : age >= 7 ? 1 : 0; const she = girl ? 'her' : 'his';
    // (from thirteen near-adult hours and little play: children.work.teen_play_cap_h, B S4 of shadow review r8)
    const teen = age >= 13, playCap = teen ? CW.teen_play_cap_h : Infinity, played = () => this.segs.reduce((a, s) => a + (s.act === 'play' && s.where !== 'road' && !/^minding/.test(s.why) ? s.t1 - s.t0 : 0), 0);
    ...
    // (and where a servant is the house's waterer, the children do not fetch its water too: the house drew water four times before noon)
    const servWater = this.hd.waterer >= 0 && P.persons[this.hd.waterer].job === 'servant';
    const wantH = (girl ? CW.girl_h : CW.boy_h)[band] * lerp(0.8, 1.2, r.next()) * (C.season === 'winter' ? CW.winter_factor : 1);
    let work = 0;
    const ms = m >= 0 ? P.rawPlan(m, d) : null;
    void ms;
    const T = H.task, helper = H.fieldHelper === this.pid && !!T && P.dryTask(T, C) !== null && !dayStorm(C), bringer = H.bringer === this.pid && !!T && !helper;
    ...
    // (12-15, and the workshop trades too: the weaver's loom, the brewer's vats; from 13 a builder's, a town porter's or a groom's
    // son goes with his father now and then as a boy labourer at a boy's ration, children.work.gang_p: A S5 and B S3 of shadow
    // review r9, 0 of 1,121 sampled days for such sons; the Liduma list's boys, B; the work C)
    const GANG = ['builder', 'porter', 'groom'];
    const fa = !girl && age >= CW.trade_ages[0] && age <= CW.trade_ages[1] && this.hh.zone === 'town' && !minder && !helper && !bringer && p.agent < 0 ? mem.find(x => x !== this.pid && P.persons[x].sex === 'm' && !P.persons[x].kin && this.ageOf(x) >= age + 16 && (['craftsman', 'scribe', 'gardener', 'storekeeper', 'weaver', 'brewer'].includes(P.persons[x].job) || (age >= 13 && GANG.includes(P.persons[x].job))) && !P.sick(x, d) && P.persons[x].agent < 0) : undefined;
    const gangBoy = fa !== undefined && GANG.includes(P.persons[fa].job);
    const trade = fa !== undefined && u01(P.seed, S.kid, this.pid, d, 7) < (gangBoy ? CW.gang_p : CW.trade_p) ? (() => { const fs = P.rawPlan(fa, d), i0 = fs.findIndex((s, i) => i > 0 && s.t0 >= Math.max(4.5, this.sun.rise - 1) && s.place !== this.home && s.act !== 'sleep' && fs[i - 1].place === this.home); if (i0 < 0) return null;
      let i1 = i0; while (i1 < fs.length && fs[i1].place !== this.home) i1++; let run = fs.slice(i0, i1); if (i1 >= fs.length || !run.length || run.some(s => (s.where === 'terrace' && !gangBoy) || s.where === 'away' || s.place === '-' || s.act === 'offmap' || s.act === 'stand_guard') || run[run.length - 1].t1 - run[0].t0 < 2) return null;
      // (a boy labourer whose father works on past the house's evening leaves before him: the run ends at the father's last
      // place before the boy's walk home, and he walks home alone)
      if (run[run.length - 1].t1 > eve0) { if (!gangBoy) return null; const k = run.findIndex(s => s.t1 > eve0 - 0.6); run = run.slice(0, k); while (run.length && run[run.length - 1].where === 'road') run.pop(); if (!run.length || run[run.length - 1].t1 - run[0].t0 < 2) return null; }
      return run; })() : null;
    if (trade && trade[0].t0 < H.breakfast + H.bLen + 0.05) { this.atHome(Math.max(0.2, trade[0].t0 - 0.45), 'sleep', 'asleep'); this.atHome(trade[0].t0 - 0.2, 'rest', 'up with his father before the household'); this.atHome(trade[0].t0, 'eat', 'bread and water before leaving with his father'); }
      ...
      if (room > 0.3 && age >= 7 && !inside) {
        if (plain && !girl && this.t >= this.lightStart()) this.atHome(this.t + Math.min(room, r.range(0.25, 0.5)), 'tend_animals', 'letting the household’s animals out and giving them water');
      ...
      if (this.t < H.breakfast) { if (age >= 8 && H.breakfast - this.t > 0.5) this.atHome(H.breakfast - 0.5 * r.next() - 0.05, 'rest', 'at home'); this.atHome(H.breakfast, age < 8 ? 'play' : 'rest', age < 8 ? 'playing in the courtyard before the morning bread' : 'waiting for the morning bread'); }
    ...
    const errHouses = new Set<number>(); let errOil = false;
    const done: Record<string, number> = {}, cap: Record<string, number> = teen ? { water: 3, fuel: 2, errand: 2, birds: 1, dung: 2, grind: 2, spin: 5, learn: 3 } : { water: 2, fuel: plain ? 2 : 1, errand: 1, birds: 1, dung: 1, grind: 1, spin: 3, learn: 2 };
    // a son of ten or more of a scribe's, a storekeeper's or a craftsman's house of the town practises his father's work at
    ...
    const doWork = (until: number): boolean => {
      ...
      else if (k === 'errand') { const kin = this.hh.kin.map(h => P.households[h]).find(K => K.zone === this.hh.zone && K.id !== this.hh.id && !errHouses.has(K.id) && P.walkH(this.home, K.home, d, W, K.zone === 'plain' ? 'plain' : 'town') < Math.min(0.25, (until - this.t - 0.3) / 2));
        if (kin && (r.chance(0.5) || errOil)) { errHouses.add(kin.id); const kw: Where = kin.zone === 'plain' ? 'plain' : 'town'; this.go(kin.home, kw, 'taking bread to a kinswoman', 'carry_bread'); this.add(this.t + r.range(0.1, 0.25), kin.home, 'talk', 'handing over the bread, a word with the kinswoman', kw); this.go(this.home, W, 'going home'); }
        else if (!errOil) { errOil = true; this.go(l, W, 'to the lane with a measure of barley', 'carry_sack'); this.add(this.t + r.range(0.15, 0.35), l, 'exchange', 'exchanging a measure of barley for oil with a neighbour', W); this.go(this.home, W, 'carrying the oil home', 'carry_jar'); }
        else { this.go(l, W, 'to the lane with a message'); this.add(this.t + r.range(0.1, 0.2), l, 'talk', 'giving a message for the household to a neighbour at the door', W); this.go(this.home, W); } }
    ...
    const spell = (until: number) => {
      for (let g = 0; g < 10 && this.t < until - 0.3; g++) {
        ...
        // (a child of thirteen or more past its play: the house's work, at home; B S4 of shadow review r8)
        if (played() >= playCap - 0.25) { home(); const lt = this.t > this.sun.rise - 0.2 && e < this.sun.set; this.atHome(Math.max(this.t + 0.1, e), girl ? 'spin' : lt ? 'craft' : 'rest', girl ? 'spinning wool with a drop spindle' : lt ? 'mending baskets and tools at home' : 'at home'); continue; }
        e = Math.min(e, this.t + Math.max(0.4, playCap - played()));
        ...
        if ((k === 'lane' || (k === 'water_edge' && this.age < 7)) && fits(l, W)) outTo(l, W, this.r.pick(GAMES), 'out to the lane'); // (a child under seven is not at the water alone: S5 of reviewer B, r7; C)
        else if (k === 'friend' && fr && fits(fr.place, fr.where)) outTo(fr.place, fr.where, `playing at ${fr.house}`, `to ${fr.house}`);
        else if (k === 'friend' && fits(l, W)) outTo(l, W, this.r.pick(GAMES), 'out to the lane');
        else if (k === 'water_edge' && this.age >= 7 && fits(wEdge, W)) outTo(wEdge, W, plain ? 'playing by the canal' : 'playing by the garden channels', plain ? 'out to the canal' : 'out to the garden channels');
    ...
    const fill = (until: number, filler: (end: number) => void) => {
      for (let g = 0; g < 40 && this.t < until - 0.01; g++) {
        ...
        home(); for (let i = 0; i < mine.length; i++) { const x = mine[i]; if (x.t1 <= this.t + 1e-6) continue;
          // (the walk home from the lane with them is taken with the outing even when `until` falls at its start: the soak's
          // "apart" on day 341, a minder at home while her little ones still walked back along the lane)
          const cont = Math.abs(x.t0 - this.t) < 1e-4 && ((this.cur ?? this.home) !== this.home || this.segs[this.segs.length - 1]?.where === 'road'); // (and the lane after the walk out to it: 2792 on day 163)
          if (!cont && (x.t0 >= until || x.t0 > this.t + 1e-4)) break;
          ...
          const out = x.where === 'road' || x.place !== this.home; if (!out && until <= this.t + 1e-6) break; // (a piece at home past `until` waits for the next fill)
          this.add(out ? x.t1 : Math.min(x.t1, until), x.place, x.act, x.why, x.where, true); if (x.where === 'road' && mine[i + 1]) { this.cur = mine[i + 1].place; this.curW = mine[i + 1].where; }
    ...
    const reaps = plain && !!T?.all && age >= 9 && !minder && !inside && !dayStorm(C);
    if (reaps) {
      ...
      const h0 = P.dryTask(T!, C)!.h0, wk = P.walkH(this.home, T!.place, d, W, 'plain'); if (this.t < h0 - wk - 0.1) this.atHome(h0 - wk - 0.05, ...this.idle()); // (out when the men go: after a morning's rain, not into it)
      this.go(T!.place, 'plain', `${this.toward(T!.place)} with the men`, 'walk'); this.workBlock(T!.place, 'plain', T!.kind === 'canal' ? 'dig_canal' : T!.kind === 'other' ? T!.act : 'field_work', why, Math.max(h0, this.t), age >= 12 ? Math.min(T!.h1, 15.5) : noonF + 0.6, true, T!.place, noonF, H.bringer >= 0 ? 'the midday meal at the field: the bread brought out from home' : 'the midday meal at the field: the bread carried out in the morning');
      ...
      for (const s of trade) { if (s.t0 > this.t + 1e-4) { if (this.t < this.hd.noon + 0.5 && s.t0 > this.hd.noon + this.hd.nLen + 0.2) this.noonAtHome(); this.atHome(s.t0, ...this.idle()); } // (the midday meal at home before an afternoon at his father's work: D-211, the soak's "midday meal" at 17:06)
        const road = s.where === 'road', learn: Record<string, string> = { write_tablet: 'learning the signs beside his father, copying on a tablet', craft: 'helping his father at his craft, learning it', garden_work: 'working the beds beside his father',
          ...
          dress_stone: 'carrying off the stone chips and earth in baskets beside his father’s gang, at a boy’s ration', haul: 'in the chain passing baskets of earth beside his father’s gang, at a boy’s ration',
          lay_brick: 'passing bricks along the chain beside his father’s gang, at a boy’s ration', mould_brick: 'passing bricks along the chain beside his father’s gang, at a boy’s ration',
          queue: 'in the ration queue beside his father', shelter: 'sheltering from the rain beside his father' };
        const act2: Partial<Record<string, ActivityId>> = { dress_stone: 'haul', lay_brick: 'haul', mould_brick: 'haul' };
        const [a, why]: [ActivityId, string] = road ? ['walk', s.t0 < 12 && this.segs.every(x => x.with !== fa) ? 'walking to his father’s work with him' : 'walking with his father'] : s.act === 'eat' ? ['eat', s.t0 >= 9.5 && s.t0 <= 17 ? 'the midday meal with his father' : 'a meal with his father'] : learn[s.act] ? [act2[s.act] ?? s.act, learn[s.act]] : ['rest', 'with his father at his work'];
        this.add(s.t1, road ? s.place : s.place, a, why, s.where, true); this.segs[this.segs.length - 1].with = fa!; }
      void fh; const lastT = trade[trade.length - 1]; if (lastT.where !== 'road' && lastT.place !== this.home) { this.cur = lastT.place; this.curW = lastT.where; this.go(this.home, W, 'home ahead of his father'); }
      else { this.cur = this.home; this.curW = W; } // (the run ends as the father comes home: 4296's walk home and back)
      work += 3; this.noonAtHome(); half(eve, 1);
      ...
      else if (age < 10) { const e = this.t + r.range(0.3, 0.9), wk = P.walkH(this.home, l, d, W, W);
        // (on a mild, dry day the little ones' rest after the meal is often the lane with the other children: gap audit item 9,
        // Q-204, the lanes empty at noon; D-211, C)
        if (this.hh.zone === 'town' && r.chance(0.5) && e - this.t > 2 * wk + 0.3 && this.fairOut(this.t, e) && !mine.some(x => x.t0 < e + 0.05 && x.t1 > this.t - 0.05)) { this.go(l, W, 'out to the lane after the meal'); this.add(Math.max(this.t + 0.2, e - wk), l, 'play', this.r.pick(GAMES), W); this.go(this.home, W); }
        else fill(e, end => { if (end > this.t) this.atHome(end, 'rest', 'resting after the meal'); }); }
      half(eve, 1);
    }
  ...
  /** a day's work on the Terrace. `roofed`: the work is under a roof (inside the Treasury, at the closed palaces' doors): the
   *  E-64 noon stop is for work in the open, so it keeps the whole working day on a heat day (A S8 of shadow review r8: the
   *  doorkeepers left at 11:49 on heat days), and only a storm keeps it in (the walk up is not made in one). The weather's
   *  part of the day as the gangs' (workSpan: A S3, B S2 of r8; was: any storm in the day's light kept everyone at home). `w1`:
   *  an end of the day's work before the window's (the porters, once the day's caravan is carried up) */
  private terraceWorker(place: string, act: ActivityId, why: string, pm?: [string, ActivityId, string], ins: [number, number, string, ActivityId, string][] = [], o: { roofed?: boolean; start?: number; end?: number; endWhy?: string } = {}): Seg[] {
    const P = this.P, C = this.C, roofed = !!o.roofed; const W0 = P.workWindow(C, roofed), span = workSpan(C.wx, W0[0], W0[1], roofed);
    if (span.dry < DRY_WORK_H) return this.homeDay(this.weatherOff('no work on the Terrace today', W0));
    const w0 = Math.max(span.a, o.start ?? 0), w1 = Math.max(w0 + 0.5, Math.min(span.b, o.end ?? 24)), end = o.end; this.morning(w0 - P.walkH(this.home, 'stair_foot', this.d, this.homeW, 'terrace') - 0.05); this.go('stair_foot', 'terrace', 'going to the Terrace');
    const issue = C.issue.get(this.p.group); this.terraceRation(issue);
    ...
    if (pm && (pm[0] !== place || pm[1] !== act) && this.t < 12 && (!C.heatRest || roofed) && w1 > 13) { wb(place, act, why, Math.max(w0, this.t), 12.6, true); [place, act, why] = pm; } // a different task after the midday meal
    wb(place, act, why, Math.max(w0, this.t), w1, false);
    this.endOfWork('work_hearth', 'terrace', issue !== undefined, end !== undefined && end < span.b ? o.endWhy : this.homeWords(w1, W0[1])); return this.finish();
  }
  /** a detailed porter of the slice (sim.ts carries the depot's sacks up to the Treasury store while the plan holds him at the
   *  stair foot "waiting at the depot for loads"): up an hour before the day's caravan to carry what was left at the depot,
   *  and home once the caravan's sacks are carried up (Population.caravanDone); with no caravan (the road stopped by the
   *  weather) a morning's carrying of what is left. Was: at the depot from sunrise to 15:30 whatever came, 4.0 h a day on
   *  average "waiting for a caravan", in the afternoons for one that had come in the morning (A S4 of shadow review r8; C) */
  private depotPorter(): Seg[] {
    const c = this.P.caravan(this.d), [a, b] = this.P.depotHours(this.pid, this.d);
    return this.terraceWorker('stair_foot', 'rest', 'waiting at the depot for loads', undefined, [], { start: a, end: b, endWhy: c ? 'home, the day’s caravan carried up' : 'home: no caravan today, the road stopped by the weather' });
  }
  ...
  private terracePorter(): Seg[] {
    ...
    // "wait" is the morning at the depot for the day's caravan: the camp's barley carried up until it comes, then its sacks
    // up to the Treasury store; the afternoon is never a wait (the caravan comes once a day: A S4 of shadow review r8)
    const task = (pmHalf: boolean): [string, ActivityId, string] => { const w = { ...(L.job_tasks.terrace_porter.v as Record<'treasury' | 'camp_grain' | 'site_water' | 'wait', number>) }; if (pmHalf) w.wait = 0; const k = this.choose(w);
    ...
    let [pl, act, why] = task(false); const pm = task(true), ins: [number, number, string, ActivityId, string][] = [];
    if (pl === 'stair_foot') { const c = this.P.caravan(this.d), done = this.P.caravanDone(this.d); [pl, act, why] = ['querns', 'carry_sack', 'carrying sacks of barley from the depot up to the work camp’s querns'];
      if (c && done !== null) ins.push([c.h, done, 'treasury_store', 'carry_sack', 'carrying the caravan’s sacks up from the stair foot to the Treasury store']); }
  ...
  private treasuryWorker(): Seg[] {
    ...
    if (k2 !== 'craft' && k2 !== k && this.t < 12.5) { this.workBlock(p.work, 'town', act, why, Math.max(7, this.t), 12.7, false);
      ...
      else { this.go(this.home, this.homeW, 'taking work home'); this.atHome(15, homeAct, p.sub === 'textile' ? homeWhy : 'finishing the work at home'); } }
  ...
  private groom(): Seg[] {
    ...
    const task = (x: string) => { if (x === 'water' && wetHours(C.wx, this.t, this.t + 2.3) === 0 && this.t + 2.3 < this.lightEnd()) this.errand('river', 'town', 'tend_animals', 'taking the horses to water at the river', 2, 'station', 'town');
    ...
    task(k); let k2 = this.choose(L.job_tasks.groom.v as Record<'station' | 'water' | 'clean' | 'lead', number>);
    // (one horse led along the road in a day, and none through the afternoon heat of an E-64 day: planCheck (g), the soak's
    // sweep on D-197, 3699 on day 110 on foot 7.03 h in 39 °C)
    if (k2 === 'lead' && (k === 'lead' || C.heatRest)) k2 = 'clean';
  ...
  private craftsman(): Seg[] {
    ...
    if (k === 'deliver' && this.t < 11 && wetHours(this.C.wx, this.t, this.t + 2.6) === 0) this.errand('worksite', 'terrace', 'carry_sack', 'carrying pigment and whitening up to the Terrace works (PW2017: the palette matches the Terrace)', 2, 'craft_zone', 'town');
    ...
    if (k2 !== k && this.t < 12) { const [pl, why] = bench(k); this.workBlock(pl, 'town', 'craft', why, Math.max(7, this.t), 12.7, false);
      if (k2 === 'deliver' && wetHours(this.C.wx, this.t, this.t + 2.6) === 0) this.errand('worksite', 'terrace', 'carry_sack', 'carrying pigment and whitening up to the Terrace works (PW2017: the palette matches the Terrace)', 2, 'craft_zone', 'town');
  ...
  private official(): Seg[] {
    ...
    // (the works are open ground: not inspected in the rain or a storm; the soak's sweep on D-197, the works' days moved by the
    // gangs' dry-hours rule)
    if (P.cal.construction.log.some(e => e.day === this.d - 1 && (e.kind === 'shaft_complete' || e.kind === 'capital_set' || e.kind === 'fluting_done')) && wetHours(C.wx, 7, 17) === 0) stops.push(['worksite', 'terrace', 'inspect', 'inspecting the building works', 1]);
    if (P.parties.some(x => x.day === this.d)) stops.push(['station', 'town', 'inspect', 'checking a party’s halmi at the road station', 0.8]);
    if (!stops.length || r.chance(0.5)) { const x = r.pick(wetHours(this.C.wx, 7, 17) > 0 ? ['apadana_hall', 'gate_hall'] : ['forecourt', 'apadana_hall', 'gate_hall']); stops.push([x, 'terrace', 'inspect', `an inspection round of the ${x.replace('_', ' ')}`, 0.8]); }
    if (C.festival && P.sipPresider(this.d) === this.pid) { const F = C.festival; this.morning(F.hours[0] - P.walkH(this.home, 'offering_place', this.d, this.homeW, 'town') - 0.3);
      this.go('offering_place', 'town', 'to the offering place for the šip'); this.add(Math.max(this.t + 1, F.hours[1]), 'offering_place', 'inspect', 'presiding over the šip on the open ground: the goods issued, the meat shared out to the households (E-33)', 'town');
      this.go(this.home, this.homeW); return this.homeRest(); }
  ...
  private messenger(): Seg[] {
    ...
    const wait = (until: number) => { for (let g = 0; g < 12 && this.t < until - 0.1; g++) {
      ...
      const lastW = this.segs[this.segs.length - 1]?.why === 'waiting at the station for the relay'; // (a wait is not followed by another: no idle hours, planCheck (c))
      const k = hot ? 'sleep' : this.choose({ wait: lastW ? 0 : 0.45, horse: 0.3, talk: 0.25 });
  ...
  private shepherd(): Seg[] {
    ...
    if (C.slaughter.length && p.id % 4 === 0) return this.dayWork('stockyard', 'town', 'slaughter', 'slaughtering small cattle at the stockyard (E-12)', Math.min(...C.slaughter), Math.min(...C.slaughter) + 3, true);
    ...
    // (a storm, as the rain, by its hours: the flock goes out when a morning storm has passed and is brought in to the fold
    // when one comes on, workBlock; kept in the fold all day when it holds the day, dryTask; A S3 of shadow review r8)
    ...
    if (this.P.dryTask({ h0: this.sun.rise + 0.3, h1: this.sun.set - 0.5 } as PTask, C) === null) return this.dayWork('stockyard', 'town', 'tend_animals', 'the flock kept in the fold at the stockyard out of the rain, fed on fodder (W-01)', this.sun.rise + 0.4, this.sun.set - 1, false, 'the midday meal under the fold’s roof, out of the rain'); // (S1 r5)
  ...
  private caretaker(): Seg[] {
    ...
    return this.terraceWorker(`palaces:${b}`, k === 'clean' ? 'clean' : k === 'water' ? 'carry_jar' : 'rest', k === 'clean' ? `cleaning the closed ${b === 'gate' ? 'Gate' : b}` : k === 'water' ? `carrying water to the ${b === 'gate' ? 'Gate' : b}` : `keeping the door of the closed ${b === 'gate' ? 'Gate' : b}`, undefined, [], { roofed: k !== 'water' });
  }
  private gardener(): Seg[] {
    ...
    // (a storm counts as the rain does, by its hours: dryTask; A S3 of shadow review r8, a storm anywhere in the day's light
    // kept him in)
    if (dry === null) return this.homeDay(this.weatherOff('no garden work', [this.sun.rise + 0.5, 15.5]));
    ...
    const block = (t1: number) => {
      ...
      else if (k === 'produce' && t1 - this.t > 2 && wetHours(C.wx, this.t, this.t + 2.2) === 0) { this.go(fruit ? trees : g, 'town', fruit ? 'to the garden trees' : 'to the garden'); this.add(this.t + 1, fruit ? trees : g, fruit ? 'pick_fruit' : 'garden_work', 'gathering produce for the store', 'town');
      ...
      else if (k === 'manure' && t1 - this.t > 2 && wetHours(C.wx, this.t, this.t + 1.2) === 0) { this.go('stockyard', 'town', 'to the stockyard'); this.add(this.t + 0.4, 'stockyard', 'carry_sack', 'filling baskets with dung', 'town'); this.go(g, 'town', 'carrying dung to the beds', 'carry_sack');
    ...
    block(12); if ((this.cur ?? this.home) !== this.home && wetHours(C.wx, this.t, this.t + 0.7) > 0) this.go(this.home, this.homeW, 'home out of the rain');
    if ((this.cur ?? this.home) === this.home) this.noonAtHome(); else this.add(this.t + lerp(L.meals.midday_work_h[0], L.meals.midday_work_h[1], this.r.next()), this.segs[this.segs.length - 1].place, 'eat', C.heatRest ? 'the midday meal in the shade of the trees, then home out of the heat (E-64)' : 'the midday meal in the garden', 'town'); if (!C.heatRest) block(15.5);
  ...
  private farmer(): Seg[] {
    ...
    // (a storm keeps him in only when it leaves the day's field task no dry working stretch: Population.dryTask; a storm that
    // comes on later sends him home from the field, Planner.workBlock; A S3 of shadow review r8)
    if (dayStorm(C) && !T) return this.homeDay(stormWord(C.wx, this.sun.rise - 0.5, this.sun.set + 0.5));
    ...
    if (T.kind === 'other' && T.opt === 'market') {
      ...
      // (the exchange ends before the rain or the storm comes on: the storm by its hours now, A S3 of shadow review r8)
      const x1 = this.t + lerp(1, 2, r.next()), ws = wetSpells(C.wx).find(([a, b]) => b > this.t && a < x1), st = C.wx.stormH;
      const stop = ws ? Math.min(x1, ws[0] - (st && ws[0] <= st[1] && ws[1] >= st[0] ? 0.25 : 0)) : x1, done = stop - this.t >= 0.3;
      if (done) this.add(stop, pl, 'exchange', town ? T.why : 'exchanging produce in kind with neighbours in the lane', pw);
      if (done && town && this.t + trip > this.hd.noon + 0.6 && wetHours(C.wx, this.t, this.t + 0.3) === 0) this.add(this.t + 0.3, pl, 'eat', 'bread and water in the town before the road home', pw);
      if (!done) this.go(this.home, w, 'carrying the barley home again, the weather coming on', 'carry_sack');
      else this.go(this.home, w, town ? 'carrying home what the barley fetched: oil and a jar' : 'going home', town ? 'carry_jar' : 'walk');
    ...
    if (T.kind === 'thresh' && p.sex === 'm' && this.age >= 16) {
      ...
      if (u < 0.1 && trip < 3) { this.morning(this.sun.rise - 0.2); this.go('store_town', 'town', 'carrying grain to the storehouse in the town', 'carry_sack'); this.add(this.t + lerp(0.35, 0.7, r.next()), 'store_town', 'queue', 'waiting for the grain to be measured and sealed for', 'town'); this.go(this.home, w, 'walking home');
    ...
    if (T.sheaves && T.late > this.t + 0.5) { this.homeHours(T.late, this.C.wx.tmax >= 30 ? 'resting through the heat' : 'resting after the midday meal'); this.errand(`threshing:${q}`, 'plain', 'carry_sack', 'carrying sheaves to the threshing floor with the household (E-43)', 1.2, this.home, w); }
    else if (act === 'reap' && T.late > this.t + 0.5 && p.sex === 'm' && r.chance(0.4)) { this.homeHours(T.late, this.C.wx.tmax >= 30 ? 'resting through the heat' : 'resting after the midday meal'); this.errand(place, 'plain', 'reap', T.kind === 'other' ? 'cutting the last of the sesame and standing the bundles up to dry' : 'binding the last sheaves and gleaning the stubble', 1.3, this.home, w); }
  ...
  private servant(): Seg[] {
    const r = this.r, p = this.p, P = this.P, W = this.homeW;
    ...
    // a town house's servant eats the morning bread with the house (was: "bread and water before leaving" for a man who does
    // not leave); the house's water is his when he is its waterer (the water pass draws its jars: no third trip of his own)
    this.morning(this.rise() + 0.5, !estate); const waterer = this.hd.waterer === this.pid;
    ...
    let last = ''; for (let g = 0; g < 60 && this.t < 17 - 0.02; g++) { if (!ate && this.t > 11.7) { ate = true; this.atHome(this.t + 0.5, 'eat', 'the midday meal with the household'); continue; }
      ...
      const left = tasks.filter(x => !spent(x)); let [k, a, why] = left.length ? r.pick(left) : r.pick(tasks); last = why;
      const lim = Math.min(17, ate ? 99 : Math.max(this.t + 0.3, 12));
      const need = k === 'fuel' ? 2.5 : k === 'wash' ? 2.1 : k === 'errand' ? 1.6 : 1;
  ...
  private elder(): Seg[] {
    const r = this.r, p = this.p, C = this.C, W = this.homeW; this.morning(this.rise() + 0.8 + r.next() * 1.2, true); this.healerCalls(); const l = `lane:${this.hh.q}`;
    ...
    const one = (until: number) => {
      ...
      else if (k === 'work') { // (a little grinding once a day at most, 0.35-0.7 h: the saddle quern is heavy work; the rest of her light work is the spindle: the quibble on 33598, shadow review r5; C)
        const g = r.chance(0.5) && !this.segs.some(s => s.act === 'grind'); this.atHome(p.sex === 'f' && g ? Math.min(t1, this.t + r.range(0.35, 0.7)) : t1, p.sex === 'f' ? (g ? 'grind' : 'spin') : 'craft', p.sex === 'f' ? (g ? 'light work: a little grinding' : 'light work: spinning') : 'mending baskets and tools'); }
  ...
  private homemaker(): Seg[] {
    ...
    if (this.hh.zone === 'plain' && T?.all && !dayStorm(C) && u01(this.P.seed, S.assign, 7950 + this.pid, this.d) < L.homemaker.harvest_share) {
      ...
      else { const HA = L.homemaker.harvest_afternoon; const dh = C.wx.dustH, k = this.choose({ grind: HA.grind, weave: HA.weave, lane: wetSoon(1.3) || (dh && dh[0] < this.t + 1.3 && dh[1] > this.t) ? 0 : HA.lane }); // (not in the dust, W-03: S5 r5)
      ...
      if (r.chance(0.6) && !C.wx.wet && this.hd.supper - this.t > 0.75 && this.canDraw()) this.well(this.t + 0.4, 'fetching water'); this.evening(Math.max(this.t, this.sun.set - 1)); return this.finish();
    }
    this.morning(this.rise() + 2.3, true); this.rationRun(); this.go(this.home, this.homeW); this.healerCalls();
    ...
    // a seller at her door in the lane round the midday meal, a day in twenty (lives.json lane_seller; D-211, C)
    const sellA = Math.max(this.t + 0.1, 10.7), sellB = 13.4 + 0.4 * r.next();
    if (this.hh.zone === 'town' && this.age >= 25 && this.age <= 60 && !this.nursing && u01(this.P.seed, S.fam, this.pid, this.d, 81) < L.lane_seller.p && sellB - sellA > 1.5 && this.fairOut(sellA - 0.2, sellB + 0.2)) {
      if (this.t < sellA - 0.2) this.homeHours(sellA - 0.1, 'at home'); this.go(l, this.homeW, 'out to the lane with a tray of bread and greens', 'carry_bread');
      const wares = this.r.pick(['bread and greens', 'roasted grain and dried figs', 'flat bread and curds', 'onions, cress and bread']), noon = Math.max(this.t + 0.4, this.hd.noon);
      this.add(noon, l, 'exchange', `selling ${wares} from a tray at her door in the lane, for barley or oil in kind`, this.homeW); this.add(this.t + 0.3, l, 'eat', 'her own bread in the lane between buyers', this.homeW);
      this.add(Math.max(this.t + 0.4, sellB), l, 'exchange', `selling ${wares} from a tray at her door in the lane, for barley or oil in kind`, this.homeW); this.go(this.home, this.homeW, 'home with the tray and what it fetched', 'carry_bread');
      this.homeHours(Math.max(this.t + 0.5, 15 + r.next()), 'at home'); this.evening(Math.max(this.t, this.sun.set - 1)); return this.finish(); }
    ...
    if (r.chance(0.5) && !C.wx.wet && this.canDraw()) this.well(this.t + 0.4, 'fetching water'); this.evening(Math.max(this.t, this.sun.set - 1)); return this.finish();
  }
  ...
  private traveller(): Seg[] {
    ...
    let [pl, wh, a, why] = stops[(k - 1 + p.id) % stops.length];
    // (the walk is named for where it goes: S7 of reviewer A, r7, "on the day's business" and then sightseeing; a queue at
    // the storehouse is not an hour and a half; the lanes are not for the rain or the dust)
    if (pl.startsWith('lane:') && (wetHours(this.C.wx, this.t, this.t + 3) > 0 || (!!this.C.wx.dustH && this.C.wx.dustH[0] < this.t + 3 && this.C.wx.dustH[1] > this.t))) [pl, wh, a, why] = ['station', 'town', 'craft', 'mending harness, bags and sandals for the road'];
    const len = a === 'queue' ? lerp(0.3, 0.65, r.next()) : 1.5 + r.next();
    if (pl !== 'station') this.go(pl, wh, a === 'queue' ? 'to the storehouse for the travel rations' : a === 'exchange' ? 'into the town to trade' : /lower town/.test(why) ? 'down into the lower town' : pl === 'official_bldg' ? 'to the official building' : pl === 'treasury_desk' ? 'up to the Treasury' : 'on the day’s business'); this.add(this.t + len, pl, a, why, wh); if (a === 'queue') this.go('station', 'town', 'carrying the rations back to the station', 'carry_sack'); else this.go('station', 'town');
    ...
    const spell = (until: number) => { for (let g = 0; g < 12 && this.t < until - 0.3; g++) { const V = L.travellers_stay.spell;
      ...
      const x = this.choose({ rest: V.rest, gear: V.gear, animals: this.C.wx.wet || watered || until - this.t < 2 ? 0 : V.animals, gamble: V.gamble, lane: this.C.wx.wet || walked || until - this.t < 1.8 || wetHours(this.C.wx, this.t, this.t + 1.6) > 0 || (!!this.C.wx.dustH && this.C.wx.dustH[0] < this.t + 1.6 && this.C.wx.dustH[1] > this.t) ? 0 : V.lane });
    ...
    if (p.rank !== 1) { const [pl2, wh2, a2, why2] = stops[(k + 1 + p.id) % stops.length]; if (pl2 !== pl && pl2 !== 'station' && !(pl2.startsWith('lane:') && (wetHours(this.C.wx, this.t, this.t + 2.5) > 0 || (!!this.C.wx.dustH && this.C.wx.dustH[0] < this.t + 2.5 && this.C.wx.dustH[1] > this.t)))) { this.go(pl2, wh2, 'on another errand'); this.add(this.t + (a2 === 'queue' ? lerp(0.3, 0.65, r.next()) : 0.8 + r.next()), pl2, a2, why2, wh2); if (a2 === 'queue') this.go('station', 'town', 'carrying the rations back to the station', 'carry_sack'); else this.go('station', 'town'); } }
    if (k >= 2 && r.chance(0.5)) { this.go('store_town', 'town', 'to the storehouse for the travel rations'); this.add(this.t + lerp(0.3, 0.65, r.next()), 'store_town', 'queue', 'drawing the next days’ travel rations', 'town'); this.go('station', 'town', 'carrying the rations back to the station', 'carry_sack'); } // (A S9 of r9: the rations carried, not walked back empty-handed)
  ...
  private herderPassing(): Seg[] {
    ...
    const at = (t1: number, place: string, act: ActivityId, why: string, where: Where = W, carry?: string) => { if (t1 <= this.t + 0.015) return; [place, act, why] = this.herdShelter(place, act, why, t1); this.add(Math.min(24, t1), place, act, why, where); const s = this.segs[this.segs.length - 1];
      if (carry) s.carry = carry; if ((role === 'little') && mom >= 0) s.with = mom; };
    // (a child under seven rides the loads after the midday halt: S11 of reviewer B, r7, a child of 6 walked 6 h 45 min; C)
    ...
    // (the flock's work waits for first light: S6 of reviewer A, S6 of reviewer B, r7, the flock counted 62 min before sunrise)
    if ((role === 'woman' && !watched && B.milk) || (withFlock && B.moving)) { const l0 = this.sun.rise - 0.4; if (this.t < l0 - 0.05) at(l0, camp(c0), 'rest', 'up in the tent before first light'); }
  ...
  private atCamp(role: string, c: number, B: BandDay, first: boolean, watched = false) {
    ...
    const at = (t1: number, place: string, act: ActivityId, why: string) => { if (t1 <= this.t + 0.015) return; [place, act, why] = this.herdShelter(place, act, why, t1); this.add(Math.min(24, t1), place, act, why, 'plain'); if (role === 'little' && mom >= 0) this.segs[this.segs.length - 1].with = mom; };
  ...
  /** the band's people out of the rain, and at leisure out of the dust (D-191, planCheck (a)): the stream's water, play and
   *  chores go to the tent while it rains (W-01), the donkeys are hobbled by the tents; the flock's own work stays out with it */
  private herdShelter(place: string, act: ActivityId, why: string, t1: number): [string, ActivityId, string] {
    // (the camp's work and leisure by the tents go into the tent in the rain, and at leisure in the dust; the ground loom is
    // covered and the spindle taken in: B S2 and A S10 of shadow review r9, 138 of 300 herder person-days on wet days; C)
    if (place.startsWith('camp:') && CAMP_OPEN.test(why)) { const wx = this.C.wx, wet = wetHours(wx, this.t, t1) > 0, dust = !!wx.dustH && /^(play|rest|talk|spin)$/.test(act) && Math.min(t1, wx.dustH[1]) - Math.max(this.t, wx.dustH[0]) > 0.05;
      if (!wet && !dust) return [place, act, why]; const w = wet ? 'out of the rain' : 'out of the dust';
      if (act === 'weave' || act === 'spin') return [place, 'spin', act === 'weave' ? `spinning in the tent ${w}, the ground loom covered` : `spinning in the tent ${w}`];
      if (act === 'play') return [place, 'play', `playing in the tent ${w}`];
      if (act === 'talk') return [place, 'talk', /fire/.test(why) ? `by the fire under the tent’s front, ${w}` : `talking in the tent ${w}`];
      return [place, 'rest', /minding/.test(why) ? `minding the little ones in the tent ${w}` : `in the tent ${w}`]; }
    if (!place.startsWith('water:')) return [place, act, why]; const wx = this.C.wx, camp = place.replace(/^water:/, 'camp:');
    const wet = wetHours(wx, this.t, t1) > 0, dust = !!wx.dustH && /^(play|rest|talk)$/.test(act) && Math.min(t1, wx.dustH[1]) - Math.max(this.t, wx.dustH[0]) > 0.1;
    if (!wet && !dust) return [place, act, why]; const w = wet ? 'out of the rain' : 'out of the dust';
    if (act === 'tend_animals') return [camp, 'tend_animals', 'hobbling the donkeys by the tents'];
    return act === 'play' ? [camp, 'play', `playing in the tent ${w}`] : [camp, 'rest', `in the tent ${w}`];
  }
