...
interface DayPlan { day: number; n: number; t1: Float32Array; place: Int32Array; act: Uint8Array; where: Uint8Array; carry: Int32Array; withP: Int32Array }
interface PS {
...
export class PopView {
  ...
  private compact(day: number, segs: Seg[]): DayPlan {
    const n = segs.length, D: DayPlan = { day, n, t1: new Float32Array(n), place: new Int32Array(n), act: new Uint8Array(n), where: new Uint8Array(n), carry: new Int32Array(n), withP: new Int32Array(n) };
    segs.forEach((s, i) => { D.t1[i] = s.t1; D.place[i] = this.str(s.place); D.act[i] = ACT_IX.get(s.act) ?? 0; D.where[i] = W_IX.get(s.where) ?? AWAY; D.carry[i] = s.carry ? this.str(s.carry) : -1; D.withP[i] = s.with ?? -1; });
  ...
  private evaluate(s: PS, t: number) {
    ...
    if (P.where[i] === ROAD) {
      ...
      if (r === undefined) { this.stats.routeWait++; if (from.out) { s.mode = 1; s.spot = from; s.route = null; s.act = i0 > 0 ? ACTS[P.act[i0 - 1]] : 'rest'; s.carry = -1; s.speed = 0; s.what = `${from.what} (waiting for a route)`; } else s.mode = 0; s.v0 = t; s.v1 = t; return; } // over this update's search budget: ask again
      ...
      if (t < S0) { // not yet gone: still at the place before, doing what was done there
        s.mode = from.out ? 1 : 0; s.spot = from; s.route = null; s.v0 = T0; s.v1 = S0; s.act = i0 > 0 ? ACTS[P.act[i0 - 1]] : 'rest'; s.carry = -1; s.speed = 0; s.what = `${from.what} (leaves ${fmtH(S0 - base)})`; return; }
      ...
      s.act = ACTS[P.act[i]]; s.carry = P.carry[i]; s.speed = r.len / Math.max(1, (T1 - S0) * 3600); s.what = `walking: ${from.what} → ${to.what} (${r.len.toFixed(0)} m)`;
    ...
    if (i > 0 && ip >= 0 && P.where[ip] !== ROAD && P.where[ip] !== AWAY) { // a change of place with no walk in the plan: walked from the start of the block
      ...
      if (pr.ok && Math.hypot(pr.e - sp.e, pr.n - sp.n) > 0.8 && (pr.out || sp.out)) {
        ...
        if (r) { const dur = r.len / this.pace(s.pid) / 3600, t1 = Math.min(base + P.t1[i], t0 + dur);
          if (t < t1) { this.stats.steps++; s.mode = 2; s.route = r; s.w0 = t0; s.w1 = t1; s.wOut = true; s.spot = sp; s.v0 = t0; s.v1 = t1; s.act = 'walk'; s.carry = -1; s.speed = r.len / Math.max(1, (t1 - t0) * 3600); s.what = `stepping ${pr.what} → ${sp.what}`; if (!pr.out) s.entry = 1; return; }
          s.v0 = t1; } } }
    s.mode = sp.out ? 1 : 0; s.spot = sp; s.route = null; if (s.v0 < t0 || s.v0 > t) s.v0 = t0; s.v1 = base + P.t1[i];
    s.act = ACTS[P.act[i]]; s.carry = P.carry[i]; s.speed = 0; s.what = sp.what;
  ...
  private vp(): ViewPerson { let o = this.out[this.nOut]; if (!o) { o = { pid: -1, e: 0, n: 0, y: 0, heading: 0, act: 'rest', moving: false, prop: null, carryNote: null, speed: 0, entry: 0, what: '', agent: -1, plot: 0, wall: 0 }; this.out[this.nOut] = o; } this.nOut++; return o; }
  ...
  private agentsOff(t: number) {
    ...
    for (const a of this.sim.agents) { if (!a.offmap) continue; const sp = this.agentSpot(a, t); if (!sp) continue; n++;
      const o = this.vp(); o.pid = a.pid; o.agent = a.id; o.e = sp.e; o.n = sp.n; o.heading = sp.heading; o.moving = sp.moving; o.speed = sp.moving ? a.speed : 0; o.act = sp.moving ? 'walk' : (a.task?.act ?? 'rest'); o.plot = 0; o.wall = 0;
