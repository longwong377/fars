...
- **Read first: what is broken, weak or unverified.**
  ...
  - The riser and bench are a shading tilt, not geometry: silhouettes and cast shadows keep the DEM's smooth 30 m form;
    the tilt fades out once a 4.2 m riser spans under ~1.4 px (with the rig's 40° lens at 540 rows: ~2.3 km). The landform
    maps (gullies, curvature, the uphill slope read for aprons and fans) stop at the mid ring (±10 km).
  - The woodland canopy dots keep the plan grid (the 3-D trees near the camera mirror it); only their shape is corrected
    on slopes. The chips and the herb layer are plan-projected, masked off steep ground as before (stretch ≤ 1.25 where
    they are at full strength).
  - Plots beyond ~5 km stay sub-pixel from the Terrace (a 100 m plot at 5 km from 14 m up is ~0.3 px tall); the far
    field now varies by 800 m district (irrigated fallow share, as the rain-fed rotation), not by plot. No new impostor
    layer: the river and canal trees already stand as impostors to 20 km (census below).
  - The town's roofs and lanes are a tone change only (C, Q-525); the town casts no sun shadow beyond 150 m
    (settlement SHADOW_RANGE, unchanged) and nothing beyond the 600 m cascades. Smoke is another workstream.
- **R9, the Naqsh-e Rustam moiré: cause found and fixed in code.** Approach 1 (D-217, bump band-limit) was not it.
  Approach 2, the terrain horizon, is excluded by measurement: the CPU horizon map gives full sun on every face vertex at
  10:00 and 15:00 on day 0. Approach 3, shadow acne, is the cause: `nr_rock` is DoubleSide, and three r186 draws a
  DoubleSide material DoubleSide into the shadow maps, so the lit face wrote its own depth. At the naqsh-200m time (day 0,
  15:00) the sun stands at grid azimuth 271°, along the face (mean N·L 0.11): the depth slope is ~9 × the cascade texel
  against a 0.06 m normal bias, and the acne drew fine wavy lines over the whole face. Fix: `shadowSide = BackSide` on the
  cliff (one outward-facing sheet: face +z, top up; the east return's winding was inward and is flipped), so its back faces
  alone cast, as the terrain's FrontSide chunks already do. Also band-limited: the materials' fine and middle mottling
  octaves and the streaks' fine octave (all surfaces; they aliased at a distance).
- **Fix 8, rock on the hills (terrainPlain.ts).** A cliff package is a riser (its top 35 %, 2.4 × the mean slope, ≤ 76°)
  over a bench, drawn as a world-space tilt of the shading normal that keeps the package's mean slope; the riser bare, pale,
  streaked by run-off; talus on the upper bench; aprons below steep ground (the slope 20–45 m uphill from the landform map);
  gravel fans at gully mouths. Scrub crowns are spheres in a 3-D jittered grid (3 m cubes, lattice-free frame), grey-green
  rather than near-black: no stretching on steep ground.
- **Fix 9, the plain from the Terrace.** The "straight radial beige streaks" were the D-190 desire lines (12 ruled runs from
  sites up to 3 km away to the stair foot, each with a ±6 m trodden band) and the roads (uniform 6–8 m strips with ruled
  edges). Now the paths are a branching net (each site joins the nearest point of the approach, a road or an earlier path;
  wear accumulates on the trunks; 23 runs, 1 at the stair foot; trodden band ±3.5 m), and the roads are worn tracks (cart
  ruts, a ragged herb verge, patches). The irrigated land's fallow share varies by 800 m district (8–32 %, mean 20 %), so
  the far plain reads in blocks. The town's roofs are lighter (a clay-and-straw finish coat) and its lanes and courts up to
  30 % darker by the roofed share within 3 m.
- **Why the plain-stats said `crops 0, margins 0, nearTrees 0`:** those are the near 3-D sets (crops within 30 m, river
  margins, plain trees within 225 m); from the landing none of them is near. The ground is shader-drawn at any distance.
  Census of the stair-noon-plain view (tools/dev/plain_view_census.ts, one ray per 4 × 4 px, node): see Measured.
- **Measured (node).**
  | quantity | before | after |
  |---|---|---|
  | shrub crowns, down/across run ratio at 0 / 45 / 60 / 75° | 1.00 / 1.41 / 2.00 / 3.83 | 1.01 / 1.00 / 1.00 / 1.00 |
  | shrub cover (every cell full) at 0 / 75° | 0.108 / 0.107 of plan | 0.127 / 0.132 of ground |
  | Naqsh face vertices with sun visibility < 0.99 (10:00, 15:00) | – | 0 of 88,988 (horizon excluded) |
  | cliff triangles facing into the rock | east return (≈2.5 k m²) | 77 of 63,733 m² (0.1 %) |
  | worn-path runs / ending at the stair foot | 43 / 12 | 23 / 1 |
  | runs radial to the stair-noon-plain camera (within 12°, in view) | 4 | 0 |
  | worn-path pixels in stair-noon-plain | 720 | 48 |
  | irrigated fallow share by district | 20 % everywhere | 8–32 % (mean 20 %) |
  | terrain fragment shader | 4 texture samples, 26 noise calls | 8, 27 |
  | draw calls / instances added | – | 0 / 0 |
  Census, stair-noon-plain (px of 518,400; after): Terrace 249,600; sky 213,520; fields 21,696 (irrigated 18,448 at
  0.2–2.5 km, rain-fed 3,248 at 1–5 km); trodden 8,880; natural 10,160; town sites 2,336; roads 608; hill 10,944 (>10 km).
...
## D-227 The town and the near plain at a distance: the town's evening fires measured by line of sight, the moments re-timed to the most fires lit at dusk, fire light on the smoke layer; the stair foot's approach track, tether lines, heaps and herb patches (session 8 workstream; D-220 and D-223 "still broken")
- **Read first: what is broken, placeholder or unverified.**
  - **The town at dusk still shows no points of fire, and cannot from these cameras (B49).** Measured with the sim's fires and
    the town as built (tests/town_glow.test.ts, a 0.5 m height raster of every wall, roof and oven top): at the new hour,
    of 886 lit town fires 420 are in the Terrace view's frame (0.45–1.7 km) and 594 in Kuh-e Rahmat's (0.8–2.0 km), and
...
- **Problem.** D-220: "the town-smoke-dusk frame reads as an empty dark plain"; D-223: "the near plain is still an empty
  brown sheet" and "the town at dusk ... needs lamp and hearth glow".
- **The town's fires (measured, tools/dev/town_glow_probe.ts, tests/town_glow.test.ts):**
  - Lit town fires by hour from the sim (hearthSmoke.ts SmokeModel, fire.ts schedules): day 14 866 at 18:00 (sun +5.6°),
...
- **Fire light on the smoke layer (hearthSmoke.ts, landSmoke.ts, world.ts):** each quarter's cell carries `fireE`, the lit
  ...
  128 rays each; Q-562). The shader adds ω · uFire · fireE · HG(−rd.y) to the layer's source (uFire = the fires' ~1900 K
  colour × the sky's fireScale, D-117). One float attribute per cell, 0 draws added. `SmokeModel.setFireLight` runs each
  frame from the fires' lit state.
- **The near plain from the stair top (tests/terrace_foot.test.ts; plain.spec stair-noon-plain):**
  - *The approach as a worn track* (settlement/water.ts): the town place → stair foot line (people_places.json) drawn in the
    roads' mesh with their track material (cart-rut pairs, crown, ragged herb verge, patches; D-223), 8 m wide (APPROACH_W,
    C), meandering like the roads, stopping 3 m short of the stair. It runs straight away from the stair's top, so its bands
...
## D-228 Phase 6+7 review MAJORs M2–M5: the plain's P22 sub-limit met by culling the near trees per tree (view and visible shadows); tiers held to the files' own rule; house geometry flagged PLACEHOLDER; stale status statements corrected (session 8 workstream; REVIEWS/phase6-7.md)
- **Still broken / unverified (read first):** (1) The P22 figures below are a **headless count, not a browser measurement**: tests/plainLib.ts `frameTriangles` models every pass as renderer.info counts it (index count / 3 × instances per draw, zero-instance draws skipped, frustum culling in the main pass, the four CSM cascades with each caster's own onBeforeShadow hook), but the cascades' own frustum culling is not modelled, so a frustum-culled caster counts in all four (an upper bound). Before this change it gives the browser's D-190 figure exactly at village-p22 (+40 calls, 2.560 M), so the method is trusted, but village-p22 has not been re-rendered. (2) No render judges the cull for pop-in. The test below checks it geometrically; a walk with the head turning is not rendered. (3) The town's garden trees (settlement/trees.ts) use the same NearTreeSet without the cull; they are not over any budget, so they are left alone. (4) The Sivand quarry is not built at all: quarries.ts finds no rock within its 100 m (Q-570). Before this change F3 showed Sivand's B on the only quarry drawn, Majdabad.
- **M2, village P22 at high over D-040's plain limit (2 M; was 2.56 M, logged nowhere):**
  - *Measured cause (headless count, P22 view, quality high, 40° lens):* main pass 1.669 M, shadow passes 0.891 M. The near 3-D trees hold every tree within r3 = 225 m all round the camera and are not frustum-culled (their bounds span the world), so the main pass drew all 1,743 of them (0.735 M), behind the camera too, and every one of the 490 shadow casters went into three cascades (0.822 M). The rest: river banks 0.222 M, orchard rows 0.177 M, canal banks 0.117 M, far tree impostors 0.087 M, tracks 0.076 M, margins 0.072 M, the village cell (0.017 M + 0.069 M shadows), river water 0.056 M. The two tomb relief batches (0.566 M) count only when updateReliefs has not run; the browser hides them beyond 1.5 km.
  - *Change (src/world/trees/render.ts `treeViewClass`, `widenedFrustum`, `VIEW_CULL`; src/world/plain/index.ts `cullNear`):* each placed near tree is classed against the view frustum widened by 8°: in view (its crown-box bounding sphere, +0.5 m, meets the frustum), shadow only (a caster whose sphere, swept away from the sun until every point of it is 10 m below the tree's foot, capped at 400 m, meets the frustum), or neither. Trees within 12 m are always in view. A NearTreeSet's records are ordered [in view, shadow only]; the main pass draws the first part (geometry.instanceCount), and the shadow hook draws all of them (nearCascadesOnly takes the set's shadow count; still nothing into cascades that start beyond 180 m). The cull is redone when the view turns 3°, the camera moves 1 m, the sun moves 0.5°, the field of view or aspect changes, or the near set is rebuilt. 8° > 3° + atan(1 / 12), so neither a turn nor a step brings an uncovered tree into the frame between two culls. Neither a culled tree nor its shadow can reach the frame, so nothing visible is lost. Headless callers that pass no camera (only a position) get every tree, as before.
  - *Result (tests/plain.test.ts, same count, quality high):* village-p22 40 calls / **1.275 M** (0.270 M of shadow passes; 135 of 1,743 near trees drawn, 89 of 490 casters) at the rig's 40° lens and 41 calls / **1.414 M** at the player's 70°; pulvar-bank-april 17–27 calls / 0.92–0.98 M; field-april 17–18 / 1.17–1.21 M; stair-dawn-plain 12–15 / 0.88–0.96 M; naqsh-200m 59–61 / 1.01–1.06 M. All under D-040's ≤ 150 calls and ≤ 2 M. Nothing in BLOCKERS: the reserved B51–B52 are unused.
  - *Naqsh-e Rustam, found on the way:* the plain's update switched castShadow on every mesh under the Naqsh-e Rustam group, so the carved tomb relief batches (which set castShadow false: their shadow proxies are the only relief draws in the shadow passes, D-048; D-226 rejected them casting into the CSM) were drawn into all four cascades within 1.2 km. Now the switch covers the group's own meshes only (cliff, façades, panels, the carved text, the Ka'ba, the Elamite relief), not the relief sets'.
  - *The headless budget test:* the old "≤ 2 M before culling" test summed the static meshes with no shadow passes and no camera; it passed while the browser measured 2.56 M. It is kept, renamed, as the check that the plain is a fixed handful of meshes (≤ 40). The budget is now asserted per frame at five Phase 7 views × two lenses with the sun of each view's day and hour, shadow passes included, reliefs settled (`frameTriangles`). A second test checks the cull loses nothing visible at P22: every tree in the exact frustum is drawn, every caster whose shadow reaches it is kept, and after a turn of 2.95° (yaw, pitch, both) or a 0.95 m step with no re-cull every tree in the new exact frustum is still drawn.
  - *Alternatives not needed:* shorter shadow reach or radius (visible loss: the shadows of trees at 100–120 m would go), splitting the rivers, canal banks, tracks and orchard rows into frustum-culled cells (≈ 0.3–0.45 M more at P22; more draw calls), a GPU cull that collapses instances (renderer.info and the vertex stage still count them).
- **M3, tiers above the files' rule "tier = the lower of existence and position" (settlement.json and plain.json `_meta.tiers`; F3 shows the row's tier):**
  - Corrected to C, with the reason appended to each note: settlement.json `pw_area_b_craft` (position ±250 m; its date in 467 is itself C), `frataraka_complex` (absent; position ±250 m), town element `precinct_plinths` (form B at Pasargadae, plinths at Persepolis an analogy: C, D-207, D-209); town.json `craft_zone` (every town.json position is C); plain.json `quarry_majdabad` (map-scale point, accuracy 10 km, moved to rock by the build), `qadamgah` (±3 km), `steppe` and `woodland` (vegetation B by pollen, extent and cover a rule, C), `dam_sang_e_dokhtar` and `bard_burideh` (±5 km), and the five absent later sites at ±3 km (`darre_ye_barre`, `hajjiabad`, `maqsudabad`, `kuh_i_ayyub`, `kuh_i_shahrak`; Q-571). Their chronology.json rows carry the same tier (every feature and its chronology row agreed before; they agree again).
  - Checked with a script over every tiered row of settlement.json, town.json and plain.json: no other row is better than C with a position uncertain by more than 200 m, no town.json quarter or facility is better than C, and no town element is better than its feature. Kept B: rows with a dataset coordinate of ±20–200 m (Sivand ±100 m, the Darius I tomb, the Elamite relief, Istakhr, Naqsh-e Rajab, the two mountains), sub-rows describing a plan or a size (Tol-e Ajori's 2026 plan), and `spring_cemetery` (absent, no position).
  - F3 on the quarries (src/world/plain/quarries.ts): the merged mesh carries C, and its `describe` names the site whose faces were hit with that site's own tier and note (Majdabad C; Sivand B if it is ever built).
  - Lint (tools/lint_chrono.ts), fail-closed: a settlement or plain feature whose position is uncertain by more than 200 m must be C; a feature's tier must equal its chronology row's; a town element may not be above any feature it stands in; every town.json quarter and facility must be C. Verified by setting pw_area_b_craft back to B: the lint fails on two of these checks.
...
## D-238 Comfort defaults: vertical FOV 60°, head-bob ±1.8 cm; frames judged at the default and at the widest (session 8, MASTER_PLAN rev 2, axis K)
- **What.** The default vertical field of view drops from 70° (~102° horizontal at 16:9) to 60° (~91° horizontal); the range
  stays 50–100° in settings. The head-bob stays on by default but its amplitude falls from ±2.5 cm to ±1.8 cm (T-K3 < 2 cm), with
  no camera roll. Axis C judges frames at the default and at the widest setting.
- **Why.** Brief §1.1 wants true scale felt: a wide lens shrinks the 18 m columns and the Gate's bulls. A vertical 55–60° is the
  usual first-person choice for felt scale; 60° keeps peripheral awareness for walking in crowds. Bob under 2 cm keeps walking
  felt without motion discomfort (the critique of MASTER_PLAN rev 1, change 10). A saved settings file keeps the player's own
  choice; only new settings take the default.
- **Measured.** Tests that set the FOV themselves (culling at 70°, 40°, 25°) are unchanged; nothing asserted the default.
  Not rendered at 60° yet: the next Tier-2 pass is at the default.

## D-239 A new game begins shortly before the court arrives (session 8, UD-09, UD-10; tier C)
- **What.** With the court coming and going by default (D-236), a new game (a fresh world seed, D-236) starts at dawn on a day
  drawn by the seed 1–3 days before the court's seed-chosen arrival in its probable season, so the first session can contain
  the defining event (the court entering, the town's preparations, the road's traffic). Players can still pick any date.
- **Depends on.** The court's arrival simulated as an event with a before and after (today the court is present from day 0 and
  only its departure is simulated: the Phase 5 fixes report, D-229). Not implemented yet; it is P1 work in MASTER_PLAN §6.
- **Tier.** C: the date is the seed's, labelled reconstructed in F3 and the translation layer.

## D-240 The world's edge: no invisible walls; everything reachable is on the board (session 8, UD-06; MASTER_PLAN §4.3)
- **What.** The walkable envelope (every point the controller can physically reach from the spawn: slope ≤ 42°, step-up,
  colliders on every terrain ring) is the world. The area registry (`data/areas.json`, generated by `tools/dev/areas.ts`, never
  from the nav grid) covers it. Land beyond the fields' 40.96 km zone is built to the bar by the landscape generator (steppe,
  scrub, pastoral camps, tracks, the mountains), as generator-class areas sampled like the villages; where content cannot
  reach the bar, only real terrain the controller cannot climb may bound the world, never an invisible wall.
- **Why.** Audit D: walk coverage was measured against a 35 ha nav grid while the player can walk 143 × 143 km. A denominator
  the build defines for itself always looks good (critique change 4).
- **Rejected.** An invisible wall at the fields' edge (a hard break by T-H0); leaving the far land unmeasured (hides failure).

## D-241 Talk in dead languages: depth from gesture, action, memory and the translation layer (session 8, UD-08; axis K)
- **What.** Speech stays in the period languages from published texts (brief §10). Its depth cannot come from long dialogue, so
  it comes from: per-person voice parameters (no two voices alike), a line pool per language and role with ≤ 10 % repeats over 50
  addresses per area (T-K1r), gesture first (a greeting, a refusal, directions pointed), people who remember the player (T-K2),
  and the translation layer (who this person is: name, household, work, history, sourced and tiered).
- **Rejected.** Invented sentences in Old Persian, Elamite or Aramaic beyond published vocabulary (brief §10); English in the
  world.

## D-242 Weather in coverage samples follows the climate, with a floor for rare states (session 8; MASTER_PLAN axis B)
- **What.** The Tier-1 sampler draws weather by the simulated climate's frequency for the month, with at least one view per area
  per rotation for each of the nine states (T-B1w). Equal weighting would over-sample snow and under-sample ordinary days
  (the critique's B7.6).

## D-243 Evidence retention: ≤ 25 MB per session in git (session 8; MASTER_PLAN §4.2)
